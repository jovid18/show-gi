...
**[docs/08-playtest.md](../../../docs/08-playtest.md) §11에서 관측된 것들을 매번 다시 본다.** 그때는 로컬에서 `positions`가 테스트 픽스처 2행뿐이었고 `explain_cache`가 비어 있었다. prod에서도 같은지 확인하고, 달라졌으면 그것 자체가 회차의 발견이다.
...
4. **개입 분포** — 카테고리별 합계. `other` 비율을 [06-status.md](../../../docs/06-status.md) §17의 65~70%와 대조한다
