...
- **`ILLEGAL … not in legalMoves`** — 브리지가 낸 것이지 서버 응답이 아니다. 합법수 목록에 없는 수는 서버로 안 가므로, 서버가 들고 있는 일본어 사유(`!! REJECTED …`)도 안 나온다. 원인은 成을 빠뜨린 것(`7c8d`와 `7c8d+`는 다른 수다 — 出発マス가 敵陣이면 成할 수 있다)이거나 成駒의 움직임을 잘못 안 것이다([§91](../../../docs/journal/82-100.md))
