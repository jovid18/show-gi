...
**반드시 한 메시지에서 세 번 호출한다.** 순차로 띄우면 병렬로 안 돈다.
...
- **`ILLEGAL … not in legalMoves`** — 브리지가 낸 것이다. 합법수 목록에 없는 수는 서버로 안 가므로, 서버가 들고 있는 일본어 사유(`!! REJECTED …`)도 안 나온다. 원인은 成을 빠뜨린 것(`7c8d`와 `7c8d+`는 다른 수다 — 出発マス가 敵陣이면 成할 수 있다)이거나 成駒의 움직임을 잘못 안 것이다([§91](../../../docs/journal/82-100.md))
...
- **맥이 자면 세 판이 한꺼번에 죽는다** — 뚜껑을 닫으면 pong 이 끊기고, 서버의 핑(45초)이 `writeTimeout` 10초를 넘겨 세션을 닫는다. Caffeine 은 디스플레이 잠자기만 막아서 이걸 못 막는다 — 회차 내내 `caffeinate -s` 를 걸거나 뚜껑을 열어 둔다([§91](../../../docs/journal/82-100.md))
