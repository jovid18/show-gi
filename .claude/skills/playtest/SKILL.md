...
목적은 승패 대신 「개입과 힌트가 사람에게 실제로 통하는가」를 재는 데 있다. 에이전트 셋이 prod에서 각자 한 판씩 끝까지 두고, 그 경험을 한 문서로 합친다.
...
**반드시 한 메시지에서 세 번 호출한다.** 순차로 띄우면 병렬로 돌지 않는다.
...
| `a2`     | 9972 | **신중형** — 玉을 굳히고 재료를 지킨다. 확실할 때만 교환한다                                                                          | 개입이 적은 판. 「개입이 뜨지 않는 것도 경험인가」 |
...
- 끝나면 `curl -s :PORT/f`를 한 번 부르고, 40줄 이내로 보고한다:
  1. 결과와 手数
  2. 개입 중 이해된 것과 이해되지 않은 것 — 문구를 그대로 인용하고, 무엇이 부족했는지
...
에이전트의 기억 대신 DB가 근거다. §1에서 적어둔 시각(`FROM`)으로 이 회차의 판만 고른다.
...
5. **문구별 평가** — 카테고리마다 「통했다 / 통하지 않았다」와 근거. 에이전트 셋의 판단이 갈리면 갈렸다고 적는다. 합의된 것처럼 쓰지 않는다
...
- **`ILLEGAL … not in legalMoves`** — 브리지가 내보낸 것이다. 합법수 목록에 없는 수는 서버로 가지 않으므로, 서버가 가진 일본어 사유(`!! REJECTED …`)도 나오지 않는다. 원인은 成을 빠뜨린 것(`7c8d`와 `7c8d+`는 다른 수다 — 出発マス가 敵陣이면 成할 수 있다)이거나 成駒의 움직임을 잘못 안 것이다([§91](../../../docs/journal/82-100.md))
- **`!! TIMEOUT`** — depth 12 탐색이 오래 걸린 것이다. 대국은 살아 있으니 `/s`로 다시 본다. 브리지를 재시작하면 판이 사라진다
- **`CLOSED`** — 연결이 끊겼고 그 판은 끝이다. 되살릴 수 없다. 서버는 그 판을 `result=abandoned` 로 닫는다
- **맥이 자면 세 판이 한꺼번에 죽는다** — 뚜껑을 닫으면 pong 이 끊기고, 서버의 핑(45초)이 `writeTimeout` 10초를 넘겨 세션을 닫는다. Caffeine 은 디스플레이 잠자기만 막아서 이걸 막지 못한다 — 회차 내내 `caffeinate -s` 를 걸거나 뚜껑을 열어 둔다([§91](../../../docs/journal/82-100.md))
- **끊긴 것을 클라이언트가 바로 모른다** — 핑이 한 방향이라 다음 착수를 보낼 때 비로소 `CLOSED` 가 나온다. 실제로 6분 걸렸다
- **개입이 반복되는데 힌트가 뜨지 않는다** — 같은 국면에서 세 번은 물러져야 열린다. 국면이 바뀌면 계단이 처음으로 돌아간다
- **포트가 이미 쓰인다** — 이전 회차 브리지가 남아 있다. `pkill -f "skills/playtest/bridge.mjs"`
- **`curl :9971/s` 가 빈 응답을 준다** — 브리지가 `127.0.0.1` 에만 바인딩한다. `http://127.0.0.1:9971/s` 로 부른다
