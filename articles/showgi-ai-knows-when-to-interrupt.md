...
LLM の呼び出しはすべて **OrcaRouter**（OpenAI 互換のメタルーター / AI ゲートウェイ）経由です。
...
決めた原則は、**盤面をそのまま LLM に渡して「この手はどう？」と聞かない**ことです。
...
モデルは固定し、出力の検証と統合テストで監視します。ゲートウェイ側の構成が変わっても、説明の品質低下が対局や悪手判定に波及しないようにするためです。

LLM は介入が必要な局面でだけ呼びます。カテゴリとレベルだけで決まる文は事前生成してキャッシュし、呼び出しを減らしています。
...
初級者には広い閾値を与えて、細かなミスでは止めません。少し慣れた人には閾値を狭めます。判定は、すでに計算された評価値と詰み距離だけを入力にします。

### 終盤だけは別の物差しが要る

勝率への変換にも弱点があります。勝勢になるほど 1 に飽和し、短い詰みを逃しても勝率差がほとんど動かなくなります。

そこで終盤は、着手前に mate solver が証明した詰みの距離を起点に判定します。詰みが消えた手と、詰みは残っていても大きく遠回りした手を分け、説明でも混同しません。
...
着手前に手筋を自動提案する機能は、誤った手を勧めないため止めています。学習サービスでは、出ない機能より間違って教える機能のほうが高くつくからです。
...
ゲートウェイ越しに LLM を呼ぶからこそ、**モデルが変わっても製品が変わらない**ように、「その文が言ってよい事実」を先に決める必要があります。
