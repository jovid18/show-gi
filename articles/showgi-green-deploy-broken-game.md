...
将棋の対局サービスを個人で作っています。対局エンジンを差し替えるプルリクエストをマージしたら、GitHub Actions は最後まで緑、ECS のデプロイも「安定」で完了、サイトも普通に開きました。**対局だけができませんでした。**
...
エンジンを別の実装に乗り換えたので、イメージからは古いバイナリ (`fairy-stockfish`) を消しました。ところが**タスク定義には名前が残っていました**。サーバは存在しないバイナリを起動しようとして、エンジンのプロセスプールを立ち上げられませんでした。
