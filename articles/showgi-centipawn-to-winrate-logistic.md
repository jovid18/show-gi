...
名前は歩を基準にしていますが、**実際の目盛りを決めているのは評価関数のほう**です。やねうら王には `FV_SCALE` という設定があります。公式サイトの説明はこうです — 「評価関数は、高速化のため整数演算で行います。しかし、小数まで扱えた方が精度が高くなるため、普通は最後に 16 (FV_SCALE) で割ります」[^fvscale]。つまり `score cp` は**内部で計算した評価値をこの数で割ったもの**で、既定値が 16 です。適切な値は評価関数ごとに違い、このプロジェクトが使っている水匠5は**この値を 16 から 24 に上げながら学習し直した**評価関数なので、24 で動かします。

[^fvscale]: [たややん、水匠5で+R100アップさせてしまう](https://yaneuraou.yaneu.com/2021/11/26/tayayan-suisho-5-makes-r100-up/) より。同じ記事に「強くなった肝は FV_SCALE を 16 から 24 にしたことです」「水匠5の評価関数を用いる時は、FV_SCALE = 24 にすると顕著に強くなるようです」とあります。既定値と割り算の向きは [量子化誤差とやねうら王の FV_SCALE の件](https://yaneuraou.yaneu.com/2025/02/26/quantization-error-and-yaneuraous-fv_scale/) の「static_eval(静的な評価値) = 内部的に計算した評価値 / FV_SCALE となっている」「このFV_SCALEのデフォルト値は16である」です。
...
この依存は作業の順番まで決めました。**エンジンを載せ替える作業を、判定を書く前に済ませています。** 別のエンジンで当てはめてから載せ替えると、締め切りの直前に、いちばん壊したくない定数を当てはめ直すことになるからです。git のログでもその順で並んでいて、載せ替えが 8 番目、判定が 12 番目のマージです (PR 番号では #8 と #15)。**「後で校正する」と決めた定数があるなら、その定数が乗っている土台を先に確定させておく**、という順番でした。
...
`画面` の行だけは、同じ手を**別の回に**確認したときのものです。パーセントは落差を四捨五入しただけの値なので (3 章 ③ のコード)、落差 0.507 の回なら −51% と出ます。
...
**どちらも勝っている局面です。** 77 手目のいちばん良い手は +537cp で、そこから入門の閾値に届く境界はおよそ **-96cp** (633cp の損) です。**合法手 76 手のうち 68 手がその線を割っていました。** 通る手は 8 手だけで、初心者がその 8 手を見つけられなければ、同じ局面で 68 回戻されます。
...
つまり直すべき方向は閾値ではありません。**68 回の判定はそのつど正しい** — 戻された手は本当に悪い手です。それでも同じ人が同じ場所で止まり続けます。この式が見ているのは**一手がどれだけ悪いか**だけで、**その人が通る手を見つけられるか**は目盛りの外です。そこに要るのは、**同じ局面で戻された回数を数えて、見つけるのを手伝う**という**別の軸**でした。$K$ の目盛りの上には無い話です。
...
三つめは SQL の側でも守っています。評価値を埋めるクエリが触るのは `eval_cp` の列だけで、**`usi` を書き換える口がありません** — ここで書き直せるようにすると、戻された手で上書きする道ができます。upsert にもしていません。無い ply に行を作ってしまうと、棋譜に無い行ができるからです。
