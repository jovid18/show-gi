...
CLAUDE.md 「굵게는 머리말 자리에만 쓴다」의 검사기다. 남기는 자리와 길이 상한,
개수가 아니라 자리를 보는 이유는 그 규약에 있다.

인자 없이 부르면 세기만 하고, --fix 면 고친다. 코드 펜스와 인라인 코드는 건드리지 않는다.
"""
import collections
import os
import re
...
def 가린_줄(line):
    ...
    구간으로 자르지 않고 덮는다. 굵게가 코드를 품는 자리가 있어서,
