...
CLAUDE.md 「결론을 먼저 평서문으로 쓴다」의 검사기다. 문서 산문은 보지 않는다 —
...
# 사람이 읽는 산문이 든 확장자. .yaml·.mjs·.sh 를 빼 두었더니 그 셋에서만
# 금지된 맺음 문구가 살아 있었다
EXT = ('.go', '.ts', '.tsx', '.tf', '.sql', '.yaml', '.yml', '.mjs', '.js', '.sh', '.py')
금지 = {
    '「~것이 요점이다」': r'것이\s*요점이다',
...
def 주석_줄(path):
    """주석 줄만 돌려준다. 문자열 리터럴 속 한국어는 UI 문구라 여기서 보지 않는다."""
    if path.endswith(('.ts', '.tsx', '.mjs', '.js')):
        여는말 = ('//', '*', '/*')
    elif path.endswith(('.yaml', '.yml')):
        여는말 = ('#', 'description:', 'summary:')
    else:
        여는말 = ('//', '--', '#')
...
def main():
    ...
    걸림 = 0
    for f in files:
        if any(s in f for s in SKIP) or not f.endswith(EXT):
            continue
        for ln, s in 주석_줄(f):
