// 시험의 입구. 시나리오를 MODE 로 고른다.
//
// 셋을 한 파일로 둔다. 「엔진 + 대인전」이 별도 시나리오가 아니라 조합이라서다 —
// 둘을 같이 걸어야 engine_pool_wait_seconds 의 borrower 라벨이 갈리는 것을 볼 수
// 있고, 그것이 이 시험의 주된 물음이다(journal §101).
//
//   MODE=engine   엔진 대국만. 쿠키는 있으면 쓰고 없으면 익명이다
//   MODE=match    대인전만. 로그인·대기열이 필요하다
//   MODE=both     둘을 동시에. 서로 굶히는지를 재는 시험이다
...
// 시나리오는 상수 VU 다. ramping 을 쓰지 않는 것은 이 앱의 부하 단위가 「동시 판수」라서다 —
// 판 하나가 연결 하나를 몇 분 잡으므로, 초당 도착률로 말하면 동시 판수가 보이지 않는다.
function scenarios() {
...
//
// MODE=both 에서만 붙인다. 그 시험의 물음이 「둘이 서로 굶히나」인데 합친 move_cycle
// 하나로는 답할 수 없어서다(journal §113). 한 갈래만 도는 시험은 전체가 곧 그 갈래다.
function scenarioSplit() {
  ...
  for (const name of ['engine', 'match']) {
    // 사람이 기다린 시간. 이 시험에서 갈라 볼 값이 이것 하나다 — 나머지 셋은 그 값을
...
export const options = {
  ...
  thresholds: {
    ...
    // 막아 줄 것이 없다 — 5xx 가 늘면 시험이 스스로 멈춘다.
    http_req_failed: [{ threshold: 'rate<0.05', abortOnFail: true, delayAbortEval: '30s' }],
    ...scenarioSplit(),
  },
};

// setup 은 사람 목록이 시험을 끌고 갈 만한지 먼저 본다. VU 안에서 터지면 판이 몇 개
...
export function setup() {
  ...
  if (SESSION_SECRET === '' || LT_UIDS.length === 0) {
    ...
    // 걸어 보는 것이 이 도구의 첫 시험이었다.
    console.warn('익명으로 돈다 — games.user_id 가 NULL 이라 cleanup.sql 이 이 판들에 닿지 않는다');
    return {};
  }

  // __VU 는 시험 전체에서 유일하다. 그래서 목록이 두 시나리오의 VU 합보다 짧으면 두
  // VU 가 같은 사람을 잡고, 대기열이 사람마다 한 행이라(match_queue 의 PK) 서로를 만나지 못한다.
