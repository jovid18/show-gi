...
-- 시험이 그것을 원한다. 벌리면 그 폭으로 중앙을 기준삼아 늘어선다.
--
-- rating_games 를 1 이상으로 두어야 그 값이 쓰인다 — 0은 「레이팅이 없다」이고
-- 그때는 낙폭 추정치에서 시드를 만든다(rating.SeedFromLoss).
--
-- 밴드가 대기로 넓어지는 것을 보려면 벌려야 하고, 그 시험은 사람이 둘이다. 두 조건이
-- 겹쳐서 그렇다. 대기열은 아무 둘이나 밴드 안에 있으면 붙이므로(internal/queue 의
...
-- 읽는 질의가 둘뿐이고 둘 다 user_id 하나를 본다) 그건 경고 없이 바뀔 수 있는 성질이고,
