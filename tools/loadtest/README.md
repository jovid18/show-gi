...
| **워커를 늘린 뒤 재측정**  | 위와 같은 계단, **`ANALYSIS_WORKERS=2`**              | 풀 둘을 다 쓰면 천장이 어디인가. §106 은 워커가 하나여서 `EnginePoolInUse` 가 내내 1이었다. **손잡이는 들어왔고 기본값은 풀보다 하나 적다** — 지금 박스에서는 1이라 회차에서 직접 올려 준다               |
