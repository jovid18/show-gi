...
| **詰み 캐시 — 대인전**             | `MODE=match`, `THINK_MS=3000`                             | 대인전에서도 캐시가 히트하나. 그쪽 소비자는 사후 분석 하나이고 결과를 버리므로 히트율이 제품 개선이 아니라 표 채우기인데, **게이트 없이 手마다 부르는 층**이라 [journal §110](../../docs/journal/101-120.md)이 잰 분당 200~300회가 이제 캐시를 지난다. 엔진 계단은 [§112](../../docs/journal/101-120.md)에서 돌았다 |
