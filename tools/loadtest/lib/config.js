// 시험 손잡이 전부. k6 는 환경변수로만 값을 받는다(__ENV).
//
...
//
// 넣으면 VU 수를 사람 수로 그대로 읽을 수 있다. 넣지 않았을 때의 「VU 하나는 사람 하나보다
// 크다」 환산이 그때 없어진다.
//
// move_cycle 을 이 값만큼 빼서 읽는다. 그 지표는 우리가 보낸 뒤 다시 우리 차례가 될
// 때까지이고, 그 사이에 상대의 생각 시간이 들어 있다 — 대인전에서는 상대가 우리 자신이라
// THINK_MS 가 그대로 얹힌다. 3초로 잰 시험의 중앙값 3.03초에서 서버 몫은 30ms 다.
export const THINK_MS = Number(__ENV.THINK_MS || 0);

// 씨앗. 시험을 다시 돌릴 수 있어야 「느렸다」를 같은 수순으로 재현할 수 있다.
export const SEED = Number(__ENV.SEED || 1);

// 시험이 쓸 사용자 번호. seed.sql 이 마지막에 찍어 주는 목록을 그대로 넘긴다.
//
...
// 시험의 동시 판수가 조용히 줄어든다 — VU 하나가 아무것도 하지 않으면서 자리를 잡고 있다.
//
// 멈춤을 잡는 것은 STALL 쪽이다. GAME 쪽은 마지막 보루라서 건강한 시험에서는 아예
// 터지지 않아야 한다 — 5분이었을 때 8판 동시의 판이 그것에 닿아 cut=capped 가 멈춘 판 집계를
