// 수를 고른다. 규칙은 하나도 모른다.
//
// 서버가 스냅샷마다 legalMoves 를 통째로 주므로 그중 하나를 뽑으면 된다 — 그래서 부하
// 도구에 룰 엔진이 필요 없다. 잘 두려는 것이 아니라 판을 진행시키는 것이 목적이다.

// pickMove 는 합법수 하나를 고른다. 없으면 null.
//
// 成 를 선호한다. 그냥 무작위로 두면 판이 잘 끝나지 않아서 시험이 手数 상한에만 걸리고,
...
// makeRNG 는 씨앗이 있는 난수다. Math.random 을 쓰면 시험을 다시 돌릴 수 없다 —
