#!/bin/sh
# 시험을 거는 자리. 손잡이는 KEY=VALUE 로 받아 그대로 k6 에 넘긴다.
#
# 명령줄에서 직접 부르면 셋이 어긋난다.
#
#   - k6 는 손잡이를 환경변수로만 받으므로(lib/config.js) 명령이 env 접두사로 시작한다.
#     그러면 Bash(k6 run:*) 허용 규칙이 접두사 매칭에 걸리지 않는다
#   - SESSION_SECRET=$(aws ssm ...) 의 명령 치환도 같은 규칙을 지나지 못한다
#   - main.js 를 상대 경로로 주면 레포 루트가 아닌 곳에서 부를 때 찾아지지 않는다
#
# 기본값은 여기 두지 않는다. k6 스크립트에 이미 있다 — 손잡이 대부분이 lib/config.js
...
if [ "$remote" = 1 ]; then
	# 익명 판은 cleanup.sql 이 걷지 않는다(users 한 줄에서 CASCADE 로 지우는 구조라
	# games.user_id 가 NULL 이면 닿지 않는다). 남으면 사람이 손으로 찾아야 한다.
	#
	# 비었는가만 보지 않는다. 숫자가 아닌 값은 lib/config.js 가 걸러 버려서 빈 목록과
	# 같아지고, 그러면 시험이 익명으로 돈다.
	if ! printf '%s' "${LT_UIDS-}" | tr ',' '\n' | grep -qE '^[[:space:]]*[1-9][0-9]*[[:space:]]*$'; then
		echo "원격($BASE)에는 LT_UIDS 에 사용자 번호가 있어야 한다 — 익명 판은 cleanup.sql 에 걸리지 않는다" >&2
		exit 2
	fi
fi
...
if [ "$remote" = 1 ] && [ "${SESSION_SECRET-}" = "" ]; then
	...
	# 빈 값이나 None 도 exit 0 이라 set -e 가 잡지 못한다. 그대로 나가면 판이 익명으로
	# 남아서 위 LT_UIDS 가드가 지키려던 것이 뚫린다.
	if [ -z "$SESSION_SECRET" ] || [ "$SESSION_SECRET" = "None" ]; then
		echo "SSM 에서 SESSION_SECRET 을 읽지 못했다" >&2
		exit 2
	fi
...
# 읽는다 — 인자만 적으면 `THINK_MS=5000 run.sh MODE=match` 시험이 나중에 「기본으로
# 걸었다」로 읽힌다.
#
# 주지 않은 것은 적지 않는다. 기본값은 k6 스크립트에 있고 그 값이 바뀐 적이 있어서
# (MAX_PLIES 60 → 100) 여기서 「기본」이라고 적으면 어느 값으로 잰 것인지 알 수 없다.
#
# SESSION_SECRET 은 목록에 없다. 가리는 것보다 읽지 않는 쪽이 확실하다.
shown=""
...
echo "시험 $(date -u '+%Y-%m-%dT%H:%M:%SZ') —$shown"
