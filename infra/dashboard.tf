...
#
# 코드로 두는 것은 시험끼리 견주기 위해서다. 콘솔에서 위젯을 그때그때 고르면 축도 기간도
# 매번 달라져서 「지난번보다 나아졌나」를 말할 수 없다.
#
# 지표 이름은 EMF 가 내보내는 것과 맞춰야 한다(internal/metrics 의 collect). 어긋나면 위젯이
# 「데이터 없음」으로 비고, 알람과 달리 누구도 알려주지 않는다.
#
# 대시보드 3개까지 무료인데도 하나로 두는 것은, 시험 중에 볼 화면이 둘이면 사람이 둘 다
# 보지 않기 때문이다.

locals {
  # dimensions 둘을 다 적어야 한다. 하나만 적으면 그런 계열이 없다(alarms.tf 의 같은 함정).
  # 두 티어가 같은 계열에 올리므로 위젯의 값은 둘을 합친 것이고, 통계가 그 합치는 법이다.
  # 카운터는 Sum, 게이지는 Maximum 이다(journal §120).
    ...
    widgets = [
      ...
      },
      {
        ...
        properties = {
          ...
          metrics = [
            ...
            # 詰み 풀을 같은 판에 그린다. 단위가 같고 묻는 것도 「어느 풀이 굶는가」로 같다.
          ...
          # 알람과 같은 선을 긋는다(alarms.tf 의 engine_pool_wait). 평상시가 어디에
          # 있는지를 눈에 보이게 한다.
      ...
      },
      {
        ...
        properties = {
          ...
          # 축을 가른다. 밀린 手는 수백까지 가고 나머지 둘은 0이나 1이다.
          yAxis = { right = { min = 0 } }
        }
      },
      {
      ...
      },
      {
        ...
        properties = {
          ...
          metrics = [
            ...
            # 대를 늘렸을 때 크기를 말하는 선이다. 합계는 캐시가 덮어서 거의 움직이지
            # 않는다. 실측이 합계 +11% 에 이 값 +46% 였다(journal §121).
      ...
      },
      {
        ...
        properties = {
          ...
          metrics = [
            concat(["show-gi", "AnalysisBacklogPlies"], local.dash_dims, [{ stat = "Maximum", label = "밀린 手" }]),
            # 대수는 ASG 그룹 지표로 본다. 서비스의 desired_count 는 Container Insights 를
            # 켜야 지표로 나오고, 여기서 묻는 것은 「EC2 가 붙었나」다(ec2.tf).
          ...
          # 임계선을 그린다. 이 선을 5분 넘긴 것이 스케일 아웃의 조건이다
          # (alarms.tf 의 analysis_backlog).
          annotations = {
