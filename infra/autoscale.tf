...
# SERVER_ROLE=both 로 겸하므로(ecs.tf) 0 이어도 큐를 아무도 집지 않는 일은 없다(journal §125).
...
# 그것을 다시 짓는 것은 두 번째 진실이고, 잴 때마다 어느 쪽이 울렸는지를 따져야 한다.
#
...
resource "aws_appautoscaling_policy" "analysis_out" {
  ...
  step_scaling_policy_configuration {
    ...
    # 없고, 시험이 재는 것이 「한 대 늘리면 얼마나 느나」라서 계단이 둘이면 기울기를
    # 읽을 수 없다(ecs.tf 의 maximum_scaling_step_size 와 같은 판단).
    step_adjustment {
...
# 되돌리는 쪽. 없으면 시험 뒤에 두 대가 그대로 떠서 하루 $2.15 가 계속 나간다 —
...
#
# 밀린 手는 포화 신호라 「비었다」와 「대수가 충분하다」가 같은 값으로 나온다 — 2대가 8판을
...
# 부하가 도는 동안은 手가 계속 도착하므로 30분 연속 0 이 나오지 않는다.
#
# 사람에게는 알리지 않는다. 조용해진 것을 사고로 보지 않아 SNS 를 붙이지 않았고,
# 언제 움직였는지는 describe-scaling-activities 에 남는다(deploy/README.md).
