...
#
# 커스텀 지표는 api 컨테이너가 stdout 으로 내보내는 EMF 에서 나온다(internal/metrics).
# CloudWatch 가 로그에서 뽑아 show-gi 이름 공간에 넣으므로, 여기서 만들 것은 없고
# 이름을 맞추기만 한다 — 이름이 어긋나면 알람이 「데이터 없음」으로 그대로 산다.
#
# 티어 둘이 같은 계열에 올린다. dimensions 에 티어를 올리지 않은 이유와 그 대가는 journal §120 에
# 있고, 여기서 지켜야 하는 것은 통계 하나다 — 카운터는 Sum, 게이지는 Maximum 이다.
#
# ALB 쪽 지표는 EMF 와 무관하게 AWS 가 항상 내보낸다. 그래서 「사이트가 떴나」는 그쪽을
...
# 만들고, 그 상태에서는 알람이 울려도 메일이 오지 않는다.
...
# 정상 타깃이 0인 상태. 「비정상 수」 대신 「정상 수」를 본다. 타깃이 아예 등록되지
# 않은 경우까지 같은 알람이 덮기 때문이다 — 비정상 수는 그때 0으로 나온다.
#
# 5분인 것은 정상 배포가 그보다 짧기 때문이다. 대상 그룹 뒤가 한 대라 배포는 하나뿐인
# 태스크를 먼저 내리고(ecs.tf 의 deployment_minimum_healthy_percent = 0) 새 것을 띄우며,
# 거기에 기동 유예 120초가 붙는다. 스팟 회수도 2~3분이다(README 맨 위). 1분 두 번으로
...
resource "aws_cloudwatch_metric_alarm" "no_healthy_target" {
  ...
  # 타깃이 하나도 등록되지 않으면 이 지표가 아예 나오지 않는다. 그것도 「정상이 0」이므로
...
# 요청의 상태가 이미 101이라 5xx 로 잡히지 않기 때문이다(internal/server/observe.go).
...
# 사람이 기다린 것인지 알 수 없다.
#
# 1분 창으로 최근 5분 중 3분을 요구한다. 5분 창 두 번이었을 때는 동시 6판에서 1분 p95 가
# 5초인데도 울리지 않았다(journal §104). 단발 겹침(퀴즈 생성이 판 끝에서 수십 초를 잡는 것)에는
# 여전히 울리지 않는다 — 3분이 필요하다.
#
...
resource "aws_cloudwatch_metric_alarm" "engine_pool_wait" {
  ...
  # 누구도 두지 않는 시간에는 표본이 없어 지표가 나오지 않는다. 조용한 것을 위반으로 읽지 않는다.
  treat_missing_data = "notBreaching"
}

# 사후 분석이 밀린다. 위의 engine_pool_wait 은 이 상태를 보지 못한다 — 대인전 포화에서
# 사람의 풀 대기는 표본이 아예 0이었다(journal §107 · §108). 병목은
# 「엔진이 처리할 수 있는 탐색 수」라서, 대국은 우선 큐라 밀리지 않고 분석만 큐에 선다.
#
...
#
# 임계 100 은 그 사이에 둔다. 다만 6판의 최고가 시험마다 11~84 로 흔들려서(journal §119)
...
#
# AWS 가 EventBridge 로 두 가지를 미리 준다. 지표 대신 이벤트로 오므로 알람으로 받을 수 없고,
...
#
# 2분으로는 용량을 구해 오지 못한다. 값은 「왜 내려갔나」를 나중에 추측하지 않는 것과,
# 부하 시험 도중이면 그 시험이 곧 무효가 되는 것을 그 자리에서 아는 것이다 — 계단
# 하나가 8분인데 박스가 12분을 산 날이 있었다(journal §107).
resource "aws_sns_topic" "spot" {
  name = "show-gi-spot"
}

# 알람 토픽에 얹지 않는다. 까닭이 둘이고 둘 다 실무적이다.
#
...
# 서비스 주체는 들어가 있지 않아서, 이 문장이 없으면 규칙이 경고 없이 아무것도 하지 않는다.
...
# 계정 전체의 스팟 이벤트를 받는다. 인스턴스 id 로 좁히지 않는다 — 그 id 가 회수마다
# 바뀌고, 이 계정에 스팟은 이 서비스뿐이라 좁힐 것이 없다.
