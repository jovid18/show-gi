# state를 로컬에 두지 않는다. 까닭이 둘이다.
#   1. 날리면 복구할 수 없다 — Terraform이 만든 자원을 손으로 추적해야 한다
...
terraform {
  backend "s3" {
    ...
    # 그래서 프로파일을 여기 한 번 더 적는다. 적지 않으면 기본 프로파일로 붙으려 하고,
    # 이 기계의 기본 프로파일은 만료된 회사 자격증명이라 init에서 403이 난다.
    profile = "show-gi"
  }
}

# 잠금에 DynamoDB를 쓰는 것은 순전히 버전 때문이다. S3 네이티브 잠금
