...
variable "instance_type" {
  description = <<-EOT
    ...
    **지금은 절약 모드다**(journal §125). 부하 시험을 한동안 돌리지 않으므로 t4g.small 로 내렸다 —
    ...
    **t4g.micro 로 내렸다가 되돌렸다.** 1 GiB 에서 엔진이 `engine exited before readyok` 로
    ...
    있는데 대국만 불가능**했다. 재시도로는 떴으므로 OOM 대신 기동 핸드셰이크가
    ...
    **이 값으로는 용량표가 성립하지 않는다.** T 계열은 크레딧이 마르면 baseline 으로 조여서
    ...
    그 분수는 시험 시작 시점의 크레딧 잔량이 정한다. **시험을 다시 돌리기 전에 c6g.large 로
...
variable "instance_type_fallbacks" {
  description = <<-EOT
    ...
    **전부 arm64 여야 하고 1 GiB 이상이어야 한다.** t3·t3a 는 x86_64 라 넣으면 안 된다 —
    그 타입을 받은 날 이미지가 아예 뜨지 않는다. 메모리도 같은 종류의 함정이다: task_memory 가
    여기 있는 어느 타입에서도 들어가야 하고, 들어가지 못하는 것이 섞이면 그날 태스크가 뜨지 않는다.
...
variable "on_demand_base_capacity" {
  description = <<-EOT
    ...
    **부하 시험 동안만 1이다.** 스팟이 80분에 네 번 가져갔고 회수 하나가 약 9분 장애다
    (journal §109) — 계단이 8~15분이라 시험이 회수를 밟을 확률이 높고, 그러면 잰 것이
    병목이 용량에서 회수 복구 시간으로 옮겨간다. c6g.large 온디맨드가 하루 $2.15 라 이틀 시험의
    차액이 $3 쯤이고, 그것이 시험 하나를 다시 도는 값보다 싸다.

    **시험이 끝나면 0으로 되돌린다.** 상시로 두면 값이 네 배다. **다만 그 apply 가 도는
    대를 갈아치운다**(journal §124) — 아래 percentage 가 이 값에서 유도되므로 ASG 가 구매
    정책을 맞추려고 온디맨드를 스팟으로 바꿔 끼우고, 실측으로 상호작용 태스크가 76초
    내려갔다. **사람이 보지 않는 시간에 건다.**

    **오토스케일이 붙으면서 뜻이 하나 늘었다**(journal §124). 부하 때 자동으로 붙는 두 번째
    대도 이 값이 0이면 스팟이라, 하필 그때 회수되면 9분치 분석 처리량이 날아간다. 그룹 공통
    변수라 「분석만 온디맨드」로는 나눌 수 없다 — 나누려면 변수를 쪼개야 하고, 그럴 이유가
...
variable "analysis_max_instances" {
  description = <<-EOT
    ...
    **desired 는 여기서 정하지 않는다.** 한때 이 값이 서비스의 desired_count 이기도 했는데
    ...
    **2 다.** 1대 6판 지속 · 2대 12판 지속까지 실측으로 있고(journal §121 · §122),
    3대는 보지 않았다(journal §123) — 그 값은 이 상한을 올리는 날에야 쓰인다.

    **상한이고 요금은 따로다.** 큐가 30분 비면 정책이 대수를 1로 되돌리므로(autoscale.tf)
    이 값을 2로 두는 것만으로는 값이 늘지 않는다. 부하가 도는 동안만 대당 하루 $2.15 다.
...
variable "task_cpu" {
  description = <<-EOT
    ...
    배치되지 않는다** — Fargate처럼 임의의 조합을 고르지 않고 실제 하드웨어에서 뺀다.
...
variable "task_memory" {
  description = <<-EOT
    ...
    **정상 상태 RSS 로 이 값을 잡으면 안 된다.** 회차 중 클러스터 지표가 태스크당 약
    ...
    (ecs.tf의 그 자리). **이 값만 올리면 늘어나지 않는다.** 엔진 몫은 `ENGINE_POOL_SIZE` ·
...
variable "admin_cidr" {
  description = <<-EOT
    ...
    **낡으면 거절 대신 타임아웃이 온다.** 그래서 「DB 가 안 떴나」로 오해하기 쉽다 —
    되살리는 한 번 사이에 두 번 낡았다(journal §133).
  EOT
  type        = string
  default     = null
