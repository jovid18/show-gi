...
#
# 직접 쓴 배포 글루가 전부 사라져서 바꿨다: 배포 스크립트,
...
# ASG가 그래서 돌아왔다 — 배포 스크립트는 돌아오지 않았다. 그건 ECS가 맡던 일이다.
#
...
locals {
  ...
  # 직접 넣는다 — 값이 디스크에 남지 않고 로그에도 찍히지 않는다. deploy/env.sh 를 대체한 자리다.
  ...
  # 있던 fairy-stockfish 가 이겨서, 배포는 성공했는데 대국만 되지 않는 상태가 됐다.
  #
  ...
  # 앞 시험과 대조가 깨진다.
  #
  # ENVIRONMENT 가 지표의 손잡이다. 비어 있으면 서버가 EMF 를 내보내지 않으므로(cmd/api 의
  # startEmitter) 이 한 줄이 CloudWatch 커스텀 지표를 켜고 끈다. 값은 EMF 문서의
  # Environment 가 dimensions 가 되므로, 이 값을 바꾸면 알람 쪽도 같이 바꾼다
  # (infra/alarms.tf). 티어를 dimensions 로 올리지 않는 이유는 journal §120 에 있다.
  api_env = [
...
resource "aws_cloudwatch_log_group" "app" {
  name = "/ecs/show-gi"
  # 기본이 무기한이라 눈에 띄지 않게 쌓인다. 14일인 것은 요청 로그와 EMF 가 같은 그룹으로
...
# 애플리케이션은 이 역할을 쓰지 않는다 — 그래서 앱이 뚫려도 이 권한은 노출되지 않는다.
...
resource "aws_ecs_task_definition" "app" {
  ...
  #
  # bridge 는 쓸 수 없다. Caddy가 reverse_proxy localhost:8080 으로 api 를 부르는데
  ...
  container_definitions = jsonencode([
    ...
    merge(local.api_container, {
      ...
      # 밀린 手를 누구도 집지 않는다 — 부하가 오면 알람이 전용 대를 부르고, 그때는 둘이
      # 같이 집는다.
      #
      # 되돌릴 때 interactive 로 바꾼다. 그러면 이 박스의 엔진이 분석에 쓰이지 않아서
      # 티어를 가른 값(사람의 착수가 분석에 밀리지 않는다)이 돌아온다.
      environment = concat(local.api_env, [{ name = "SERVER_ROLE", value = "both" }])

      # 로그인과 세션이 이 티어에만 있다. 분석 티어는 사람을 받지 않으므로 DATABASE_URL 뿐이다.
      #
      # OPENAI_API_KEY 도 이 티어뿐이다. 기보를 가져오는 요청이 사람에게서 오고
      # (server/kifu_import.go) 분석 워커는 그 계층을 지나지 않는다 — 옮겨 적는 일은 판이
      # 줄에 서기 전에 이미 끝나 있다.
      #
...
# 분석 티어. 사람을 받지 않으므로 web 컨테이너가 없다.
#
# 컨테이너가 하나인 것이 이 티어의 정의다. Caddy 는 사람의 요청을 api 로 넘기는 자리인데
# 여기는 대상 그룹 뒤에 없어서 넘길 요청이 없고, 띄우면 80 을 잡아 누구도 보지 않는 서버가
# 인스턴스마다 하나씩 는다.
#
# /healthz 를 밖에서 물어볼 수 없다. 대신 이 티어가 죽으면 AnalysisBacklogPlies 가
# 부푸는 것으로 보인다(alarms.tf) — 그래서 게이지를 티어마다 올린다(journal §119).
resource "aws_ecs_task_definition" "analysis" {
  ...
  # host 인 것도 상호작용 쪽과 같은 판단이다(위). 포트가 겹치므로 이 모드가 곧
  ...
  container_definitions = jsonencode([
    merge(local.api_container, {
      ...
      # 주지 않는 만큼 이 박스가 뚫렸을 때 나가는 것이 적다.
      secrets = [
...
# 꺼 두면 그 ASG 의 대수를 terraform 이 계속 정한다.
...
# 분석 쪽은 켠다. 배치하지 못한 태스크를 보고 ASG 의 desired 를 올리는 것이 이 블록이고,
...
resource "aws_ecs_capacity_provider" "analysis" {
  ...
  auto_scaling_group_provider {
    ...
    managed_scaling {
      ...
      # 사람이 기다리지 않으므로 그 값을 낼 이유가 없다 — 밀린 手는 늦게 재도 사람이 보지 않는다.
      target_capacity = 100

      # 한 번에 한 대씩 움직인다. 대당 2 vCPU 인데 시험이 재는 것이 「한 대 늘리면
      # 처리량이 얼마나 느나」라서, 계단이 두 대씩이면 그 기울기를 읽을 수 없다.
      minimum_scaling_step_size = 1
      maximum_scaling_step_size = 1

      # 새 대가 뜨고 ECS 에 등록되어 태스크를 받기까지가 실측으로 4분쯤이다(journal §109).
      # 그 전에 다음 계단을 밟으면 아직 일을 시작하지 않은 대를 「모자라다」로 읽는다.
    ...
    # AWS 기본값이라 코드에 보이지 않던 값이다. 켜져 있으면 ECS 가 ASG 에
    ...
    #
    # 끄지 않은 것은 대안의 값을 재 보지 않았기 때문이다. 끄면 즉시 종료되는 대신 드레이닝이
    # 없어지고, 훅의 타임아웃만 줄이면 ECS 가 뒤늦게 완료를 보낼 때 무슨 일이 나는지 모른다.
    managed_draining = "ENABLED"
  }
}

# 클러스터에 등록해야 서비스가 이름으로 고를 수 있다. 기본 전략은 두지 않는다 —
# 두 서비스가 각자 자기 것을 명시하므로, 기본값이 있으면 잘못 적었을 때 경고 없이 붙는다.
#
# 여기서 이름을 빼는 것으로는 공급자가 지워지지 않는다. 쓰는 서비스가 있는 동안 삭제가
...
# 이름을 바꾸지 않은 것은 RDS와 ALB의 규칙이 이 이름을 가리키고 있어서다.
...
#
# api의 8080은 열지 않는다. Caddy가 localhost:8080 으로 부르므로 밖에서 닿을 필요가 없고,
...
resource "aws_ecs_service" "app" {
  ...
  # 태스크 하나에 맞춰진 한 대뿐이라 두 개가 동시에 올라가지 못한다 — 그대로 두면 새 태스크가
  # 영원히 배치되지 않고 배포가 서킷 브레이커에 걸려 되돌아간다.
  #
  # 값은 배포마다 1~2분 내려간다. 쓰는 사람이 한 명이라 받아들이는 쪽이고,
  ...
  # ASG가 먼저다. 클러스터에 등록된 인스턴스가 없으면 서비스가 태스크를 띄우지 못하고,
...
# 짝이 맞지 않고(journal §98) 로그에 아무것도 남지 않는다. 배포 워크플로가 /healthz 의 role 을
...
#
# 그 값은 terraform 이 정하지 않는다. 밀린 手가 정하고(autoscale.tf), 여기 적힌 1은
# 서비스를 처음 만들 때만 쓰인다.
resource "aws_ecs_service" "analysis" {
  ...
  # 옛 태스크를 먼저 내리고 새 것을 띄운다. 상호작용 쪽과 같은 값이고 판단도 같다 —
  # 인스턴스가 태스크 하나에 맞춰져 있어 두 개가 동시에 올라가지 못한다.
  #
  ...
  # 유예라서 대상 그룹이 없는 서비스에는 줄 수 없다 — 적으면 ECS 가 거절한다.
  ...
  lifecycle {
    ...
    # 시험이 손잡이 하나만 고쳐 apply 하는 자리라 그것이 경고 없이 시험을 무효로 만든다.
