...
生成モデルが動く場所は二つだけで、どちらも**書き写すだけ**です。**取り込んだ棋譜の書式を写すところ**（`internal/kifunorm`）と、**盤が写った画像を読むところ**（`internal/boardread`）です。

どちらにも座標を作らせません。プロンプトは筋も段も一度も言わず、画像のほうが言われるのは「上の行から、左から、描かれているとおりに」だけです。先手か後手かも決めさせません — 画像から分かるのは「上側の駒か下側の駒か」だけで、下側を先手として扱うのはコードが決めます。

写した表記は一手ずつルールエンジンの検算を通らなければ指し手になりませんから、棋譜のほうは作り話をすればそこで必ず落ちて取り込みが断られます。画像のほうは指し手の並びがないので、局面そのものを検算します（二歩・玉の数・行き所のない駒・王手放置）。**それでも捕まらない読み違いはあります** — 銀を成銀と読んだ盤はそれ自体は合法な局面です。だから読み取った局面は必ずあなたに見せて、直してもらってから解析します。
...
| **棋譜の取り込み** | ほかで指した自分の棋譜を貼ると、ここで指した一局と同じ解析・クイズ・棋力の目安まで通る。KIF・KI2・CSA・USI を読み、読めない書式だけ AI が写す |
