...
LLM の出力は検証し、不正なら決定的な日本語テンプレートに戻します。ルータが落ちても、事実に基づく説明は表示されます。
...
| **説明と表示**     | LLM の説明は検証・フォールバック付き。盤面は three.js で読みやすく描く   |
...
| DB       | PostgreSQL 17 + pgvector。対局、解析、説明キャッシュを保存                  |
| LLM      | OrcaRouter（OpenAI 互換 HTTP）、`temperature=0`                             |
| インフラ | AWS ECS Fargate（ARM64）+ ALB + RDS + Route53、Terraform、GitHub OIDC       |
