...
生成モデルが動く場所は一つだけです — **取り込んだ棋譜の書式を写すところ**（`internal/kifunorm`）。そこがするのは文字を写す作業で、どの駒がどこへ動いたかは決めません。写した表記は一手ずつルールエンジンの検算を通らなければ指し手になりませんから、作り話をすればそこで必ず落ちて取り込みが断られます。プロンプトは局面を知りません。
