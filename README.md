...
| 最善手               | ヒントボタンで見せる       | **見せない。「なぜ悪いか」までで止める**       |
...
- 制止型（悪手のあと）— 「なぜ悪いか」まで
- 提案型（着手の前）— 「ここに何かある」まで。**どの手かは言わない**
- 詰みゲージ — 手順も手数も出さない。盤の縁が燃えるだけ

唯一手を指すのは**行き詰まりヒント**で、同じ局面で 3 回戻されたらその駒に、5 回で手に印がつきます。5 回失敗しないと開かない扉なので、頼れるヒントにはなりません。
...
```mermaid
flowchart TD
    A[プレイヤーの手番] --> B{好手筋のある局面か}
    B -- ある --> C["提案型: 「ここに何かある」だけ通知<br/>（着手の前・どの手かは言わない）"]
    B -- ない --> D[着手]
    C --> D
...
| **介入判定と指し直し**             | レベル別の勝率落差閾値。悪手カテゴリ **8 種 + `other`**、判定は順序つき（順序そのものがルール）                                       |
...
- **手筋の名前の精度はまだ本番水準ではありません。** 実戦棋譜 1 局で 8 件中 3 件が誤判定でした（修正済みですが、標本が 1 局です）。「良い手に間違った名前を付ける」のが学習アプリで最も高くつく失敗なので、**提案型ヒントの名前はまだ広げていません**
...
- **提案型ヒントが人の前で出るのを、まだ見ていません。** ゲートが間に合うようになったことは実測しましたが（一巡 8.65 秒 → 2.15 秒）、実際に出るかは着手間隔次第で、その時刻が棋譜に残っていません
