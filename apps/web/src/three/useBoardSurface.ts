...
import type { BoardSurface, Layout } from './surface';
import type { Board } from '@/shogi/sfen';
import { exposure, influenceOf } from '@/shogi/influence';
...
function measure(boardEl: HTMLElement, surface: BoardSurface, layoutRef: React.RefObject<Layout | null>): void {
  ...
  // 캔버스는 판의 테두리 **안쪽**을 채운다. 테두리 폭을 여기서 빼는 대신 판의
...
export function useBoardSurface({ boardRef, board, active, from }: Options): boolean {
  ...
  // 순간 뒤의 것이 같이 죽는다. 붙는 자리는 **맨 뒤**다 — 첫 자식은 打 화살표가 칸 크기를
