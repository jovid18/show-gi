...
/** 대기열에 서 있는 동안 다시 물어보는 주기. 서버가 걷어가는 시한보다 넉넉히 짧아야 한다. */
const POLL_MS = 2000;

export interface QueueState {
  /** 대기열에 서 있는가. 눌러야 서고, 짝이 잡히거나 그만두면 내려간다. */
  searching: boolean;
  /** 지금 대기열에 서 있는 사람 수(자기 포함). 아직 모르면 0. */
  waiting: number;
  /** 대기열에 선 뒤로 흐른 시간(ms). 서버가 준 값이다. */
  waitedMs: number;
 ...
 *
 * 폴링이다. 기다리는 동안 서버가 할 말이 없다. 짝은 상대가 대기열에 서는 순간 생기고, 그것을
 * 아는 것은 그 사람의 요청이다.
 *
 * 같은 호출 하나가 대기열에 서기·heartbeat·짝짓기를 다 한다(`pollQueue`). 이 훅이 하는 일은
 * 그것을 주기적으로 부르고, 짝이 잡히면 `onMatched` 를 부르는 것뿐이다.
 *
 * 멈추는 것을 서버에 알린다(`leaveQueue`).
 */
export function useQueue(onMatched: (roomId: string) => void): QueueState {
  ...
  /**
   * 지금 화면이 짝을 받을 자리인가. 콜백을 ref 에 담아 두는 것은 폴링을 그것 때문에 다시
   * 걸지 않기 위해서다. 부모가 매번 새 함수를 넘기면 대기열에서 나갔다 다시 서게 된다.
   */
      ...
      void pollQueue(ac.signal)
        .then((status: QueueStatus) => {
          ...
          if (status.status === 'matched') {
            // 대기열에서 내려온다. 서버는 이 자리를 이미 지웠다(TakeQueueSeat). 여기서
            // `leaveQueue` 를 부르면 방금 잡힌 상대의 자리까지 흔든다.
            setSearching(false);
        ...
        .catch((e: Error) => {
          ...
          // 재시도를 멈춘다. 로그인이 필요한 실패가 여기로 오는데, 그때 계속 부르면 2초마다
          // 401이 쌓인다. 사람이 다시 누르는 것이 재시도다.
          setSearching(false);
          setError(e.message);
        });
    };
  ...
  // 탭을 닫는 것도 대기열에서 나가는 것이다. 창을 닫으면 React 가 정리를 부르지 않는다.
  useEffect(() => {
    if (!searching) return;
   ...
   * 그 방은 누구도 들어가지 않고 상대는 60초를 기다린다.
   *
