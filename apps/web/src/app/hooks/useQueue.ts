...
/** 줄에 서 있는 동안 다시 물어보는 주기. 서버가 걷어가는 시한보다 넉넉히 짧아야 한다. */
const POLL_MS = 2000;

export interface QueueState {
  /** 줄에 서 있는가. 눌러야 서고, 짝이 잡히거나 그만두면 내려간다. */
  searching: boolean;
  /** 지금 줄에 서 있는 사람 수(자기 포함). 아직 모르면 0. */
  waiting: number;
  /** 줄에 선 뒤로 흐른 시간(ms). 서버가 준 값이다. */
  waitedMs: number;
 ...
 * 짝은 상대가 줄에 서는 순간 생기고, 그것을 아는 것은 그 사람의 요청이다.
 *
 * 같은 호출 하나가 줄에 서기·heartbeat·짝짓기를 다 한다(`pollQueue`). 그래서 이 훅이
 * 하는 일은 그것을 주기적으로 부르고, 짝이 잡히면 `onMatched` 를 부르는 것뿐이다.
 *
 * 멈추는 것을 서버에 알린다. 안 알려도 만료가 걷어가지만, 그동안 상대에게는 내가 줄에
...
export function useQueue(onMatched: (roomId: string) => void): QueueState {
   ...
   * 다시 걸지 않기 위해서다 — 부모가 매번 새 함수를 넘기면 줄에서 나갔다 다시 서게 된다.
   */
      ...
      void pollQueue(ac.signal)
        .then((status: QueueStatus) => {
          ...
          if (status.status === 'matched') {
            // 줄에서 내려온다. 서버는 이 자리를 이미 지웠다(TakeQueueSeat) —
  ...
  // 탭을 닫는 것도 줄에서 나가는 것이다. 언마운트 정리로는 안 잡힌다 — 창을 닫으면
  // React 가 정리를 안 부른다.
  useEffect(() => {
    if (!searching) return;
  ...
  /**
   * 화면이 사라지는 것도 줄에서 나가는 것이다. 대국이 시작되면 이 화면이 내려가는데
   * (`GameScreen`), 그때 줄에 남아 있으면 만료까지의 몇 초 사이에 짝이 잡힐 수 있다 —
   * 그 방은 아무도 안 들어가고 상대는 60초를 기다린다.
   *
   * 값을 ref 로 본다. 의존성에 `searching` 을 넣으면 이 정리가 줄에 설 때마다 돌아서
   * 방금 선 줄을 그 자리에서 취소한다.
   */
