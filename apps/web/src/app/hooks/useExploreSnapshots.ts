 ...
 *
 * 고친 뒤에 다시 묻지 않고 서버가 답한 행으로 그 자리만 고친다. 다시 물으면 목록이 한 번
 * 사라졌다 다시 그려지고, 그 사이에 방금 저장한 줄을 찾지 못한다.
 *
 * `useFetch` 를 쓰지 않는다(useReview). 저쪽은 주소 하나를 읽는 것뿐이고, 여기는 목록의
 * 주인이라 고치는 자리가 셋이다.
 */
export interface SnapshotSource {
  ...
  signedOut: boolean;
  /**
   * 기록이 없는 배포라 이 표면이 아예 없다. 그때는 화면이 패널을 그리지 않는다. 로그인
   * 검사와 갈리는 자리다: 저쪽은 사람이 열 수 있고 이쪽은 열 방법이 없다.
   */
  unavailable: boolean;
  ...
  error: string;
  /** 지금 보고 있는 국면을 남긴다. 성공하면 true 이고, 부르는 쪽이 입력을 비운다. */
...
export function useExploreSnapshots(): SnapshotSource {
  ...
  /** 떠난 뒤에 온 답으로 상태를 고치지 않는다. 고치기 셋은 취소를 걸지 않는다. */
    ...
    fetchSnapshots(controller.signal)
      ...
      .catch((err: unknown) => {
        ...
        if (err instanceof SignedOutError || err instanceof UnavailableError) {
          // 빈 목록으로 두면 「하나도 없다」로 읽히고, 그 옆에 눌러도 듣지 않는 저장 버튼이
          // 뜬다. 붉은 알림도 쓰지 않는다. 둘 다 다시 눌러도 열리지 않는다.
  ...
  /**
   * 고치는 요청 하나를 감싼다. 셋이 같은 자리에서 잠기고 같은 자리에 실패를 남긴다. 세
   * 벌로 적으면 한쪽만 `pending` 을 내리지 않는다.
   */
