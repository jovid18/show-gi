 ...
 *
 * 끈 것만 끈 것으로 센다. 저장된 것이 없으면 켜짐이고, 그래야 `localStorage` 를 쓸 수 없는
 * 브라우저에서도 「없음」과 「읽지 못함」이 같은 답이 된다.
 */
 ...
 *
 * 되물러진 자리에서 手数가 뒤로 가는데, 그때 우는 것은 「두어졌다」는 거짓말이다. 그 수는
 * 판에 남지 않는다. `before` 가 null인 첫 렌더에서도 울지 않는다: 이어하기로 들어온 판은
 * 手数가 0이 아닌 채로 시작하고, 그것을 새 수로 세면 화면에 들어서자마자 울린다.
 */
 ...
 * 내용으로는 「새 수인가」를 가를 수 없다. 手数는 되물리지 않는 한 늘기만 한다.
 */
export function useMoveSound(ply: number): [boolean, () => void] {
  ...
  const toggle = useCallback(() => {
    setOn((was) => {
      ...
      // 켜는 그 클릭에서 한 번 울린다. 이 클릭이 곧 브라우저가 기다리던 사용자 조작이라
      // 여기가 깨우기에도 맞다.
      if (next) clack();
      return next;
    });
