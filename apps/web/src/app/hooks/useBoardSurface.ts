...
// 타입만 가져온다. 여기서 `three` 를 정적으로 들여오면 번들러가 이미 첫 덩어리에 넣어 버려서
// 아래의 늦은 import가 소용이 없다.
 ...
 *
 * 켤 때와 끌 때가 같은 값이다. 쓸어 들어오는 연출이 없어서 갈릴 이유가 없다(journal §69).
 */
const FADE_MS = 240;
 ...
 *
 * 두 값인 것은 판이 두 가지 밝기이기 때문이다. 회상 중에는 `.board-tint` 가 판 전체를 낮춰
 * 둬서 평시의 세기가 묻힌다(journal §35). 회상 쪽은 대신 그 한 칸 둘레에만 고인다.
 */
...
interface Options {
   ...
   *
   * `black` 으로 박으면 안 된다. 後手로 두는 판에서 상대가 받는 자리를 그리게 되어 판이
   * 정반대를 같은 그림으로 말한다.
   */
  me: Side;
  /**
 ...
 * 판의 자를 재서 표면에 넘긴다.
 *
 * CSS에서 베끼지 않고 판과 칸의 실제 크기에서 끌어낸다. 베끼면 여백을 고치는 날 셰이더의
 * 격자만 옛 자리에 남는다.
 */
function measure(boardEl: HTMLElement, surface: BoardSurface, layoutRef: React.RefObject<Layout | null>): void {
  // 칸을 클래스로 찾는다. `firstElementChild` 로 잡으면 아래에서 붙이는 캔버스가 걸리고,
  // 그때 cell 이 한 칸에서 판 폭으로 바뀌어 gap 이 음수가 된다(journal §127).
  //
  ...
  // 캔버스는 판의 테두리 안쪽을 채운다. 테두리 폭을 빼는 대신 판의 clientWidth 를 쓴다.
  // 그것이 정확히 캔버스가 깔리는 상자다.
...
export function useBoardSurface({ boardRef, board, active, from, me, flipped }: Options): boolean {
  ...
  /** 지금 그늘이 얼마나 들어와 있나. 1에서 시작하면 켜진 적도 없는 그늘이 첫 렌더에 번쩍인다. */
  const amountRef = useRef(0);

  // 캔버스는 손으로 붙인다. JSX로 두면 StrictMode의 두 번째 마운트가 같은 캔버스에 WebGL
  // 컨텍스트를 또 잡는데, 캔버스 하나에 컨텍스트는 하나뿐이라 앞의 것을 정리하는 순간 뒤의
  // 것이 같이 죽는다.
  //
  // three.js는 늦게 들여온다. 번들에서 제일 무거운 한 덩어리라 첫 그림을 그것 때문에
  // 기다리게 두지 않는다. 판이 처음 뜨는 자리는 CSS 판이 이미 채운다.
  useEffect(() => {
    ...
    void import('@/libs/three/surface')
      .then(({ BoardSurface, paletteOf }) => {
        // 기다리는 사이에 판이 사라졌으면 캔버스를 붙이지 않는다. 붙이고 나서 지우면
        // StrictMode의 두 번 마운트에서 캔버스가 하나 남는다.
        if (!live) return;
  ...
  // 판이 바뀌면 그늘을 다시 센다. 회상에서는 장면마다 다시 센다. 그 장면의 판에서 나온
  // 값이라 어느 장면에서도 그늘이 그 판의 사실이다.
  useLayoutEffect(() => {
    const surface = surfaceRef.current;
  ...
  // 밀려들고 물러난다. 움직임을 줄여 달라고 한 사용자에게는 그 자리에서 켜진다. 밀려드는
  // 것은 분위기이고, 어느 칸이 깊은가가 정보다(詰み 게이지와 같은 기준).
  useEffect(() => {
    const surface = surfaceRef.current;
    ...
    // 회상에서는 판을 다 덮지 않는다. 물러진 수가 간 칸 둘레에만, 대신 더 깊게 고인다
    // (RECALL_DEPTH).
    ...
    const step = (now: number) => {
      ...
      // 켜질 때도 꺼질 때도 한꺼번에 옅어지고 짙어진다. 반지름을 0에서 키우면 퍼지는 고리가
      // 정보로 읽히는데, 그늘이 말하는 것은 「어느 칸이 깊은가」뿐이다(journal §69).
