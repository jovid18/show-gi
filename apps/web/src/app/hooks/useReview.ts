 ...
 * 화면에서 전부 다른 말을 해야 한다. 합치면 빈 목록이 오류처럼 보인다.
 */
 ...
 *
 * 떠난 요청은 버린다. 목록에서 판을 빠르게 옮겨 다니면 응답이 순서대로 오지 않고, 그때 늦게
 * 온 것이 화면을 덮으면 다른 판의 기보를 지금 판이라고 그린다.
 *
 * 퀴즈도 이걸 쓴다(useQuiz). `reload` 로 다시 물을 때는 직전 답을 그대로 들고 있고, 무엇을
 * 남기고 무엇을 비우는지는 `startingLoad`·`afterFailure` 가 정한다.
 */
 ...
 * 그리던 컴포넌트가 언마운트되고, 되짚기에서는 고른 手数와 가정 수순이 사라진다(journal §136).
 *
 * 주소가 바뀌면 비운다. 새 판을 받는 동안 앞 판의 기보가 남아 있으면 화면이 남의 판을 지금
 * 판이라고 말하게 된다.
 */
...
/**
 * 그 판의 총평. 기보와 따로 받아 판을 먼저 그리고 총평을 뒤에 채운다. 대국에서 스냅샷과
 * 총평이 갈라져 오는 것과 같은 자리다(journal §49).
 *
 * 돌아오는 것은 `protocol/game.ts` 의 `GameSummary` 이고, `protocol/review.ts` 의 같은
 * 이름(목록 한 줄)과 다른 타입이라 여기서 이름을 바꿔 받는다.
 */
 ...
 * 갈리는 자리다(server.go). 이 값을 묻는 것은 가정 수순 하나 때문이다. 눌러도 503만
 * 돌아오는 버튼은 「고장 났다」로 읽히므로 그 자리를 미리 닫고 이유를 적는다.
 *
 * `null`은 아직 모른다이다. 물어보지 못했다고 없는 것으로 치면 /healthz 만 실패한 상황에서
 * 멀쩡한 기능 전체가 사라진다. 그때는 열어 두고 서버가 답하게 둔다.
 */
