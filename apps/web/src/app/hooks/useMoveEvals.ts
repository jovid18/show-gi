 ...
 * `playerScore`). 후보(`WhatIfCandidate.evalCp`)는 둔 쪽 관점이라, 한 열에 함께 놓는 쪽이
 * 뒤집어서 쓴다.
 *
 * 둘은 배타적이다. 詰み인 자리에는 `mateIn` 만 오고 `cp` 는 오지 않는다. 화면은 手数로
 * 말한다(`scoreJa`).
 */
 ...
 * 시절의 판이 있다(migrations/005 이전, journal §39 ⑥). 그래서 그 자리를 다시 재서 채운다.
 *
 * `useWhatIf` 를 쓰지 않는다. 그쪽은 「지금 서 있는 분기」를 갖고 있는 장치라, 값을 얻으려고
 * 그것을 건드리면 판이 딴 데로 간다. 여기는 읽기만 한다.
 *
 * 한 번에 하나씩 묻는다. 한 국면에서 다섯 수를 물린 판이 있고(622의 159手), 그걸 동시에
 * 던지면 대국과 공유하는 엔진 풀을 그만큼 잡는다(01-core.md §4).
 */
    ...
    void (async () => {
      ...
      for (const usi of usis) {
        ...
        try {
          ...
          if (!alive) return;
          // 끝난 국면(詰み·手詰まり)에는 값이 없다. 그 줄은 값 없이 남는다: 0으로 채우면
          // 호각으로 읽힌다.
          //
          // 「詰みが見える」와 「값이 없다」를 가른다. 앞엣것은 `mateIn` 만 오고 `evalCp` 는
          // 오지 않으므로(서버 쪽 규약) cp 만 보고 자르면 그 줄 전체가 사라진다.
