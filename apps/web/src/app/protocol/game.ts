 ...
 *
 * `resigned` 와 따로 둔다. 엔진이 답하지 않은 것을 「相手が投了しました」로 그리면 지고
 * 있던 판이 화면에서 이긴 판이 된다.
 */
...
export interface Intervention {
   ...
   *
   * 화면은 이걸로 문장을 짓지 않는다. 문구는 `message` 에 있고, 두 벌이 되면 어긋났을 때
   * 어느 쪽이 맞는지 알 수 없다. 연출을 카테고리별로 가르는 자리라 문자열로 열어 뒀다.
   */
  category: string;
  /** 물러진 수의 USI. */
  retractedUsi: string;
 ...
 * 반박 수순의 한 수.
 *
 * `sfen` 이 있어서 화면은 수를 두지 않는다. 클라이언트가 스스로 두면 규칙 엔진을 한 벌 더
 * 갖게 된다.
 */
export interface RefutationMove extends KifuMove {
   ...
   *
   * 어느 말이 걸고 있는지는 규칙을 알아야 하고, 화면은 규칙을 갖지 않는다. 両王手는 여기서
   * 둘로 오고, 그 둘이 「먹어서 풀 수 없다」의 근거다.
   */
  checks?: Attack[];
}
 ...
 *
 * 단계는 서버가 자른다(`game.buildHint`). 답이 페이로드에 있으면 화면이 그리지 않아도
 * 이미 알려준 것이 된다.
 *
 * 개입(`intervention`)과 수명은 같고 방향이 반대다. 저쪽은 방금 둔 수를, 이쪽은 지금 둘
 * 수를 말한다.
 */
 ...
 * 섞으면 「시한을 넘겨 확인하지 못했다」가 「이 수는 괜찮았다」로 읽힌다.
 */
...
export interface Snapshot {
   ...
   *
   * 初期配置에서는 비어 있다. 첫 수 전에 이름을 그리면 아직 하지 않은 선택에 이름을 붙이게 된다.
   */
  styleTags?: StyleTag[];
  /**
   ...
   *
   * 상대 玉 쪽 하나뿐이라, 불이 붙으면 언제나 「내가 詰み에 가까워졌다」다. 같은 테두리에
   * 둘을 그리면 이기는 중인지 지는 중인지가 반대로 읽힌다.
   *
   * 세기만 온다. 手数가 여기 있으면 화면이 그리지 않아도 이미 알려준 것이 된다.
   */
  mateHeat?: number;
  /**
   ...
   *
   * `canUndo` 와 뜻이 다르다. 이쪽은 예산이라 상대 차례에도 그대로 참이고, 저쪽은 「지금
   * 이 순간 누를 수 있나」다.
   */
  undoLeft: number;
  /**
   * 지금 무르기를 누를 수 있는가. 사람 차례인지, 예산이 남았는지, 되돌릴 사람의 수가
   * 있는지를 서버가 이미 봤다.
   */
  canUndo: boolean;
...
export type ClientMessage =
  ...
  | { type: 'resign' }
  // 手数를 보내지 않는다. 무엇을 되돌릴지는 서버가 자기 기보에서 정한다. 화면이 짚으면
