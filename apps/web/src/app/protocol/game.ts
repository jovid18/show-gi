...
export interface RefutationMove extends KifuMove {
   ...
   * 화면은 규칙을 갖지 않는다. 両王手는 여기서 둘로 오고, 그 둘이 「먹어서 풀 수
   * 없다」의 근거다.
   */
  checks?: Attack[];
}
...
export interface Snapshot {
   ...
   * 규칙(docs/01-core.md §7)과 어긋나지 않는 근거는 서버의 `Snapshot.OpponentOpening` 에 있다.
   */
  opponentOpening?: string;
  /**
   * 이 판의 「형세 0」(플레이어 관점 cp). 平手면 오지 않는다.
   *
   * 개입 카드의 후보 줄 색이 이 값을 뺀다(`evalTone`) — 빼지 않으면 駒落ち에서 모든 줄이
  ...
  hint?: Hint;
  /** 대국은 계속되는데 서버가 해주지 못한 것. 다음 착수에서 사라진다. */
  notice?: Notice;
  /**
   ...
   *
   * 오지 않으면 조절이 꺼져 있다 — 0이나 3으로 메우지 않는다. 조절하지 않는 판에
...
export type ServerMessage =
  ...
  // 가정 수순의 한 자리. 스냅샷과 따로 온다 — 「벌어지지 않은 일」이고, 하나로 합치면
  // 화면이 두 판을 같은 것으로 그린다.
