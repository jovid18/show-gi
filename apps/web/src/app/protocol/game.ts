...
export interface Snapshot {
  ...
  hintLeft: number;
  /**
   * 지금 힌트를 누를 수 있는가. **국면마다 갈린다는 것이 `canUndo` 와 다른 점이다** —
   * 예산이 남아도 같은 자리를 세 번째로 물으면 false 다.
   */
  canHint: boolean;
}
 ...
 * 대국이 끝난 뒤 **한 번** 오는 총평.
 *
 * 스냅샷과 갈라져 온다 — 이건 국면의 상태가 아니라 판 전체에 대한 이야기이고, **기록이
 * 다 쓰이기를 기다리므로** 결과 문구보다 늦게 도착한다(서버의 `dbRecorder.done`).
 *
...
export interface GameSummary {
  /** 화면에 그대로 나가는 일본어. **절대 비지 않는다** — 사실이 없으면 짧아질 뿐이다. */
  body: string;
  /**
...
export type ClientMessage =
  ...
  | { type: 'undo' }
  // **국면을 안 보낸다.** 어느 국면의 힌트인지는 서버가 자기 판에서 정한다 — 화면이
  // 짚으면 그 사이에 도착한 스냅샷과 어긋난 자리의 최선수가 나온다(`undo` 와 같은 규약).
  | { type: 'hint' }
