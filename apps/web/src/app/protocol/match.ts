...
//
// `protocol/game.ts` 와 따로 둔 이유는 여기 없는 것들 때문이다. 개입·힌트·待った·
...
/**
 * 승패가 없는 끝이 둘이다. 따로 두는 이유는 화면이 할 말이 정반대이기 때문이다.
 *
 * - `aborted` — 서버가 내려갔다. 두 사람 다 잘못한 것이 없다
 * - `expired` — 한 수도 안 둔 채 시간이 다 됐다. 누구도 안 뒀으면 판이 없었던 것이라
 *   승패를 안 적는다
 *
...
/**
 * 기보의 한 수를 누가 뒀나. 절대 이름(先手/後手)이 아니라 보는 사람 기준이다.
 *
