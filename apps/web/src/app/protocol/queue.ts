...
//
// `protocol/match.ts` 와 갈라 둔 이유는 여기 판이 하나도 없기 때문이다. 큐에 서는 것은
...
export type QueueStatus =
  | {
      status: 'waiting';
      /** 큐에 선 뒤로 흐른 시간(ms). 정본이 서버라 새로고침해도 이어 센다. */
      waitedMs: number;
      /** 지금 큐에 서 있는 사람 수(자기 포함). */
      waiting: number;
    }
  | {
      status: 'matched';
...
/**
 * 큐에 서거나, 이미 서 있으면 다시 물어본다.
 *
 * 같은 호출 하나가 셋을 한다 — 큐에 서기 · 살아 있다고 알리기 · 짝짓기. 그래서 화면은
 * 이것을 주기적으로 부르기만 하면 되고, 멈추면 서버가 알아서 큐에서 걷어낸다
...
/**
 * 큐에서 빠진다. 안 부르고 화면을 떠나도 서버가 걷어가지만, 그때까지 상대에게는
 * 내가 큐에 있는 것으로 보인다 — 그 사이에 잡힌 짝은 아무도 안 오는 방이 된다.
 */
