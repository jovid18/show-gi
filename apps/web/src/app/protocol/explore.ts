 ...
 * 「이 뿌리에서 이 수순을 뒀다면」.
 *
 * 뿌리가 둘이다 — 手合割 id 하나와 판(SFEN) 하나이고, 둘은 같이 올 수 없다. 판을 받게 된
 * 것은 사진에서 읽어 온 국면 때문이고(journal §129), 그 국면은 手合割+수순으로 표현할
 * 수가 없다. 문을 열어도 「아무 국면이나 재 주는 공개 엔진」이 되지는 않는다 — 서버의
 * 슬롯 하나가 막는다(§85).
 *
 ...
 * 다시 묻는다(`routeExplore`). SFEN 을 왜 저장하지 않는지는 journal §96 에 있다.
 *
...
export interface ExploreSnapshot {
  id: number;
  name: string;
  /** 手合割 id. 平手면 오지 않는다 — 빈 값이 平手라는 규약을 그대로 쓴다. */
  handicap?: string;
  /** 그 手合割의 일본어 이름. 平手면 오지 않는다 — 화면이 id로 이름을 만들지 않는다. */
  handicapJa?: string;
  moves: string[];
  /** 저장한 시각(ISO). */
  savedAt: string;
