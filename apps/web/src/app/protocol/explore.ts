...
// (`useWhatIf`) 갈리는 것은 뿌리 하나뿐이라, 여기서 새로 정하는 것은 **요청**과 **그 手合의
// 「형세 0」** 두 칸이다.
 ...
 * 0手目라, SFEN을 받는 표면이었다면 그건 **아무 국면이나 깊이 12로 재 주는 공개 엔진**이
 * 된다. 보내는 것은 手合割 id와 수순뿐이고 서버가 한 수씩 되짚어 검증한다.
 *
 * **手数가 없다.** 뿌리가 언제나 0手目라 되짚기의 `ply` 에 해당하는 값이 상수다.
 */
export interface ExploreRequest {
  /** 手合割 id. **빈 값이 平手**다(`/api/handicaps` 목록에 平手가 없는 것과 같은 규약). */
  handicap: string;
...
/**
 * 검토의 한 국면. 가정 수순의 노드에 **그 手合의 기준점**이 얹힌다.
 *
 * **화면이 그 값을 만들 수 없다.** 二枚落ち의 0手目가 +1490인데 그것을 모르면 화면이
