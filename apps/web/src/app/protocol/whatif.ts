...
/**
 * 그 국면에서 **수번 쪽**이 둘 수 있는 좋은 수 하나.
 *
 * 첫 번째가 최선수이고 그것이 판 위의 **초록 화살표**다 — 「다음에 올 수」에 이미 배정된
...
export interface WhatIfNode {
  ...
  ply: number;
  sfen: string;
  /** 지금 手番. **합법수와 후보가 이 쪽의 것**이고, 어느 駒台를 집을 수 있는지도 이 값이 정한다. */
  turn: Turn;
  yourTurn: boolean;
  checked?: string;
