...
export interface QuizPayload {
   ...
   * 걸리므로 그 사이에 「問題はありません」을 그리면 거짓이 된다.
   */
  ready: boolean;
  /**
   * 문항이 아직 오는 중인가. `ready` 가 거짓일 때만 뜻이 있다.
   *
   * 둘 다 거짓이면 오지 않는다 — 화면이 그 자리에서 기다리기를 그만둔다. 시간으로
   * 재던 것을 이 값이 대신한다: 문항이 큐를 지나면서 벽시계가 「큐에서 기다린 시간 +
   * 만드는 시간」이 됐고, 만드는 시한 하나로는 그것을 말할 수 없다.
   *
   * 큐만 보지 않는다. 가져온 판은 手를 다 재고 나서야 문항을 큐에 세우므로, 재는
   * 동안도 참이다 — 가져오기 직후에 여는 자리가 그것이다.
   *
...
export interface MateItem {
   ...
   * 둘 수 있는 수 — 王手인 것만이다.
   *
   * 詰将棋에서 攻方은 매 수 王手를 걸어야 하므로 그 밖은 애초에 문항의 입력에서 빠진다.
...
export interface MateAttempt {
  moves: string[];
  /**
   * 이 문항에서 몇 번째 시도인가(1부터). 화면이 센다 — 서버에 남기지 않는다.
   *
   * 몇 번 틀렸는지는 그 판의 사실이라기보다 지금 이 사람이 이 화면에서 하고 있는 일이고,
...
export interface MateResult {
   ...
   * 끝난다(playtests/2026-08-14-human-2.md §6 #10 · #11).
   */
  hint?: string;
}
...
export interface BestResult {
   ...
   *
   * 문구에서 지우는 것으로는 되지 않았다 — 응답에 남아 있으면 화면이 그것을 아래 cp 표에
   * 그대로 적고 있었다.
   */
  answer?: string;
  answerJa?: string;
