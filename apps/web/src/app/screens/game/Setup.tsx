 ...
 * 대국을 시작하기 전에 고르는 화면.
 *
 * 여기서 고르기 전에는 서버에 붙지 않는다(`useGame`). 미리 붙으면 그때 판이 하나 열려 기록에
 * 남고, 아무것도 고르지 않은 채로 先手 平手 대국이 시작된다.
 *
 * 고를 것이 셋이다: 얼마나 접나(手合割) · 어느 쪽을 잡나 · 상대가 무엇을 하나. 난이도 눈금은
 * 여기 없다. 두는 동안 상대가 스스로 맞춘다(journal §47).
 *
 * 手合割이 맨 위이고, 고르면 아래 둘이 사라진다. 駒落ち는 사람이 下手로 정해져 있고(journal
 * §88) 진형은 平手 수순이라 같이 쓸 수 없다. 서버도 같은 순서로 덮으므로(`newSetup`) 화면은
 * 그 규칙을 되비추기만 한다.
 */
...
/** 사람과 둘 때의 手番. 상대가 사람이면 振り駒가 원래 정하는 법이라 그것이 하나 더 있다. */
...
interface SetupProps {
  /** 지난 판의 설정. 매번 처음부터 고르게 하면 「もう一局」이 두 번 더 눌러야 하는 일이 된다. */
...
export function Setup({ initial, onStart }: SetupProps) {
  ...
  return (
    <div className="setup">
      ...
      {/* 상대가 사람인 갈래는 여기서 갈린다. 위에서 고른 것을 하나도 쓰지 않는다. 手番은
          振り駒가 붙어 저쪽이 따로 고르고(FriendMatch), 手合割과 戦型은 컴퓨터에게 시키는
          것이라 사람 상대에게는 뜻이 없다. */}
      <FriendMatch />

      {/* 사람 상대의 두 번째 갈래. 위가 「아는 사람에게 링크를 보낸다」이고 이쪽이 「모르는
          사람을 서버가 찾아 준다」다. 상대가 있는 사람에게는 링크 쪽이 확실해서 아래다. */}
      <Search />

      {/* 홈 메뉴에서 누르지 않은 사람이 다시 만나는 자리는 여기 하나다. 위에 두면 두러 온
          사람을 먼저 붙잡으므로 시작 버튼 아래다.
...
/**
 * 平手에서만 고르는 둘. 手番과 상대의 진형이다.
 *
 * 한 덩이로 따로 둔다. 駒落ち에서 둘이 같이 사라지고 조건도 하나라서, 조건을 두 군데 두면
 * 나중에 한쪽만 남는다(Setup 의 doc).
 */
 ...
 * 사람과 두는 갈래로 나가는 문.
 *
 * 로그인해야 열린다. 익명은 서로 구별할 수단이 없어서 「이 방의 상대가 아까 그 사람인가」에
 * 답할 수 없고, 그러면 정원 2명이라는 규칙이 성립하지 않는다.
 *
 * 눌러도 아무 일도 일어나지 않는 버튼 대신 이유를 적은 줄이 뜬다(journal §63).
 *
 * 手番을 위 화면과 따로 고른다. 여기는 사람 상대라 振り駒가 붙고, 같은 값을 쓰면 그 선택지가
 * 엔진 대국으로도 새어 나간다.
 */
function FriendMatch() {
  ...
  // 로그인이라는 것이 이 배포에 없으면 자리를 아예 그리지 않는다. 안내문을 띄우면 없는
  // 기능을 말하게 된다.
        ...
        <>
          ...
          <button
            ...
            onClick={() => {
              ...
              setError(null);
              const ac = new AbortController();
              // 성공해도 되돌린다. 이 화면은 방으로 옮겨 가도 `App` 이 `hidden` 으로만
              // 감추므로, 여기서 되돌리지 않으면 돌아왔을 때 버튼이 영영 눌리지 않는다.
