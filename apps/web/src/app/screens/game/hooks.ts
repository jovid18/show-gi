// 대국 화면만 쓰는 훅. `hooks/` 는 여러 화면이 같이 쓰는 자리다(useGame·useWhatIf).
 ...
 *
 * 사이드바에 상시 띄우지 않는다. 「中飛車」가 상태 문구 아래에 홀로 떠 있으면 무엇을 가리키는
 * 말인지 알 수 없어 라벨을 하나 더 얹어야 하는데, 짜는 순간 판 위에 잠깐 떴다 사라지면 그것이
 * 위치와 타이밍으로 전달된다(03-frontend.md).
 *
...
export function useTagAnnounce(tags: StyleTag[] | undefined, ply: number): [StyleTag | null, () => void] {
  ...
  // 언마운트를 타이머로 하지 않는다. `setTimeout` 으로 지우면 길이가 CSS 애니메이션과 두
  // 벌이 되고, 어긋나면 요소가 DOM 에 남은 채 `opacity: 0` 으로 보이지 않는다. 애니메이션이
  // 끝나는 것을 신호로 쓰면 길이의 주인이 CSS 하나가 된다.
