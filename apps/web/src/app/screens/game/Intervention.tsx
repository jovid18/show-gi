...
interface InterventionProps {
  ...
  /**
   * 분기의 지금 자리. `null` 이면 아직 받지 못했고, 그동안에도 카드는 그대로 떠 있다.
   */
   ...
   *
   * 후보 줄의 색만 이 값을 쓰고(`evalTone`) 숫자는 원본 cp 그대로다.
   */
  baselineCp: number;
  /**
   * 판을 만질 수 있는가. 대국 화면이 이미 정한 값이라 여기서 다시 계산하지 않는다. 두 벌이면
   * 어긋난다(成りますか가 떠 있는 동안이 실제로 그 자리였다).
   */
  playable: boolean;
...
/** 목록의 한 줄. 엔진이 고른 후보와 사람이 둬 본 수가 같은 모양으로 그려진다. */
interface Row {
  usi: string;
  ja: string;
  ...
  /**
   * 최선수 대비 낙폭. 서버가 준 것만 쓴다. 여기서 뺄셈을 하면 낙폭의 정의가 두 벌이 되고,
   * 사람이 둬 본 수는 그 자리의 후보 탐색을 지나지 않아 애초에 기준이 없다.
   */
...
/**
 * 최선수 대비 낙폭 한 칸. 「この手を選ぶといくら損か」이고, 사람이 직접 요구한 값이다(회차 1 #11).
 *
 * 1위 줄의 「最善」이 열 제목을 대신한다. 맨 위에 그 한 단어가 있어서 아래의 「−320」이 무엇에
 * 대한 값인지가 그 자리에서 읽히고, 1위를 색으로만 표시하던 자리라 색을 구별하지 못하는
 * 사람에게도 1위가 남는다.
 *
 * 낙폭이 없는 줄은 비운다. 0으로 채우면 「최선수와 같다」는 거짓이 된다.
 */
 ...
 *
 * 되짚기의 `branchStatusJa` 와 시제가 다르다. 저쪽은 끝난 판을 보는 자리라 「負けでした」이고,
 * 여기는 아직 벌어지지 않은 일이라 「負けになります」다.
 */
function noteJa(node: WhatIfNode | null, pending: boolean, failed: boolean): string {
  // 튕겼으면 「조사 중」이라고 말하지 않는다. 기다리면 온다는 거짓이 된다.
  ...
  switch (node.status) {
    ...
    default:
      // 어느 쪽 차례든 사람이 둔다. 상대 차례에는 초록 화살표가 엔진의 답을 짚고 있다.
 ...
 * 제지형 개입의 문구 쪽.
 *
 * 최선수를 말하지 않는다(docs/01-core.md §1). 여기 나오는 수는 전부 물러진 수 뒤의 것이고 그
 * 국면은 이미 사라졌다. 바닥을 지키는 쪽은 `useWhatIf` 의 `floor` 와 서버의 `branchRoot` 둘이다.
 *
 * 이 카드는 다른 것으로 바뀌지 않는다(journal §54). 분기가 되는 것은 판뿐이다.
 *
...
export function Intervention({
  ...
  // 입력이 판 쪽으로 열려 있어도 초점의 첫 자리는 여기다. 키보드·스크린리더 사용자가 판을
  // 더듬어 「指し直す」를 찾게 두지 않는다.
  useEffect(() => {
   ...
   *
   * 자를 자리가 없는 하나뿐인 수순이라 남았다(journal §54).
   */
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    // 사람이 둬 본 수도 표식 없이 같은 줄로 그린다. 읽는 사람에게 그 둘은 같은 물음의
    // 답이다: 이 수를 두면 얼마인가.
  ...
  // 낙폭은 서버가 준 0~1이다. 보이는 단위로만 바꾸고, 막대가 넘치지 않게만 자른다. 숫자를
  // 손보기 시작하면 화면과 판정이 갈라진다.
  ...
  /** 한 자리도 받지 못한 채 튕겼다. 저절로 풀리지 않아 사람이 다시 눌러야 한다. */
  ...
  return (
    <div className="intervention" role="alert">
      ...
      <p className="intervention-move">
        ...
        {/* 그 수를 두면 얼마가 되나. 낙폭(%)은 판정의 값이고 이건 국면의 값이다. 아래
            목록의 cp와 같은 자를 쓰므로 「거기서 더 나빠지는가」가 견줘진다. */}
      ...
      <div className="intervention-branch">
        ...
        {/* 지금까지 둬 본 줄. 뿌리에서는 아예 그리지 않는다. 빈 자리를 만들어 두면 목록이
            그만큼 아래로 밀려 매번 높이가 흔들린다. */}
        {branching && (
        ...
        {/* 자리는 지키고 내용만 기다린다. 목록을 통째로 바꾸면 값이 올 때마다 카드가
            무너졌다 다시 그려진다(WhatIfPanel 과 같은 자리). */}
          ...
          {rows.map((r) => (
            <li key={r.usi}>
              <button
                ...
                // 파랑·빨강이 「나에게 좋은가」라 플레이어 관점으로 뒤집어 넘긴다(evalTone).
                style={
                  {
        ...
        {/* 여기가 막히면 카드에 남는 것이 문구뿐이다. 노드가 없으면 목록도 무르기도 뜨지
            않고, 요청은 회차가 끝날 때까지 저절로 다시 나가지 않는다. */}
        {stuck && (
        ...
        {/* 되돌아가는 길이 둘이다. 몇 수를 들어간 뒤에 처음으로 돌아가려고 「一手戻る」를
            다섯 번 누르게 두지 않는다. 바닥은 물러진 수다(useWhatIf 의 floor). */}
        {branching && (
          <div className="intervention-branch-actions">
