 ...
 * 아무나와 두는 갈래 — 대기열.
 *
 * 링크 방식(`Setup.tsx` 의 `FriendMatch`)과 갈리는 것이 셋이다. 手番을 고르지 않고
 * (先手 랜덤), 手合割도 없고(平手 확정), 짝이 잡히면 확인 화면 없이 그 방으로 들어간다 —
 * 스스로 대기열에 선 사람에게는 「자리를 태워도 되나」를 물을 이유가 없다(journal §92).
 *
 * 상대에 대해 아무것도 보여주지 않는다. 段級도 전적도 레이팅도 없다 — 레이팅은 밴드를
      ...
      {me.user === null ? (
        ...
        <>
          <p className="setup__caveat">
            {/* 정한 것 넷을 누르기 전에 말한다. 手番·手合割을 고를 수 없는 것은 이 갈래의
                성질이고, 적어 두지 않으면 위에서 고른 것이 무시된 것으로 읽힌다. */}
          ...
          {searching ? (
            <>
              ...
              <p className="setup__caveat">
                {/* 「짝이 잡히지 않는 것」과 「고장」을 여기서 가른다. 동시에 기다리는 사람이
                    없으면 잡히지 않는 것을 그대로 받아들이기로 정했고(journal §92), 그러면
