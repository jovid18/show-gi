...
export function ExploreScreen({ handicap, moves }: ExploreScreenProps) {
   ...
   *
   * 이 표면의 실패 둘은 **설계된 것**이다 — 검토가 이미 하나 돌고 있으면 429이고
  ...
  /**
   * 手合割을 고른다. **이쪽은 이력을 쌓는다**(`replace` 없이).
   *
   * 手合が바뀌면 뿌리가 다른 판이라 줄이 통째로 없어지는데, 이력에 남겨 두면 뒤로 가기
   * 한 번으로 방금 보던 줄이 그대로 돌아온다 — 「정말 버립니까」를 묻지 않는 이유다.
   */
  const pickHandicap = useCallback((id: string) => {
    navigate({ name: 'explore', handicap: id, moves: [] });
  }, []);
   ...
   * 보러 오는 화면**이라 그 근거가 서지 않는다 — 옆의 목록이 이미 같은 수를 첫 줄로 들고
      ...
      <div className="game" data-flipped={flipped || undefined}>
        <div className="game-board">
          ...
          {/* **판이 없으면 駒台도 안 그린다.** 빈 받침 둘만 남으면 판이 있어야 할 자리에
              ...
              그 그림이었다. 받침이 자리를 지키는 규칙은 **판이 있을 때**의 것이다(`Hand`). */}
          {board ? (
            <>
              ...
              <Board
                ...
                hintRay={null}
                mateHeat={0}
                // 그늘(`相手の利き`)의 기준. **아래에 있는 쪽**이다 — 검토에는 「나」가 없어서
                // 판을 돌리면 보는 쪽도 함께 돈다.
            ...
            // 이유는 옆 패널이 이미 말한다(`error`).
            // **엔진이 없으면 기다릴 것이 없다.** 그때 「읽는 중」은 영원히 오지 않는
            // 약속이고, 이유는 옆 패널이 이미 말한다.
        ...
        <aside className="game-side">
          ...
          {/* 지금까지의 줄. 실제 기보와 **같은 어휘로 같은 모양**으로 선다 — 手数 · 수 · cp.
