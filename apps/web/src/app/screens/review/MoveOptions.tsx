 ...
 *
 * 최선수와 물러진 수와 실제로 둔 수를 한 목록에 담아 평가치로 정렬한다. 따로 두면 「내가 둔
 * 것이 최선수 몇 위쯤인가」를 눈으로 견줄 수 없다.
 *
 * 값은 전부 지금 잰 것이다. 저장된 `moves[].evalCp` 는 대국 중 다른 k 로 잰 값이라 후보들과
 * 자가 다르고(journal §34 ②), 한 줄에 두 자를 섞지 않는다.
 */
interface MoveOptionsProps {
  ...
  ply: number;
  /**
   * 서 있는 국면. 후보가 여기서 나오고, 분기로 한 수 두면 그 다음 국면의 후보로 갱신된다.
   *
   * 실제로 둔 수·물러진 수는 뿌리에서만 나온다(`line.length === 0`). 그 둘은 확정된 기보의
   * 사실이라 가정으로 몇 수 들어간 국면에 얹으면 거짓이 된다.
   */
  node: WhatIfNode | null;
  /**
   * 후보 목록 밖의 수를 다시 잰 값(`useMoveEvals`). 플레이어 관점이고, 아래 `moverScore` 가
   * 이 열의 자(둔 쪽 관점)로 옮긴다.
   *
   * 두 자리가 이걸 쓴다: 값이 저장돼 있지 않은 물러진 수와, 실제로 둔 수다. 뒤엣것은 후보 셋
   * 밖이면 서버가 값을 준 적이 없어, 재지 않으면 그 줄만 빈칸으로 남는다(회차 2 #7).
   */
   ...
   * 줄을 눌렀을 때.
   *
   * 실제로 둔 수에는 「가정」을 붙이지 않는다. 그 줄을 분기로 열면 판은 같은 국면에 머무는데
   * 화면이 「もしも」라고 말하게 되므로, 한 수 진행으로 처리한다(`played`).
   */
...
interface Option {
  usi: string;
  ja: string;
  /** 지금 잰 평가치, 둔 쪽 관점. 재지 못한 줄은 undefined 이고 자리만 지킨다. */
  cp: number | undefined;
   ...
   * 사람이 스스로 무른 수라면 그 횟수.
   *
   * `retracted` 와 따로 둔다. 한 수가 둘을 겸할 수도 있고, 한 칸으로 합치면 「AI 가 막았다」와
   * 「내가 되돌렸다」가 같은 표식이 된다.
   */
   ...
   *
   * `retracted` 와 따로 둔다. 저쪽은 기보에 없는 수인데 이쪽은 그대로 둔 수라, 같은 칸에 넣으면
   * 화면이 그 수를 물러진 것으로 그리고 없던 개입을 있었다고 말하게 된다(journal §126).
   */
...
export function MoveOptions({ game, ply, node, measured, chosen, onPick }: MoveOptionsProps) {
  ...
  const options = useMemo<Option[]>(() => {
    ...
    /**
     * 한 줄을 겹쳐 쓴다. 먼저 들어온 값을 `undefined` 로 덮지 않는다. 후보의 값은 이 국면의
     * 탐색에서 나온 것이고, 다시 잰 값은 그 자리가 빈 줄에만 필요하다.
     */
    ...
    // 그 줄만 빈칸이고 정렬도 맨 아래로 보낸다(회차 2 #7).
    ...
    // 가져온 판의 悪手는 실제로 둔 수 그 자체다. 아래 「물러진 수」 목록에는 줄이 없으므로
    // (`retractedUsi` 가 비어 있다) 여기서 이름을 붙이지 않으면 판정한 것이 화면 어디에도
    // 나오지 않는다.
    ...
    for (const [usi, { iv, tries }] of tried) {
      // 저장된 것이 있으면 그것, 없으면 다시 잰 것. 둘 다 플레이어 관점이라 같은 자로 옮긴다
      // (`afterCp` 는 `moves[].evalCp` 와 같은 자다). 詰み이면 手数 쪽이 온다: 두 칸이 같이
      // 오지 않는 것이 서버 쪽 규약이다.
    ...
    // 사람이 스스로 무른 수(待った). 물러진 수와 같은 자리에 같은 규약으로 그려진다. 둘 다
    // 「이 국면에서 뒀는데 기보에 없는 수」라서 여기 말고는 보일 자리가 없고, 갈리는 것은 표식
    // 하나다: 저쪽은 AI 가 막았고 이쪽은 사람이 되돌렸다(회차 1 #4).
    ...
    for (const [usi, tries] of undone) {
      // 저장된 평가치는 `moves[].evalCp` 에서 옮겨 온 것이라 이미 플레이어 관점이다(물러진
      // 수의 `afterCp` 와 같은 자, 같은 변환). 詰み이면 手数 쪽이 오므로(`ReviewUndo.evalCp`)
      // 여기서도 手数를 먼저 본다.
  ...
  return (
    <section className="review-panel review-options" aria-label="この局面で指せた手">
      {/* 어느 쪽 수인지를 양쪽 다 적는다. 「相手の番」만 적으면 말이 없는 것이 「내 차례」인지
          「아직 모른다」인지 갈리지 않는다. 분기로 몇 수 들어가면 차례가 한 수씩 바뀌므로 그
          자리에서 특히 그렇다. */}
          ...
          <li key={o.usi}>
            ...
            >
              ...
              <span className="review-options-tags">
                ...
                {/* 가져온 판의 悪手. 위 줄과 모양이 같다. 한 판이 둘을 겸할 수 없어서
                    (가져온 판에는 개입이 없다) 같은 목록에 나란히 설 일이 없다. */}
                {o.blunder && <span data-role="blunder">{o.blunder.categoryJa || '悪手'}</span>}
                {/* AI 가 막은 것과 다른 표식이다. 저 줄은 카테고리 이름(タダ捨て)을 쓰고 이
                    줄은 「待った」를 쓴다. 같은 목록에 나란히 서므로 표식이 갈려야 「막힌 수」와
                    「내가 되돌린 수」를 구별할 수 있다. */}
                {o.undone && (
            ...
            {/* 가져온 판의 悪手는 고르는 것과 무관하게 편다. 그 줄은 실제로 둔 수라 눌러도
                분기가 열리지 않아(`onPick` 의 played) 위 조건으로는 문구에 닿는 길이 없고,
                한 국면에 悪手 줄이 하나뿐이라 목록이 글이 되지도 않는다. */}
