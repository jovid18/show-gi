 ...
 *
 * 되짚는 화면 전용이다. 대국 중에도 둬 보는 길이 있지만(journal §54) 그쪽은 이 패널이
 * 아니라 개입 카드 안이다 — 두 패널이 같은 자리를 다투면 읽던 설명이 사라진다
 * (`Intervention`).
 *
 * 여기서 최선수 목록을 아무 手数에서나 그릴 수 있는 것은 끝난 판이라서다. 살아 있는 판은
 * 뿌리가 물러진 수 하나로 못박혀 있고, 그 제한이 「지금 어떻게 둬야 하나」의 답을 막는다
 * (01-core.md §7 · `useWhatIf` 의 `floor` · 서버의 `branchRoot`).
 *
 * 국면을 기다리는 동안에도 이 패널은 그대로 있는다. `node` 가 `null` 이면 아직 못 받은 것이고,
 * 그때 통째로 다른 컴포넌트로 바꾸면 手数를 넘길 때마다 옆 열이 무너지고 다시 그려진다 — 그것이
...
interface WhatIfPanelProps {
   ...
   * 다른 국면의 숫자가 남아 있으므로, 흐리게 하고 누를 수 없게 한다.
   */
  stale: boolean;
  pending: boolean;
  error: string | null;
  ...
  return (
    <section className="review-panel review-whatif" aria-label="もしもの手順">
      ...
      {/* 기다리는 동안 글자를 바꾸지 않는다. `読んでいます…` 로 바꾸면 手数를 넘길 때마다
          500ms씩 문구가 번쩍인다. 「아직 이 국면의 값이 아니다」는 흐림이 이미 말한다
      ...
      {/* 최선수 목록은 여기 없다. `MoveOptions` 가 그것을 실제로 둔 수·물러진 수와 한
          줄로 늘어놓는다 — 같은 국면의 같은 종류의 사실을 두 목록으로 따로 두면 견줄 수가 없다. */}
