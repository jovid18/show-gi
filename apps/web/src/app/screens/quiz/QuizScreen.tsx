 ...
 * 그 판에서 뽑은 문항을 푸는 화면.
 *
 * 문제도 정답도 이 판에서 나왔다. 문제집을 들여오지 않으므로 저작권도 없고, 「내가 놓친
 * 詰み」이라는 말이 성립한다.
 *
 * 채점은 서버가 한다. 정답이 화면에 오는 것은 답을 낸 뒤라, 판을 열어 보는 것으로는 답을 알 수
 * 없다.
 */
...
function Questions({
  ...
  /**
   * 문항 하나가 한 페이지다. 詰み이 그 판에서 가장 큰 사건이라 먼저 온다.
   */
  const pages = useMemo(() => {
  ...
  return (
    <section className="quiz" aria-label="この対局から出た問題">
      ...
      {/* 「아직 만드는 중」·「기다리기를 그만뒀다」·「문항이 없다」 셋을 구분해 말한다.
          만드는 데 수십 초가 걸리고, 그만둔 쪽에서 아는 것은 「정해진 동안 오지 않았다」
          뿐이라, 없다고 단정하면 둘 다에서 거짓이 된다. */}
      {!quiz.ready && gaveUp ? (
        ...
        // 이유를 말하지 않는다. 「詰み이 없었고 뚜렷한 한 수도 없었다」는 서버가 모른다.
        // 생성기가 없는 배포는 아무것도 찾아보지 않고 빈 행을 남기고(server/quiz_jobs.go),
        // 트리가 깨져 詰み 문항이 빠진 판도 여기로 온다(quiz.go get).
        <p className="quiz-status">この対局からは問題が作れませんでした。</p>
      ) : null}

      {pages.length > 0 && (
        <>
          {/* 한 번에 한 문항이다. 넷을 한 페이지에 늘어놓으면 판 넷이 세로로 쌓여 스크롤이
              생기고, 지금 푸는 문항이 화면 밖으로 나간다(회차 2 #9).

              넘어간 문항은 언마운트하지 않고 `hidden` 으로 덮어 둔다. 언마운트하면 채점
              결과와 시도 횟수가 함께 사라져, 돌아왔을 때 세 번 틀린 문항이 처음 열린 것처럼
              된다. `hidden` 은 접근성 트리에서도 빠지므로 낭독기가 읽지 않는다. */}
          {pages.map((page, i) => (
          ...
          <nav className="quiz-nav" aria-label="問題の切り替え">
            ...
            {/* 「어디쯤인가」는 한 자리에서만 말한다(되짚기의 이동 바와 같은 판단). */}
 ...
 * 詰み 문항.
 *
 * 王手만 빛난다(`legalMoves` 가 王手만 담아 온다). 攻方이 매 수 王手를 걸어야 한다는 규약을
 * 화면이 판으로 가르치는 자리다. 글로만 적어 두면 초심자는 그것을 모른 채 오답을 받는다.
 */
function MateQuestion({ id, item }: { id: number; item: MateItem }) {
  const [grading, grade, clear] = useMateGrader(id);
  // 내가 낸 수만 갖고 있다. 玉方의 응수는 서버가 트리에서 꺼내 두므로 화면이 기억하면 두
  // 벌이 된다(protocol/quiz.ts).
  const [mine, setMine] = useState<string[]>([]);
  /**
   * 이 문항을 몇 번 틀렸나. 「最初から」로 지워지지 않는다. 다시 푸는 길이 그 버튼 하나뿐이라,
   * 지우면 세 번째 힌트에 영영 닿을 수 없다.
   */
  ...
  // 지금까지 진행된 국면이고, 문항 쪽으로 되돌리면 맞힌 수가 사라진 것처럼 보인다
  // (quiz.GradeMate 가 그래서 그 경우에도 둘 수 있는 수를 준다).
  ...
  const play = (usi: string): void => {
    const next = [...mine, usi];
    // 시도 횟수는 틀린 횟수다. 王手가 아닌 수는 안내로 처리해 세지 않는다(서버가 그 자리에서
    // 정답을 주지 않는 것과 같은 판단, quiz.MateNotCheck).
  ...
  return (
    <article className="quiz-item" aria-label="詰みの問題">
      <header className="quiz-item-head">
        ...
        {/* 놓친 詰み과 決めた 詰み은 사람에게 전혀 다른 이야기다. 같은 문장으로 두면 이미
            이긴 사람에게 「놓쳤다」고 말하게 된다. */}
      ...
      <QuizBoard
        ...
        // 판을 지금 모양으로 만든 수를 짚는다. 오답이면 방금 낸 그 수이고 정답이면 玉方의
        // 응수다. 문장이 짚는 것과 같은 수라야 둘이 서로를 가리킨다.
 ...
 * 「この局面の最善手は?」 문항.
 *
 * 첫 수만 받는다. 그 뒤를 이어 두게 하면 「그 수순이 좋은가」를 묻는 다른 문항이 되고, 그것은
 * 판정에 엔진이 다시 필요하다(journal §53).
 */
function BestQuestion({ id, item }: { id: number; item: BestItem }) {
   ...
   * 맞혔는가. cp 표가 여기 걸린다.
   *
   * 맞혔을 때만 그 자리를 만든다. 채점 결과가 있다는 것만 보면 오답 문구에서 정답을 지워도
   * 표가 정답과 두 cp를 그대로 적는다(회차 2 #10 · #11). 서버도 오답에는 그 값들을 보내지
   * 않는다.
   */
  const solved = res?.correct === true;
  /**
   * 최선 수순에서 지금 보고 있는 자리. null 이면 정답을 둔 국면이다.
   *
   * 판 하나를 그대로 쓴다. 수순을 따로 그리는 작은 판을 하나 더 두면 같은 국면이 화면에 둘 뜨고
   * 어느 쪽이 지금인지가 없어진다(journal §14 와 같은 판단).
   */
  ...
  // 낸 수를 판에서 보여준다. 서버가 그 수를 둔 뒤의 국면을 주므로 화면이 수를 두지 않는다.
  // 만들지 못했으면(`sfen` 이 빈 값) 문제 국면 그대로다.
  //
  // 문장만으로는 부족했던 자리다. 정답과 打 한 글자로만 갈리는 수를 낸 사람은 무엇이 등록됐는지
  // 확인할 길이 없었다(회차 1 #17 · #18). 출발 칸이 빛나면 반상 이동이고 빛나지 않으면
  // 持ち駒에서 온 수다.
  ...
  return (
    <article className="quiz-item" aria-label="最善手の問題">
      ...
      {/* 판은 낸 수 뒤의 국면에서 멈춘다. 빛나는 칸은 문제 국면의 합법수라, 그 국면을 그린
          채로 다시 두게 하면 판과 빛이 어긋난다. 다시 푸는 길은 아래 「もう一度」이고 그것이
          판을 문제 국면으로 되돌린다. */}
      <QuizBoard
      ...
      {/* 회차 2 #12. 정답만으로는 왜 최선인지가 보이지 않았다(「その手が捨てる手に見える」).
          누르면 판이 그 자리로 가고, 取り返す·逃げる가 거기서 눈에 들어온다.

          추가 탐색이 0이다. 문항을 만들 때 이미 손에 있던 수순을 잘라 둔 것이고, 옛 판에는 그
          칸이 없어서 이 줄이 통째로 뜨지 않는다. */}
      {line.length > 0 && (
 ...
 *
 * 문항은 늘 사람이 둘 차례인 국면에서 뽑히므로(quiz.bestItems·mateItem) 그 手番이 곧 사람이다.
 * 진행된 판은 답한 뒤 상대 차례가 되므로 문항의 국면을 본다.
 */
...
/** cp 는 부호를 붙여 적는다. 0과 「없다」가 구별되어야 하고, 부호가 곧 누가 좋은가다. */
 ...
 * 채점 결과 한 줄.
 *
 * 자리를 지킨다. 문장이 있을 때만 그리면 답할 때마다 아래 것들이 밀려 올라가, 방금 무엇이
 * 바뀌었는지를 눈이 따라가지 못한다(WhatIfPanel 과 같은 자리).
 */
 ...
 * 문항 하나의 판.
 *
 * 되짚기와 같은 `Board`·`Hand` 다. 빛나는 칸은 서버가 준 `legalMoves` 가 정하고, 詰み
 * 문항에서는 그 배열이 王手만 담고 있다.
 */
function QuizBoard({
  sfen,
  me,
   ...
   * 「▲3五金打」가 한 글자 차이라 문장만으로는 갈리지 않았다(회차 1 #17).
   */
  ...
  // 판이 갈리면 고른 駒를 놓는다. 「最初から」·「もう一度」는 위에서 상태를 되돌리는데 이
  // 컴포넌트는 그대로 살아 있어, 고른 칸이 새 국면에서는 출발점이 아닌 채로 빛난다.
  ...
  const onSquare = (usi: string): void => {
    ...
    if (origin && lit.has(usi)) {
      ...
      if (!dest) return;
      // 성·불성이 둘 다 되면 물어본다. 한쪽만 되면 물어볼 것이 없어 그것으로 둔다.
