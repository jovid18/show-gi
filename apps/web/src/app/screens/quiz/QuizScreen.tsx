 ...
 * 그 판에서 뽑은 문항을 푸는 화면.
 *
 * 문제도 정답도 이 판에서 나왔다. 문제집은 들여오지 않는다 — 자기 기보에서 만들면
 * 저작권도 없고, 「내가 놓친 詰み」이라는 말이 성립한다.
 *
  ...
  return (
    <section className="quiz" aria-label="この対局から出た問題">
      ...
      {!quiz.ready && gaveUp ? (
        ...
        // 사실이다 — 생성기가 없는 배포는 아무것도 찾아보지 않고 빈 행을 남기고(server/quiz_jobs.go), 트리가
  ...
  return (
    <article className="quiz-item" aria-label="詰みの問題">
      ...
      <QuizBoard
        ...
        // 판을 지금 모양으로 만든 수를 짚는다. 오답이면 방금 낸 그 수이고, 정답이면
...
function BestQuestion({ id, item }: { id: number; item: BestItem }) {
   ...
   *
   * 판 하나를 그대로 쓴다. 수순을 따로 그리는 작은 판을 하나 더 두면 같은 국면이 화면에 둘
   * 뜨고, 어느 쪽이 지금인지가 없어진다 — 개입 카드가 수순을 판 위에서 재생하는 것과
   * 같은 판단이다(§14).
   */
