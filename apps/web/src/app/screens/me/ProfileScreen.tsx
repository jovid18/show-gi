...
/**
 * 여러 대국의 기록을 모아 보여주는 마이페이지다.
 *
 * 段級·전적·崩れやすいところ·組んだ形을 표시한다. 개별 대국의 상세 정보는 되짚기에서 보여준다.
 *
 * 약점만 강조하지 않도록 組んだ形에는 플레이어가 완성한 형태를 보여준다(journal §77).
 *
          ...
          <ul className="profile__weak">
            {profile.weaknesses.map((w) => (
              <li key={w.code}>
                ...
                {/* 막대는 비율이고 숫자는 횟수다. 둘 다 두는 것은 「12回」가 많은지
                    적은지를 횟수만으로는 읽을 수 없기 때문이다. */}
