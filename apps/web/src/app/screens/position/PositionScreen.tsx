 ...
 *
 * **확인 걸음이 없으면 이 기능이 거짓말이 된다.** 룰 엔진은 「그런 판은 없다」까지만
 * 잡는다(二歩·玉 수·行き所のない駒·王手放置). 銀을 成銀으로 잘못 읽은 판은 여전히 합법적인
 * 국면이라 아무 코드도 걸리지 않고, 그 자리의 검증자는 사람뿐이다 — 그래서 올린 그림을 판 옆에 그대로
 * 띄운다. 두 개를 함께 놓지 않으면 사람이 무엇과 대조해야 하는지를 알 수 없다.
 *
 * **手番을 묻는다.** 사진은 그것을 말해 주지 않는다. 그리고 「先手か」 대신
...
export function PositionScreen({ me }: { me: MeResponse }) {
  ...
  // 보이지만 주소를 직접 열면 익명으로 여기 들어온다 — 그때 상자를 그려 주면 사람이 그림을
  // 고르고 누른 뒤에야 로그인이 필요하다는 것을 알게 된다(ImportScreen 과 같은 자리).
...
function PositionForm() {
  ...
  /**
   * 판이 바뀌면 성립하는지를 다시 묻는다. 엔진도 로그인도 쓰지 않는 자리라 한 칸을
   * 고칠 때마다 물어도 되고, 그래서 二歩가 되는 순간 그 자리에서 보인다.
   *
   * 늦게 온 답을 쓰지 않는다. 이어서 두 칸을 고치면 요청 둘이 겹치는데, 먼저 보낸 것이
   * 나중에 오면 화면이 한 걸음 전의 판정을 그린다 — 그러면 사람이 고친 것이 고쳐지지
   * 않은 것처럼 보인다.
   */
  ...
  useEffect(() => {
    ...
    void checkPosition(sfen, controller.signal)
      ...
      .catch(() => {
        // 검사가 돌지 못해도 판은 그린다. 여기서 사유를 비우면 「문제가 없다」로 읽혀서
   ...
   *
   * `navigator.clipboard.read()` 대신 `paste` 이벤트다. 저쪽은 Chromium 전용이고
   ...
   *
   * `dragover` 에서 기본 동작을 막아야 한다. 막지 않으면 브라우저가 그 그림을 탭에서
   * 전부 열어 버려서, 사람이 만든 판이 그 자리에서 사라진다.
   */
   ...
   * 결과를 갖고 있고, 그때 열려 있으면 성립하지 않는 판이 넘어간다.
   */
    ...
    >
      ...
      <p className="import__lead">
        ...
        {/* 실측에서 판이 화면의 절반인 방송 캡처가 크게 틀렸다(journal §129). 자르면
            나아지는지는 아직 재지 않았지만, 사람에게 시키는 값이 작아서 먼저 권한다. */}
      ...
      {image !== null && board !== null && (
        <>
          <div className="position__compare">
            {/* 올린 그림을 판 옆에 그대로 둔다. 룰 엔진이 잡지 못하는 오독(銀↔成銀·駒の向き)의
