// SFEN 을 화면에 그릴 수 있는 모양으로 푼다.
//
// 여기서 규칙을 판단하지 않는다. 합법수는 서버가 `legalMoves` 로 주고, 이 파일은 「지금 판에
// 무엇이 어디 있나」만 읽는다. 두 벌이 생기면 어긋났을 때 어느 쪽이 맞는지 모른다.
 ...
 *
 * 이 파일에서 오직 「글자를 만드는」 자리다. 사진에서 읽어 온 판을 사람이 한 칸씩 고치는
 * 화면이 필요로 하고(journal §129), 고친 판이 주소가 되어 서버로 간다.
 *
 * 여기서도 규칙을 판단하지 않는다. 二歩든 玉이 둘이든 그대로 적는다. 걸러 버리면 사람이
 * 고치는 중간 상태를 화면이 그릴 수 없다.
 *
