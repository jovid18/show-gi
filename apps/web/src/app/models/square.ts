// 쇼기 좌표 변환. 판은 9×9 이고 두 축의 이름이 다르다.
//   筋(すじ, file) — 세로줄. 오른쪽부터 1~9
//   段(だん, rank) — 가로줄. 위부터 1~9 (사람이 읽을 때는 一~九)
//
// 사람이 읽는 표기는 「7六」이고 USI 는 `7f` 다. 段을 a~i 로 적는다. ▲7六歩가 `7g7f` 다.
//
...
/**
 * 한 수가 지나간 두 자리(화면 배열 인덱스). 도착 칸의 駒를 출발 칸에서 끌어오는 데 쓴다.
 *
 * `components/Board` 대신 여기 있는 것은 칸 인덱스 두 개가 전부이기 때문이다. 판에 있으면
 * `libs` 가 화면 부품을 참조하게 되고, 그러면 층의 방향이 거꾸로 선다.
 */
export interface Motion {
  /** 도착 칸. */
  to: number;
  /** 출발 칸. 打이면 null 이고, 위에서 떨어진다. */
  from: number | null;
  /**
   * 같은 칸에 두 번 들어와도 다시 나게 하는 열쇠. 되잡기가 그 경우다.
   *
   * 같은 칸에 연달아 들어오면 요소가 그대로 남아 CSS 애니메이션이 다시 시작하지 않는다.
   */
  id: number;
}
