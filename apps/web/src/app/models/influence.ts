...
//
// 합법수를 구하지 않는다. 여기서 나오는 것은 駒가 닿는 칸의 매수뿐이고, 자기 玉이 王手에
// 걸리는 수(pin)도 二歩도 打ち歩詰め도 보지 않는다. 그래서 이 값으로 착수를 통제하지
...
// 않는 것이 그 선이다. 두 벌이 위험한 것은 어긋났을 때 어느 쪽이 맞는지 모르기
 ...
 *
 * 쭉 가는 駒는 처음 만난 駒에서 멈추고, 그 칸까지는 센다. 그 칸은 잡을 수 있는 자리이고,
 * 그 너머는 세지 않는다. 사이에 낀 駒 너머를 세면(X-ray) 판이 없는 위협을 말한다.
 */
 ...
 * 보지 않는다 — 그늘은 「상대가 여기까지 손을 뻗고 있고 이쪽이 받지 않는다」까지다.
 * 「반드시 잡힌다」인지를 정하는 것은 엔진이다(docs/01-core.md).
 */
