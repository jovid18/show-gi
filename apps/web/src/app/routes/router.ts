// 외부 라우팅 라이브러리 없이 주소별 화면을 선택한다.
//
// 대국 화면을 언마운트하면 WebSocket이 끊겨 대국이 `abandoned`로 종료된다(App.tsx).
// 라우팅 방식과 무관하게 대국은 라우트 밖에 마운트한 상태로 유지하고 숨기기만 한다.
//
// 딥링크는 Caddy의 `try_files {path} /index.html`과 vite dev의 동일한 폴백으로 처리한다.
...
export type Route =
  ...
  | { name: 'game' }
  | { name: 'reviews' }
  // ply 는 열 때의 手数다. 없으면 0手目(시작 국면)에서 연다.
  ...
  // 대인전 방의 문자열 id다. 방 번호를 유추하기 어렵도록 난수를 사용한다(routeRoom).
  | { name: 'room'; id: string };
 ...
 *
 * 여기서는 SFEN 형식만 확인한다. 국면의 합법성은 서버의 룰 엔진이 검증한다.
 */
