...
describe('parseRoute', () => {
  ...
  // 퀴즈가 아닌 꼬리는 그 판이다. 목록으로 튕기면 링크를 잘못 복사한 사람이 판을 잃는다.
  it('모르는 꼬리는 그 판이다', () => {
    expect(parseRoute('/reviews/12/nope')).toEqual({ name: 'review', id: 12 });
  });

  // id 검사가 꼬리보다 먼저다. 그러지 않으면 읽을 수 없는 id 로 퀴즈 요청이 나간다.
  ...
  // 마이페이지에는 판이 없으므로 꼬리도 없다. 읽을 수 없는 꼬리는 마이페이지 자신이다.
  ...
  // 가져오기도 꼬리를 보지 않는다. 붙여 넣은 글이 화면 안에만 있다.
  it('가져오기', () => {
  ...
  // 방 id 는 영숫자 8자다(서버의 NewRoomID). 아무 문자열이나 통과시키면 그 값이 그대로
  // `fetch` 의 경로가 된다.
  it('방', () => {
    const id = 'AbCdEf12';
...
/** 뿌리 국면. 「성립하는 판인가」는 보지 않는다. 그 판단의 정본은 서버의 룰 엔진 하나다. */
describe('뿌리 국면', () => {
  it('판이 실리면 그것이 뿌리다', () => {
  ...
  // 성한 駒는 `+P` 로 두 글자다. 40장이 전부 성하고 빈 칸이 잘게 흩어지면 판 칸이 123자가
  // 되고, 상한이 90이던 동안 그런 판은 경고 없이 버려져 平手가 열렸다.
  it('판 칸이 긴 국면도 그대로 싣는다', () => {
    // 여기서 보는 것은 길이 관문이고, 룰은 서버가 본다.
...
// 주소 조각은 서버가 준 값이다. 타입이 number 라도 JSON 은 그것을 보장하지 않는다. 막지
// 않으면 프로토콜 상대 주소가 만들어져 링크가 밖으로 나간다(CodeQL 의
