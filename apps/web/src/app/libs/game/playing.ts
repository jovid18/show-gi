/**
 * 지금 두는 중인가. **대국 화면이 쓰고 앱 셸이 읽는다**(App.tsx) — 그 값 하나가 다른
 * 주소를 판으로 되돌리고, 헤더에서 누를 것을 없애고, 홈 메뉴의 검토 줄을 감춘다
 * (journal §86).
 *
 * 이 한 값을 위해 가게를 따로 두는 이유가 소유권이다 — 판의 상태는 `useGame` 이 들고
 * 있고 그건 WebSocket 하나에 매여 있어서, 셸이 알려고 그 훅을 한 번 더 부르면
 * **연결이 둘이 되고 판도 둘이 된다.** 위로 끌어올리는 것도 답이 아니다: 대국 화면이
 * 라우트 밖에 상시 마운트인 이유가 그 연결이라(App.tsx), 그 훅은 거기 있어야 한다.
 *
 * 그래서 **한 방향으로만 흐른다** — 대국 화면이 알리고, 셸이 듣는다. 반대로 쓰는 쪽이
 ...
 * 매 手마다 스냅샷이 오는데 그때마다 셸을 다시 그릴 이유가 없다.
 */
let playing = false;
