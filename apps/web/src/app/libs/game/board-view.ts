// 대국 화면이 판에 넘길 값을 만드는 순수 계산들. React 가 없어서 국면 하나를 넣고 결과를
// 견주는 테스트가 그대로 된다. 규칙 판단도 없다. 합법수도 국면도 서버가 준다.
...
/**
 * 직전 수가 지나간 두 칸. 도착만 짚으면 「저기 뭔가 있다」까지이고, 초심자가 알아야 하는
 * 것은 무엇이 어디서 왔나다.
 */
...
/** 王手를 거는 줄. 서버가 준 두 칸을 옮길 뿐이고, 누가 王手인지는 계산하지 않는다. */
 ...
 * 놓이는 자리는 변형 전의 배치 좌표다. 판이 어떤 변형을 받아도 이 계산은 그대로 맞는다.
 */
...
export function resultText(snapshot: Snapshot): string | null {
  ...
  switch (snapshot.status) {
    ...
    case 'repetition':
      return '千日手。引き分けです。';
    // 승패를 말하지 않는다. 상대의 수를 구하지 못해 접은 판이다. 「이어할 수 있다」도 붙이지
    // 않는다. 이어하기는 로그인한 사람만이라(서버의 resumeSetup) 익명 대국에서는 거짓이 된다.
    case 'aborted':
