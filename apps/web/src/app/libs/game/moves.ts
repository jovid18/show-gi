...
//
// 규칙을 판단하지 않는다. 목록에 있으면 둘 수 있고 없으면 둘 수 없다. 二歩도 打ち歩詰め도
// 애초에 목록에 들어오지 않는다.
...
/** USI 한 수를 풀어 놓은 것. 문자열 해석뿐이라 둘 수 있는 수인지는 알 수 없다. */
 ...
 * 합법수를 출발점별로 묶는다.
 *
 * 같은 (출발, 도착)이 성/불성으로 두 번 올 수 있어서 한 항목에 두 갈래를 함께 담는다. 어느
 * 쪽이 가능한지가 곧 「成りますか」를 물을지 말지다.
 */
 ...
 * 화면 부품을 참조하면 층의 방향이 거꾸로 선다(`models/square` 의 `Motion` 과 같다).
 */
export interface MoveSquares {
  /** 출발 칸. 打이면 null 이고, 짚을 자리가 판 위에 없다. */
  from: number | null;
  to: number;
}

/**
 * USI 한 수를 판 위의 두 자리로 옮긴다. 판이 어느 수로 이 모양이 되었는지를 짚는 값이다.
 *
 * 되짚기와 퀴즈가 같이 쓴다. 퀴즈에서는 이것이 「▲3五金」과 「▲3五金打」를 눈으로 가르는
 * 하나뿐인 자리다(회차 1 #17·#18). 출발 칸이 빛나면 반상 이동이고, 아니면 持ち駒에서 온 수다.
 *
 * 읽을 수 없거나 좌표가 판을 벗어나면 null.
 */
