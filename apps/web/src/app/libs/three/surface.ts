...
// 쇼기 駒는 전부 같은 오각형이고 한자로만 갈리므로, 원근을 넣으면 뒷줄이 뭉개진다.
//
// 「시점이 움직였다」는 신호는 개입 때 `.game-board` 가 CSS 로 판째 기울여서 이미 준다.
// 여기서 카메라를 또 기울이면 DOM 駒와 이 표면이 어긋난다 — 그래서 이 캔버스는 칸과
 ...
 * 그늘이 밀려드는 모양.
 *
 * 개입(회상)에서는 물러진 수가 간 칸에서 퍼져 나간다 — 「네가 두려던 그 자리가
...
const FRAGMENT = /* glsl */ `
  ...
  void main() {
    ...
    // ── 그늘 ──────────────────────────────────────────────
    // 두 번 읽는다. 칸 값은 「이 칸이 몇 매인가」라서 또렷해야 하고,
    // 번진 값은 칸 사이를 메워 그늘이 판에 드리운 한 겹으로 보이게 한다.
