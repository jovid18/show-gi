...
//
// 카메라는 정사영이고 정면이다(journal §35). 기울이면 이 캔버스 위에 얹힌 DOM 駒·빛·광선이
// 표면과 어긋난다. 그래서 이 캔버스는 칸과 픽셀이 맞는 평면이다.
//
// three.js 를 쓰는 값은 利き을 스칼라 필드로 합성하는 것 하나다. 9×9 매수를 텍스처로 올리면
// GPU 가 칸 사이를 메워 그늘이 판에 드리운 한 겹으로 읽힌다. DOM 으로는 칸마다 상자를 81개
// 겹치는 것 말고 방법이 없다.
 ...
 * 그늘이 밀려드는 모양.
 *
 * 개입(회상)에서는 물러진 수가 간 칸에서 퍼져 나가고, 평시 토글에서는 판 한가운데다.
 */
...
// 색 계산을 감마 공간에서 그대로 한다. 옆에 붙은 것이 전부 CSS 로 칠한 면이고 CSS 는 sRGB
// 값끼리 섞으므로, 여기서만 선형으로 옮기면 같은 --board 가 두 가지 색으로 보인다.
const FRAGMENT = /* glsl */ `
  ...
  // 榧 판은 柾目라 결이 세로로 곧게 간다. 그래서 y를 눌러 늘인다.
  ...
  void main() {
    ...
    // 칸 안인가, 칸 사이의 선인가. 판의 안쪽 여백도 선과 같은 색이다. 거기서 local 이
    // 격자 밖으로 나가므로 clamp 뒤에 inside 가 칸을 벗어난다.
    ...
    // 판이 빛을 받는다. 이 한 겹이 없으면 셰이더로 그린 판이 색종이처럼 평평해 보인다.
    vec2 n = p / uSize;
    ...
    // ── 그늘 ──────────────────────────────────────────────
    // 두 번 읽는다. 칸 값은 「이 칸이 몇 매인가」라 또렷해야 하고, 번진 값은 칸 사이를 메운다.
    ...
    // 텍스처가 0~1 로 돌려주므로 매수로 되돌린다. 깊이 곡선이 매수 위에 놓여 있어야 이
    // 줄만 고쳐서 세기를 조절할 수 있다.
    ...
    // 매수가 늘수록 깊어지되 포화한다. 3매와 4매가 1매와 2매만큼 벌어지면 판에서 제일
    // 중요한 「닿는가 닿지 않는가」가 깊은 쪽에 묻힌다.
 ...
 *
 * 부르지 않으면 아무것도 그리지 않는다. 상시 rAF 루프를 두면 판이 가만히 있는 대부분의
 * 시간에 모바일 배터리만 먹는다.
 */
export class BoardSurface {
  ...
  constructor(canvas: HTMLCanvasElement, palette: Palette) {
    ...
    // 가장자리를 물고 늘어지게 둔다. 반복이면 판 왼쪽 끝의 그늘이 오른쪽 끝에서 번져 나와
    // 아무 일도 없는 칸에 상대의 利き이 있는 것처럼 보인다.
    ...
    this.uniforms = {
      ...
      uAmount: { value: 0 },
      // 부르는 쪽이 정한다(`useBoardSurface`). 세기가 판의 밝기에 매여 있는데, 회상 중에
      // 낮아져 있느냐를 여기서는 알 수 없다.
      uDepth: { value: 0 },
   ...
   *
   * 0~1 로 눌러 두지 않는다. 그러면 「몇 매인가」가 이 줄에서 사라져, 깊이 곡선을 고치는 데
   * 두 파일을 같이 고쳐야 한다.
   */
  ...
  dispose(): void {
    ...
    // 컨텍스트를 놓지 않으면 판이 몇 번 사라졌다 다시 뜬 뒤 브라우저가 가장 오래된 WebGL
    // 컨텍스트를 죽이고, 그때 판이 검게 남는다.
    this.renderer.dispose();
    this.renderer.forceContextLoss();
  }
}

/** `.board` 에서 색을 읽는다. 팔레트의 정본은 CSS 변수이고 셰이더가 그것을 따라온다. */
