...
describe('scoreJa', () => {
  ...
  // cp 가 같이 와도 手数가 이긴다. 서버는 둘을 같이 보내지 않지만, 그 규약이 깨지는
...
describe('playerCp', () => {
  // 파랑·빨강은 어디서나 「나에게 좋은가」다. 상대 차례의 국면에서 뒤집지 않으면 색 전체가
  ...
  // 詰み은 얹을 cp 자체가 없다. 규약이 깨져 cp 가 같이 오더라도 그 줄은 말로만 남는다.
  it('詰み은 색으로 말하지 않는다', () => {
