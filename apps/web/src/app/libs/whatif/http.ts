...
/**
 * 끝난 판의 가정 수순은 요청/응답이다. 대국 중에는 같은 것을 그 대국의 WebSocket 으로
 * 묻는다(useGame).
 *
 * 갈리는 것은 뿌리를 어디서 얻느냐다. 여기는 DB 기록이고, 기록은 비동기로 쌓이므로 개입
 * 직후에는 마지막 수가 아직 없을 수 있다.
 */
export function httpSend(gameId: number): Send {
  return async (req: WhatIfRequest, signal: AbortSignal): Promise<WhatIfNode> => {
    ...
    if (!res.ok) {
      // 서버가 이유를 일본어로 준다(whatif.go). 읽지 못할 때만 우리 문구를 쓴다.
