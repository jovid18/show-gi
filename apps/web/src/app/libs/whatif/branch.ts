 ...
 * 뜻하는 것(힌트·王手)과 자리가 겹치지 않는다. 대신 판에 쓰는 토큰을 그대로 쓴다.
 *
 ...
 * 빼지 않으면 六枚落ち(+2003)에서 모든 후보가 ±800 을 넘어 방금 물러진 수까지 최대 파랑이
 * 된다 — 판정과 형세 그래프가 같은 값을 빼는 것과 같은 판단이다(journal §84).
 */
const TONE_FULL = 800;
 ...
 *
 * 詰み의 手数만 플레이어 관점으로 옮긴다. 手数는 세는 값이라 관점을 바꿔도 자가 갈리지 않고,
