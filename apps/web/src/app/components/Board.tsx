...
export interface Ray {
   ...
   * 색을 가르는 것은 가리키는 쪽이 반대이기 때문이다. 저 둘은 상대가 무엇을 하는가,
...
interface BoardProps {
   ...
   * 打 화살표의 출발점은 변형 전의 배치 좌표를 재고 있고(GameScreen 의 `measureDrop`),
...
/**
 * 물러진 수가 어느 駒였는지를 짚는 유령 駒.
 *
 * 자리는 칸 수로 준다(`--col`·`--row`). 픽셀로 계산하면 `--sq` 가 화면 폭에 따라
 * 변하는 만큼 어긋난다.
 *
 * 격자에 얹지 않고 띄운다. 자리를 `grid-column`으로 지정하면 그 칸이 먼저 잡히고
 * 81칸이 그것을 피해 한 칸씩 밀린다 — 명시 배치는 DOM 순서보다 먼저 처리된다.
 *
 * 가려던 칸에 선다. 끌고 오지 않는다 — 그 이동은 두 칸의 테(`.blunder-mark`)와 화살표가
 * 이미 말한다. 이 유령이 혼자 말하는 것은 어느 駒였나 하나이고, 그것은 서 있기만 해도
 * 말한다. 조용해야 하는 판에서 움직이는 것을 하나 줄인다(journal §69).
 */
