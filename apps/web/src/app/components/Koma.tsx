 ...
 *
 * 성한 駒는 붉은 글자이고, 움직임이 표식으로 새겨져 있다 — 한 칸 가는 곳은 점,
 * 쭉 가는 곳은 화살표, 뛰어넘는 곳은 점선. 실물 초심자용 駒가 하는 것과 같고,
 * 「香車가 어떻게 가더라」를 외우지 않아도 되게 하는 것이 이 앱이 하려는 일이다.
 *
 * 표식은 駒와 함께 돈다 — 後手 駒는 전부 180° 뒤집히므로 화살표도 상대 쪽을 가리킨다.
 */
 ...
 * 판의 한 칸에 그대로 대응한다 — 玉과 銀을 함께 놓으면 그 차이가 바로 보인다.
 *
 ...
 *
 * 두 칸 앞의 좌우는 격자 밖이다. `ne`·`nw` 자리에 점을 찍으면 대각선 한 칸으로 읽혀
 * 銀과 같은 그림이 되고, 그건 규칙을 틀리게 가르치는 것이다.
 *
 * 도착 칸은 다른 駒와 똑같은 점이고, 격자보다 바깥에 놓일 뿐이다. 다른 것은 길이 함께
 * 그려진다는 점 — 잔 점이 駒에서 그 점까지 이어지고 굵기가 도착점의 1/3이라 「여기가
 * 도착, 저건 지나가는 길」로 읽힌다. 길이 끊겨 있으면 사이 칸을 밟지 않는다.
 *
 * 줄기(`STEM`)는 갈래까지 한 번만 그어 Y 가 된다. 두 길을 각자 그으면 V 가 되고, 표식
 * 둘이 우연히 같은 곳을 향하는 것처럼 보인다.
 *
 * 자리는 재서 잡았다. 줄기의 시작은 글자 상자가 끝나는 곳(viewBox 30)이고 — 더 내리면
 * 글자가 첫 점을 덮는다 — 도착점은 오각형 안에 반지름 5가 온전히 들어가는 한계 근처다.
 *
 * 이 자를 대려면 letterbox 를 빼먹으면 안 된다. 駒는 0.8×0.84 라 세로가 길고 SVG 가
 * `meet` 로 맞추므로, `clip-path` 의 %가 가리키는 오각형 어깨는 viewBox 자로 y=13.3 이다.
 */
 ...
 * 手数를 넘기는 데 80ms가 든다 — 판이 즉시 바뀌지 않는 것으로 보인다.
 *
