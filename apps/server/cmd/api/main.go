...
// 엔진 풀 크기의 기본값.
//
// 빌리는 자리가 다섯이다 — ① 상대 수 ② 개입 판정 ③ 가정 수순 ④ 手筋 힌트
...
func main() {
	...
	if pool := startEngines(); pool != nil {
		...
		// 플레이어가 실제로 그 수를 두면 바로 다시 물어볼 국면이라, 캐시 적중이
...
func startMateEngines() *usi.Pool {
	...
	// 대국 중에는 매 수 두 번이지만(종반 판정과 詰み 게이지) 그 둘이 시간상 겹치지
	...
	// 빌리고 돌려주므로(Pool.Do) 굶는 것이 아니라 줄을 서는 것이고, 대국 쪽은 지연으로
