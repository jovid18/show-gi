...
// (defaultMatePoolSize).
//
// 3으로 그 여섯을 다 덮지는 못한다. 퀴즈가 한 번에 십여 초를 쓰므로(journal §53) 그동안
// 진행 중인 대국의 착수가 그만큼 뒤로 밀린다. 그래도 올리지 않은 것은 그 지연이 대국을
// 멈추지 않기 때문이다(mate 풀과 달리 여기는 원래도 여럿이 다툰다). 퀴즈가 동시에 도는
// 수는 하나로 묶여 있다(server 의 quizSlots).
...
func main() {
	...
	if pool := startEngines(); pool != nil {
		...
		// 게이지는 상대의 수 직후) 퀴즈는 한 번에 수십 초를 잡는다 — 그래서 하나로는
		// 모자라다(matePoolSize). 퀴즈가 동시에 도는 수는 하나로 묶여 있다(quizSlots).
		opts.Mate = mate
...
func startMateEngines() *usi.Pool {
	...
	// 세 번째가 한 번에 수십 초 동안 풀을 잡는다 — 그래서 기본이 2다(journal §53).
	// 넷째는 게이트 없이 手마다 부른다(journal §110).
	//
