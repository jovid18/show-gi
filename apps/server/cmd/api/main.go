...
// 엔진 풀 크기의 기본값. 빌리는 여섯 자리는 archive 패키지 주석에 있다. 詰み 탐색은
// 다른 바이너리라 풀이 따로다(defaultMatePoolSize).
//
// 3으로 그 여섯을 다 덮지는 못한다. 퀴즈가 한 번에 십여 초를 쓰는 동안 진행 중인 대국의
// 착수가 그만큼 뒤로 밀린다(journal §53). 그래도 그 지연이 대국을 멈추지는 않는다.
//
// 프로덕션은 2다. 코어가 2개뿐이고, 슬롯만 늘리면 대기가 탐색 시간으로 옮겨갈 뿐인 것을
// 4로 재 봤다(journal §110). 올릴 자리는 태스크 정의의 ENGINE_POOL_SIZE 다.
...
// defaultMatePlies 는 詰み 탐색의 手数 한계다. 11인 근거는 01-core.md §2. 비용이 7·9·11
// 에서 평평해서 11이 詰み을 하나 더 찾고도 값이 같다.
...
func main() {
	...
	// 실력 추정이 붙기 전까지는 제일 너그러운 쪽이 기본이다(intervene.Beginner 주석).
	//
	// 판정과 기록이 같은 값을 봐야 한다. 갈리면 「어느 임계치에서 걸린 개입인가」가
	// 기록에서 틀리고, 그 위에서 상수를 흔들어 보게 된다.
	opts := server.Options{Level: intervene.Beginner}

	// 티어를 여기서 한 번 읽는다. 두 자리에서 각각 os.Getenv 하면 잘못 적은 값이 한쪽에서만
	// 경고를 남기고 다른 쪽은 경고 없이 갈린다.
	role := analysisRole()
	opts.Role = role

	// 서버·엔진 풀·탐색이 같은 레지스트리를 쓴다. 무엇을 재는지가 metrics.New 한 자리에
	// 다 있어야 새 지표를 늘릴 때 EMF 쪽을 같이 보게 된다.
	...
	// 가져온 기보의 정규화 창구. 키가 없으면 nil 이고, 그때는 결정적 파서로 읽히는 기보만
	// 들어온다. 표면이 닫히는 것이 아니라 폴백 한 겹만 꺼진다(internal/kifunorm).
	...
	// 판독을 재는 그림과 라벨을 모아 두는 폴더. 로컬에서만 켠다. 값이 없으면 그림도 남지
	// 않고 라벨 경로도 열리지 않는다(apps/server/README.md).
	...
	// 대인전. 엔진이 없어도 사람끼리는 둘 수 있으므로 엔진 앞에 둔다.
	//
	// ctx 는 여기서 준다. 대국은 연결이 끊겨도 이 프로세스만큼 산다. 핸들러의 r.Context()
	// 에 매달면 한쪽이 탭을 닫는 순간 시계까지 멈춰서, 남은 사람의 대국이 끝나지도
	// 이어지지도 못한다(server.NewMatch).
	...
	if pool := startEngines(); pool != nil {
		...
		// 모든 탐색이 데이터가 된다. 풀을 한 겹 감싸고 여섯 자리가 같은 하나를 받는다
		// (internal/archive).
		//
		...
		// 계측도 같은 자리에 붙는다. 캐시가 답한 것과 엔진을 부른 것이 여기서 갈린다.
		searcher.Observe(reg.Search())
		// 떠 있는 기록이 끝나기를 기다린다. 기다리지 않으면 마지막 수의 분석이 버려진다.
		// 등록 순서가 곧 종료 순서다(LIFO). 이 줄이 위 defer st.Close() 보다 뒤라서 기록이
		// 다 흘러간 뒤 DB가 닫힌다.
		defer searcher.Wait()

		// 詰み 탐색도 같은 겹을 지난다. 표를 따로 둔 이유는 017 의 DDL 에 있고, 여기서
		// 값을 하는 것은 빌리는 넷이 같은 국면을 겹쳐서 묻는다는 것이다(journal §110).
		//
		...
		// 이 풀을 빌리는 넷은 startMateEngines 에 있다.
		opts.Mate = mate

		// 가정 수순도 같은 풀이다(internal/server/whatif.go). 대국과 자리를 다투지만
		// 겹치면 풀이 순서대로 빌려주고, 그 지연은 여기서 허용된 비용이다.
		//
		// 검토(internal/server/explore.go)도 이 풀이다. 저쪽은 뿌리가 0手目라 아무 국면이나
		// 물을 수 있어서, 그 표면만 자기 슬롯을 하나 갖는다(exploreSlots, journal §85).
		opts.Search = searcher

		// 되짚기 퀴즈의 생성기. 詰み 문항은 solver, 「최선수는?」은 탐색부를 쓴다. 탐색부
		// 쪽을 감싼 것으로 넘겨야 그 결과도 positions 에 쌓인다(journal §37).
		//
		// mate 가 nil이면 「최선수는?」 문항만 나온다(Build 가 b.mate != nil 로 가른다).
		// searcher 가 없으면 이 자리 자체가 없다.
		opts.Quiz = quiz.NewBuilder(mate, searcher, engineDepth())

		// 사후 분석. 대인전은 되짚기의 평가치와 두 사람의 실력 추정치를 채우고(journal
		// §105), 가져온 기보는 거기에 悪手 줄과 문항까지 만든다(§126). 미리 재는 것이
		// 논블로킹이라 착수 경로는 그래도 엔진을 지나지 않는다.
		//
...
//
// 기본은 info 다. debug 는 헬스체크까지 들어와 로그의 대부분이 그것이 되고(server.levelFor),
// warn 은 아직 slog 로 옮기지 않은 log.Print* 전체를 지운다.
...
//
// ENVIRONMENT 가 비면 아무것도 내보내지 않는다. 로컬에서 EMF 줄이 stdout 에 섞이지 않고,
// 켠 줄도 모르는 채로 커스텀 지표가 쌓이지도 않는다. 그때도 /metrics 는 그대로 있다.
//
// 돌려주는 함수는 마지막 한 줄을 내보내고 돌아온다. main 이 끝나기 직전에 부른다. defer
// 로 두면 리스너 오류가 log.Fatal 로 끝날 때 그 defer 가 돌지 않는다.
...
//
// 기본이 풀 크기다. 분석은 대국과 같은 풀에서 엔진을 빌리는데, 다 가져가도 되는 것은
// 사람이 기다리는 요청을 풀이 먼저 빌려주기 때문이다(usi.priorityOf).
//
// 풀이 커지면 이 값도 같이 커져야 해서 상수가 아니다(journal §106).
...
// analysisRole 은 이 프로세스가 어느 티어인가다. 큐를 집는가와 사람을 받는가를 정하고,
// 손잡이는 SERVER_ROLE 이다.
//
...
//
// ROLE 로 두지 않는다. .github/workflows 가 IAM 역할 ARN 을 그 이름으로 갖고 있어서,
// 밖에서 들어온 값이 하필 위 셋 중 하나면 티어가 경고 없이 갈린다.
//
// 이 손잡이로 상호작용 티어를 여러 대로 올리지는 못한다. 방이 메모리에 있다(journal §98).
...
//
// 없다고 프로세스를 죽이지 않는다. 엔진·DB 와 같은 판단이고, 로그인이 바꾸는 것은 그
// 판이 누구 것으로 남느냐 하나다. 어느 쪽으로 돌고 있는지는 기동 로그가 말한다.
...
//
// DB가 없다고 프로세스를 죽이지 않는다. 국면 캐시가 없으면 매번 계산할 뿐이지 대국은
// 된다. 죽이면 ECS 재시작 루프로 사이트 전체가 내려간다.
...
//
// 탐색 한계는 手数(DepthLimit) 로 준다. 시간으로 자르지 않는 것은 다른 탐색과 같고,
// 11인 것은 실측 결과다(06-status.md).
func startMateEngines() *usi.Pool {
	...
	// 소비자가 넷이다. 종반 판정, 詰み 게이지, 되짚기 퀴즈의 詰み 트리, 대인전 사후 분석.
	// 앞 둘은 시간상 겹치지 않지만(판정은 사람의 수 직후, 게이지는 상대의 수 직후) 세 번째가
	// 한 번에 수십 초 동안 풀을 잡아서 기본이 2다(journal §53). 넷째는 게이트 없이 手마다
	// 부른다(journal §110).
	//
	// 「대국 쪽에 늘 한 자리가 남는다」는 성립하지 않는다. 퀴즈가 동시에 도는 수는 하나로
	// 묶여 있지만(server 의 quizSlots) 넷째가 남은 자리를 가져갈 수 있다. 탐색 하나마다
	// 빌리고 돌려주므로(Pool.Do) 큐에 서서 기다리고, 대국 쪽은 그것을 지연으로 겪는다.
...
// 달라서, 한 값으로 묶으면 어느 쪽 때문에 올렸는지 다음에 누구도 모른다.
...
//
// 읽는 자리가 둘이라 여기 하나로 모은다. 풀에 주는 DepthLimit 과 캐시가 견주는 값이고,
// 두 값이 갈리면 한계 9의 「詰み이 없다」를 한계 11의 답으로 쓴다(archive.Mate.lookup).
func matePlies() int {
...
//
// 엔진이 모르는 옵션은 경고 없이 무시되므로(광고된 것만 보낸다) 값이 틀린 채로 돌아도
// 깨지지 않는다. 엔진을 바꿀 때 같이 확인할 것.
...
//
// 상수를 실측으로 잡는 동안 흔들어 볼 손잡이라 환경변수로 뺐다. 값이 정해지면 기본값이
// 되고 여기는 남는다. 레이팅이 붙으면 플레이어마다 달라질 자리다.
...
// 줄인다(기본값은 game.DefaultDepth). 시간 상한을 걸어 중간에 자르지 않는다.
//
