...
// Run 은 every 주기로 EMF 를 출력하고 ctx 가 끝날 때까지 막힌다.
//
// 끝나면서 한 줄을 더 출력한다. 출력하지 않으면 종료 직전 주기 전체가 사라지고, 배포마다
...
// emit 은 한 줄을 쓰고 실패는 로그로 끝낸다. 지표를 내보내지 못했다고 대국을 멈추지 않는다.
...
func (e *Emitter) collect() []metric {
	...
	// 배열은 비어 있으면 아예 내보내지 않는다. 빈 배열은 0 관측으로 읽히지 않고, EMF 검증에
	// 걸려 줄 전체가 버려진다.
...
// delta 는 누적 카운터에서 지난 주기 이후의 증분을 내보낸다.
