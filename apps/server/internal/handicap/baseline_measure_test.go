...
//
// 깊이가 14다. 기준점은 대국 중에 다시 구하는 값이 아니라 표에 박히는 상수라, 우리가
// 신뢰하는 가장 깊은 값으로 잡는다 — 여기서 얕게 재면 그 오차가 판 전체의 판정 기준으로
// 굳는다.
//
// **이제 JudgeDepth 도 14다**(journal §130). 이 표와 화면 값이 서로 다른 자를 쓰고
// 있었던 것이 그 상수를 올린 근거이고, 그래서 여기 적혀 있던 「판정에 쓰는 얕은 깊이가
