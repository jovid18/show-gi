...
//
// 두 값을 한 표에 둔다. 국면만 주면 판정식이 포화 구간에 갇힌다 —
// 승률 낙폭은 우세 구간에서 압축되므로(01-core.md §2) 二枚落ち에서는 낙폭 0.25를 넘기는 데
// 989cp가 필요하다(平手는 635cp). 銀을 공짜로 주는 것이 약 1000cp이므로(01-core.md §2)
// 그것도 간신히 걸린다는 뜻이다. 기준점이 곧 그 국면의 「아직 아무것도 안 흘렸다」이고,
// 빼고 나면 駒落ち도 660cp 언저리에서 걸린다 — 실측과 그 결과는 journal §84 · §88.
//
...
//
// > 平手의 실측 초기 평가치는 +91이다. 그 값을 기준점으로 쓰지 않는 것은 지금까지의
...
type Handicap struct {
	...
	// (香落ち 270 → 97) b 로 적으면 판정이 一手만큼 다른 국면의 기준점 위에서 돈다.
	SFEN string
...
// (실측 +3014·+3890), 上手에 攻め駒가 하나도 없어 판이 개입이 아니라 詰め 연습이 된다 —
...
// BaselineCp 는 그 판의 「형세 0」이다. 下手(SFEN의 b 쪽) 관점 cp. 平手는 0이다.
