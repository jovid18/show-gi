...
#
# **라벨을 룰 엔진에 물어본다.** 성립하지 않는 판은 픽스처로 앉히지 않는다 — 틀린 라벨은
...
# 주소로 왔으면 `s=` 를 꺼낸다. 쿼리 순서를 가정하지 않는다 — `m=` 이 앞에 올 수 있다.
case $label in
*'?'*|*'&s='*|*'/explore'*)
  ...
  # percent 를 푼다. `+` 를 공백으로 읽지 않는다 — 폼 인코딩이 아니라
  # 주소이고, `2b3c+` 의 `+` 는 成을 뜻한다(routes/const.ts 와 같은 이유).
  label=$(printf '%b' "${label//%/\\x}")
  ;;
esac

# 룰 엔진에 물어본다. 사유가 하나라도 있으면 픽스처로 넣지 않는다.
check=$(curl -sS --max-time 10 -X POST "$api/api/position/check" \
  ...
  echo "서버에 묻지 못했다. docker compose up -d api 로 띄운 뒤 다시 해 본다" >&2
  exit 1
}
...
# 경고는 거절이 아니다. 駒台가 잘려 나간 그림은 40장이 되지 않는 것이 정상이다.
printf '%s\n' "$faults" | sed -n 's/^WARN /  경고: /p'

# 번호는 이어서 매긴다. 이름 순이 곧 표의 줄 순서라, 측정 둘을 나란히 놓고 읽으려면
