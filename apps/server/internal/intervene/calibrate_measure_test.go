...
func TestMeasureCalibrationFromRecords(t *testing.T) {
	...
	var (
		...
		bands  []bandRow
		scored []scoredGame
		// stuckRuns 는 평가치와 무관하게 전부 센다. 여기에 재채점의 조건을 씌우면
		// 평가치 이전에 둔 판 전체가 빠진다. 실제로 그렇게 틀렸다(journal §39 ②).
	...
	for _, g := range games {
		...
		//
		// 여기가 재구성하는 것은 「개입 루프가 돈 판」이라 아래 검증이 「통과한 수는
		// 임계치 아래」를 참으로 쓴다. 가져온 판에서는 누구도 막지 않아 그 수가 기보에
		// 그대로 남아 있고, 섞으면 그 검증이 빨개진다.
		if g.Imported {
			imported++
			continue
		}
	...
	fmt.Fprintf(&b, "\n표본 — 평가치가 기록된 판 %d개 (가져온 기보 %d개는 뺐다)\n", len(scored), imported)
	...
	// 잘못 이었거나 부호를 뒤집었다는 뜻이라, 아래 표 전부를 쓸 수 없다.
	//
	...
	// ─── ② 판마다 얼마나 갈리나 ──────────────────────────────────────
	//
	// 임계치를 흔드는 것보다 이쪽이 크다(journal §39 ①).
	b.Reset()
	b.WriteString("\n② 판마다 얼마나 갈리나\n")
	...
	// ─── ⑤ 갇힘 ─────────────────────────────────────────────────────
	//
	// 계단이 열리는 지점(game.HintPieceAfter · HintMoveAfter)의 근거다.
	b.Reset()
...
//
// 밴드 통계에서 뺀다. 詰み이 보이는 국면은 밴드가 조절할 수 없는 구간이고, 그런 값
// 하나가 중앙값 빼고 전부를 망가뜨린다. usi 를 import 하지 않으려고 여기에 둔다.
const mateCp = 30000
