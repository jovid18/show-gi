...
const (
	...
	//
	// 위와 따로 두는 것이 규칙의 일부다. 둘 다 종반 판정으로 걸리지만 사람에게 할 말이
	...
	// 王手·玉の薄さ)으로는 이 국면을 부를 말이 없다. 이것이 없어서 other 가 이유를
	// 대지 못했다(journal §40 ③).
	...
	//
	// 이 목록에서 가장 교육적인 카테고리다. 나머지는 「보면 알 수 있었던 것을 보지 못했다」
	...
	// 큰 실패다. 판을 읽지 못했을 때도 여기로 떨어진다 — 모른다고 말하는 편이 낫다.
	CategoryOther Category = "other"
)

// Features 는 카테고리를 정하는 데 쓰는 국면의 사실들이다. 전부 이미 구해진 숫자와
// 참거짓이고, 판도 엔진도 여기 들어오지 않는다(패키지 doc). 뽑아 오는 쪽은 game.MoveFeatures 다.
type Features struct {
	...
	//
	// 「노리고 있는가」와 다르다. 핀에 묶여 움직일 수 없는 駒는 노리기만 할 뿐이고,
	...
	// game.UnpromotedOnly 이고, 여기로는 참거짓만 온다. 우선순위 근거는 아래 classify.
	UnpromotedOnly bool
	...
	// 詰み일 것(탐색의 mate 점수는 쓰지 않는다) ② 최선수 뒤에는 그 詰み이 없을 것. ②가 없으면
	// 이미 詰んでいた 국면의 죄를 이 수의 죄라고 가르친다. ②는 아직 실전에서 걸러지지 않았다
	// (journal §40 ③).
...
// 벌어지지 않으면 함정 대신 평가가 흔들린 것으로 본다. 제안형의 reversal 임계치와 같은 기준이다.
//
...
//
// 詰み이 한쪽에라도 있으면 폭을 재지 않는다 — 부호가 반대라는 것이 이미 최대 반전이고,
...
// 확인할 수 있는 것을 앞에 둔다 — 초심자가 판을 보고 「아 정말 그렇네」까지 가야 배운 것이 된다.
func classify(in Input, lostMate bool) Category {
	...
	// 딴 것보다 잃는 것이 클 때만 걸리므로 정당한 駒交換은 여기 들어오지 않는다.
	case f.HangsPiece():
	...
	// 판 내내 나오지 않는다는 뜻이고, 하필 가장 교육적인 자리다(01-core.md §3).
	...
	// 걸리지 않았으니 교환은 유리해 보이는 쪽) 玉이 더 밀리거나(이름의 「옥 안전 무시」가 이쪽).
	...
	// 「후속 詰めろ가 없을 것」이 원안인데 詰めろ 판정이 없어 이득 없는 王手로 좁혔다.
	// [미확정: 詰めろ 판정이 붙으면 좁힌다]
	case f.GivesCheck:
