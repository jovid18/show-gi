...
type Features struct {
	...
	// 없어 그대로 배운다.
	//
	// **②는 아직 실전에서 시험된 적이 없다.** 실측 23건이 전부 통과했는데, 그건 조건이
	// 걸러낸 것이 아니라 **그 23건이 전부 이기고 있는 국면이었기** 때문이다(착수 전 승률
	// 중앙값 0.809 ≈ +866cp). 이기고 있으면 최선수가 죽을 리가 없다. 시험이 안 됐다는 것이
	// 필요 없다는 뜻은 아니다 — 그 표본이 그것을 증명하지 않았다는 뜻이다(06-status.md §40 ③).
	//
