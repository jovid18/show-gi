...
//
// > **조심할 것이 둘이다.** 재채점한 265시도는 (1) **전부 beginner로 둔 판**이고
// > (2) **전부 에이전트가 뒀다** — 사람이 둔 판은 아직 하나도 안 쌓였다(§39). 그래서
// > 0.12 칸은 「5급이 27.5% 개입당한다」가 아니라 **「이 기록 위에서 그 상수가 27.5%를
// > 골랐다」**까지다. 레벨별 값은 그 레벨의 **사람**이 둔 판이 쌓여야 잰다.
