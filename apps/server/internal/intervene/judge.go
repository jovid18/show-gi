...
// 시그니처에 없다. 그래야 상수를 흔들어 보는 데 엔진을 띄우지 않는다 — 이 성질을 깨는 import
// 하나가 이 패키지가 따로 있는 이유를 없앤다.
//
// 튜너블 넷(K · Level.Threshold · JudgeMatePlies · ShallowTrapCp)은 전부 초기값이고,
// 기록을 다시 채점하고도 하나를 옮기지 못했다. 아래 상수 주석은 「무엇을 정하나」까지고,
...
//
// cp를 그대로 쓰지 않는 것은 형세가 한쪽으로 기운 구간에서 의미가 압축되기 때문이다 —
// -800이 국면마다 다른 뜻이 된다.
...
//
// 詰み은 K를 지나지 않는다. 이기는 詰み이 1, 지는 詰み이 0이고 그것이 사실 그대로다 —
// cp로 환산해서 넣으면 K를 어떻게 바꿔도 움직이지 않는 숫자가 식에 하나 앉는다.
// 기준점도 빼지 않는다: 手合割은 cp의 자이고 詰み은 그 자 위에 없다.
...
const (
	...
	//
	// 제일 너그러운 쪽을 기본으로 둔다. 학습 앱에서 과잉 개입은 잔소리가 되고,
...
// 에이전트라 레벨별 값은 아직 잴 수 없다(journal §39 ①).
...
//
// 수 번호가 없다. "언제 두었나"는 판정에 들어가지 않는다 — 오프닝을 봐주는 것은
...
type Input struct {
	...
	// 헌납도 임계치에 닿지 않기 때문이고, 이 값을 빼면 발화선이 平手의 감도로 돌아온다
	// (journal §88). 平手는 이 칸이 0이라 지금까지와 한 비트도 다르지 않다.
	//
	// Verdict 의 Best·After 는 이 값을 빼지 않고 원본으로 남는다 — K를 바꿔 다시
...
func Judge(in Input) Verdict {
	...
	// 통과한 수도 낙폭을 담아 돌려준다. 임계치를 넘지 않았어도 손해는 남고, 실력 추정은
