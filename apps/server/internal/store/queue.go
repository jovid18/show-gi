...
// QueueWaiter 는 큐에 서 있는 한 사람이다. internal/queue.Waiter 와 같은 칸이고,
...
//
// 큐에 서는 그 요청이 부른다 — 리더도 sweeper 도 두지 않는 것이 이 큐의 설계다(journal §92).
...
// JoinQueue 는 큐에 서고, 이미 서 있으면 살아 있다고 알린다. 한 사람이 한 행이라 멱등이다.
//
...
// LeaveQueue 는 큐에서 빠진다. 없는 사람을 지워도 에러가 아니다 — 「이미 없다」와
...
// QueueWaiting 은 지금 큐에 서 있는 사람 수다. 화면에 안 나간다 — 확인용이다.
