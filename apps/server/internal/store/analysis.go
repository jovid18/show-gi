...
// AnalysisPly 는 아직 재지 않은 手 하나다. 재는 데 필요한 입력이 전부 여기 있다.
type AnalysisPly struct {
	MatchID string
	Ply     int
	// StartSFEN 은 그 판의 시작 국면이다. 手数를 빼지 않은 원문이라 판정에 그대로 넘긴다.
	StartSFEN string
...
// 쓰지 않는 것은 store 가 game·skill 을 모르는 채로 있어야 하기 때문이다.
//
...
// ClaimAnalysisPly 는 재지 않은 手 하나를 집는다. 없으면 ErrNoAnalysisPly.
//
...
// CountAnalysisBacklog 는 아직 재지 않은 手数다. AnalysisBacklogPlies 가 이 값이다.
...
// 끝난 판의 큐(019)의 표 접근. 미리 재는 큐와 같은 규약이다 — 정책은 부르는 쪽이 맡는다.

// AnalysisJob 은 집어 온 판 하나다.
type AnalysisJob struct {
	MatchID string
	// Plies 는 아직 재지 않은 手数다. 판의 길이 대신 이 큐에서 엔진을 부르는 양이다.
	Plies int
}

// MatchSeat 는 대인전 한 판의 한 자리다. games 행 하나가 곧 한 자리다.
//
// server 의 analysisSeat 와 같은 칸이고, 그 타입을 쓰지 않는 것은 store 가 shogi 를
...
// AnalysisJobBacklog 은 아직 집히지 않은 판의 수와 그 판들이 재지 않은 手数다.
