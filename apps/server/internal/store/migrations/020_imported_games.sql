-- 사람이 밖에서 둔 자기 기보를 가져오는 자리. 정한 것과 그 근거는 journal §126.
--
-- **실행은 사람이 DB 클라이언트로 직접 한다** (deploy/README.md §4).
--
-- **되돌릴 수 있고 순서를 어느 쪽으로 잡아도 안 깨진다.** 더하는 것이 nullable 컬럼 셋이라
-- 이 파일 앞의 이미지는 이 칸들을 안 읽고, 뒤의 이미지는 표가 없으면 가져오기만 못 한다 —
-- 대국도 대인전 분석도 그대로 돈다.

BEGIN;

-- 가져온 기보라는 사실과, 무엇으로 읽었는가를 한 칸이 같이 든다.
...
-- 안 들고 있는 사실 하나다.
--
-- **어느 API도 이 칸의 값을 그대로 안 돌려준다.** 밖으로 나가는 것은 「가져온 판인가」라는
...
-- 미리 잰 手가 판정 결과까지 들고 있어야 가져온 판의 悪手 줄이 만들어진다.
--
