-- kb_chunks 코퍼스 첫 적재 — 47항목(囲い 21 · 戦法 7 · 戦型 3 · 手筋 8 · 블런더 8).
--
-- **감지기가 없는 이름에는 항목을 넣지 않는다.** 검색 키가 `internal/tag` 가 내는 태그라
-- 꺼내올 키가 없는 항목은 영원히 안 걸린다. 범위와 근거는 09-tags.md §0.
--
-- `source_url`·`source_license`·`verified_by` 규칙도 같은 절에 있고, 비어 있으면 검색에
-- 걸려도 프롬프트에 안 붙는 것은 001_init.sql 의 부분 인덱스가 강제한다.
-- 본문은 **전부 일본어**다 — `TestCorpusIsJapaneseOnly` 가 기계로 본다.
