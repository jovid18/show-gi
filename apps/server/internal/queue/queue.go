...
//
// DB도 방도 엔진도 모른다. 입력은 큐에 서 있는 사람들의 레이팅·불확실성·선 시각뿐이고,
...
// Waiter 는 큐에 서 있는 한 사람이다. 이름도 사람도 안 들고 있다 — 고르는 데 안 쓴다.
type Waiter struct {
	UserID int64
	// Rating·Deviation 은 큐에 설 때 읽은 값이다. 서 있는 동안 안 바뀐다 — 대기 중에
	// 그 사람의 판이 끝날 수가 없다.
	Rating, Deviation float64
	// JoinedAt 은 큐에 선 시각이다. 밴드가 이 값으로 넓어지고, 같은 밴드 안에서는
	// 이 값이 순서를 정한다.
	JoinedAt time.Time
}
...
// StaleAfter 는 이 시간만큼 다시 안 물어보면 큐에서 빠지는 시간이다.
//
...
//
// 안 걷으면 방으로 못 간 자리가 영영 남고, 그 사람은 다시 큐에 설 때마다 이미 죽은
...
//
// 전부 잠그지 않는 이유는 큐가 길어질 때다. 밴드 안의 짝은 대개 앞머리에 있고, 뒤까지
