...
//
// DB도 방도 엔진도 모른다. 입력은 대기열에 서 있는 사람들의 레이팅·불확실성·선 시각뿐이고,
// 그래서 아래 상수를 흔들어 보는 데 DB도 대국도 필요 없다 — intervene·rating 과 같은 성질이다.
//
// 밴드를 SQL 에 적지 않으려고 이 패키지가 있다. 후보를 고르는 질의에 식을 넣으면
// 같은 식이 Go 와 SQL 에 두 벌 있게 되고, 한쪽을 고칠 때 다른 쪽이 경고 없이 어긋난다.
...
// Waiter 는 대기열에 서 있는 한 사람이다. 이름도 사람도 갖고 있지 않다 — 고르는 데 쓰지 않는다.
type Waiter struct {
	UserID int64
	// Rating·Deviation 은 대기열에 설 때 읽은 값이다. 서 있는 동안 바뀌지 않는다 — 대기 중에
	// 그 사람의 판이 끝날 수가 없다.
...
//
// [미확정] 200은 실측 없이 정한 값이다. 재려면 사람끼리 둔 판이 쌓여야 한다(journal §92).
const Base0 = 200
...
// StaleAfter 는 이 시간만큼 다시 물어보지 않으면 대기열에서 빠지는 시간이다.
//
// 대기열에 heartbeat 가 따로 없다. 기다리는 쪽이 스스로 재시도하고 그 호출이 그 일을 겸한다
// (journal §92) — 탭을 닫은 사람을 걷어내는 장치가 이것뿐이라, 화면의 재시도 주기보다
// 넉넉히 길어야 한다(지금 2초).
const StaleAfter = 12 * time.Second

// PickupTTL 은 짝이 잡힌 자리를 잡아 두고 기다리는 시간이다. 넘으면 그 행도 걷는다.
//
// 걷지 않으면 방으로 가지 못한 자리가 영영 남고, 그 사람은 다시 대기열에 설 때마다 이미 죽은
...
//
// 전부 잠그지 않는 것은 대기열이 길어질 때다. 밴드 안의 짝은 대개 앞머리에 있고, 뒤까지
