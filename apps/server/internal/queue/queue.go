...
// Waiter 는 대기열에 서 있는 한 사람이다. 이름도 사람도 안 들고 있다 — 고르는 데 안 쓴다.
type Waiter struct {
	UserID int64
	// Rating·Deviation 은 대기열에 설 때 읽은 값이다. 서 있는 동안 안 바뀐다 — 대기 중에
	// 그 사람의 판이 끝날 수가 없다.
...
// StaleAfter 는 이 시간만큼 다시 안 물어보면 대기열에서 빠지는 시간이다.
//
...
//
// 안 걷으면 방으로 못 간 자리가 영영 남고, 그 사람은 다시 대기열에 설 때마다 이미 죽은
...
//
// 전부 잠그지 않는 이유는 대기열이 길어질 때다. 밴드 안의 짝은 대개 앞머리에 있고, 뒤까지
