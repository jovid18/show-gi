...
//
// 판도 엔진도 모른다. 여기 있는 것은 USI 문자열 목록뿐이고, 그 수를 지금 국면에서 둘 수
// 있는지는 부르는 쪽이 정한다(game.bookOpponent). 수순을 고치는 데 엔진도 DB도 필요 없다.
//
// 상대 자신의 수만 담는다. 사람의 수와 짝으로 묶으면 초심자가 정석대로 받아주지 않는
// 순간(거의 매번이다) 북이 첫 수에서 끝난다.
package book
...
// openings 의 순서가 곧 화면에 뜨는 순서다. 振り飛車 둘 · 居飛車 둘로 짝을 맞춘다.
var openings = []Opening{
	{
		...
		black: []string{
			...
			"4i4h", // ▲4八金
			"6i5h", // ▲5八金
			// ▲6六歩이 ▲7八銀보다 먼저다. 銀이 7九를 비우면 8八의 角을 받치는 것이
			// 없어지고, 대각선이 열려 있으면 그 자리에서 角을 공짜로 준다.
			"6g6f", // ▲6六歩
			"7i7h", // ▲7八銀
			"1g1f", // ▲1六歩
			"9g9f", // ▲9六歩
	...
	},
	{
		...
		black: []string{
			...
			// 밀면 2二의 角이 8八까지 닿고, 거기서 ▲6八銀을 두면 角을 공짜로 준다
			// (journal §48).
			"6g6f", // ▲6六歩
			"7i6h", // ▲6八銀
			"6i7h", // ▲7八金
...
//
// 後手 몫은 돌려서 만든다. 180° 회전이 정확히 先手/後手 대응이라 좌표 변환으로 닫히고,
// 두 벌 적어 두면 한쪽을 고칠 때 다른 쪽이 경고 없이 낡는다.
...
//
// 읽을 수 없는 문자열은 그대로 돌려준다. 수순이 상수라 에러를 만들어도 부를 곳이 없고,
// 착수 직전에 룰 엔진이 한 번 더 본다.
