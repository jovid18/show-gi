...
// 1위를 짚던 자리의 크기다(journal §58 · 회차 1 #8).
//
...
// 같은 모양). 물러진 수 자체는 기보에 없으므로 그 국면을 그대로 되만들 수는 없고,
// 여기서 재는 것은 「그 깊이에서 k가 1위를 바꾸는 빈도」다.
func TestMeasureCardBestDivergence(t *testing.T) {
	...
	// step 은 몇 手마다 재는가다. 짝수라야 사람(先手)이 둔 뒤의 국면만 걸린다.
	const step = 8
	...
	for i := 0; i < len(moves); i += step {
		...
		if err != nil {
			// 여기서 죽지 않는다. 이 판의 종반은 엔진이 실제로 죽는 자리라(journal §56)
			// 재던 것 전체를 버리면 숫자가 사라진다. 잰 데까지 요약하고 멈춘다.
		...
		// 그대로 두지만, 아래 k=3 소요는 찬 상태의 값보다 짧다.
		start = time.Now()
