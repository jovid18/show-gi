...
// 상대 수 탐색이 돌아오지 않으면 판이 접힌다. 매달려 있지도 않고, 승패를 지어내지도 않는다.
...
// (gateTesujiOptions) 시한은 그 한 번에서 걸리고, 그러면 후보 전부를 보지 못한 것이 된다.
...
// 시한이 끝나면 묻지 못한 후보를 세어서 돌려준다. 경고 없이 넘기면 「手筋이 없었다」와
// 「묻지 못했다」가 같은 결과가 된다.
