...
// Band 는 상대가 겨냥하는 형세 구간이다. 플레이어 관점 cp다.
//
...
//
// 밴드 적중이 10에서 멈추고 20은 덮이는 국면은 늘지 않는데 비용만 배가 된다.
// k=1은 0/6이다. 후보가 하나면 밴드 제어라는 것이 성립하지 않는다.
const CandidateK = 10

// MaxConcessionCp 는 난이도가 아무리 낮아도 한 수에 양보할 수 있는 최대 폭이다.
// 최선의 안전한 후보와의 차이로 센다. 절대 형세로 세면 이미 크게 이기거나 지는
...
// AdaptsToSkill 은 언제나 true 다. 이 구현이 하는 일이 밴드를 옮기는 것이다(SkillAdapter).
func (o *adaptiveOpponent) AdaptsToSkill() bool { return true }

// ChooseBest 는 밴드를 보지 않고 최선수를 고른다(BestPlayer). 사람이 詰み을 걸고 있는
// 동안만 불린다(MateChasePlies).
//
// 같은 k로 묻는다. 깊이도 k도 평소와 같아야 positions 캐시가 같은 행을 쓰고, k가
// 갈리면 같은 국면의 1위가 갈린다(journal §34 ②).
...
func (o *adaptiveOpponent) Choose(ctx context.Context, startSFEN string, moves []string, sk skill.Estimate) (string, error) {
	...
	if err != nil {
		// 판을 읽지 못하면 고를 근거가 없다. 최선수로 물러선다.
		return res.Best, nil
	}

	// 밴드의 원점을 手合割이 옮긴다. 절대 좌표 쪽은 「호각」을 0cp 로 읽는데(Band),
	// 駒落ち에서 호각은 그 手合의 초기 평가치다.
	//
	// 옮기지 않으면 그 좌표가 판 내내 도달 불가능해지고(첫 수부터 now 가 상단 위라
	// 상대 좌표 분기로만 간다), 상대가 핸디캡을 되돌려 주는 일을 아예 하지
	// 않는다(journal §55 의 거울상 · §84).
	//
	...
	//
	// 뿌리 점수(res.Score)를 쓰지 않는다. 같은 값이지만 저쪽은 거른 뒤를 모르므로,
...
// option 은 상대가 실제로 둘 수 있는 후보 하나다. cp는 플레이어 관점으로 뒤집은 값이다.
type option struct {
	move     string
	playerCp int
}
...
//
// 詰み 줄을 뺀다. 밴드 산수는 cp 로만 성립하고(eval.Score.Centipawns), 詰み 줄은 어느
// 쪽이든 밴드에서 가장 먼 후보다. 전부 詰み이면 후보가 비어 부르는 쪽이 최선수로
// 물러선다.
...
// 고정하는데, 그 값은 아직 실측 전이다(journal §47). 0.3으로 옮기는 날 너그러운 쪽이
// 최대 폭을 40% 넘어가고 세지는 쪽은 60%에서 잘린다. 아래는 prior 가 어디에 있어도
// 양 끝이 정확히 ±SkillShiftCp 다.
func skillShift(sk skill.Estimate) int {
	...
	// prior 가 0이나 1이어도 0으로 나누지 않는다. loss 를 이미 잘라서 그쪽 분기가
	// 돌지 않는다.
	var ratio float64
	switch {
	case loss > prior:
...
//
// 밴드와 같은 숫자에서 나온다. 단계를 따로 계산하면 화면이 말하는 강함과 상대가
// 실제로 겨냥하는 강함이 갈린다(journal §31).
...
// shifted 는 밴드 전체를 옮긴 것이다. 폭은 그대로다. 넓히면 같은 실력에서도 상대의
// 강함이 수마다 튄다.
