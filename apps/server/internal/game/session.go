...
//
// **3과 5에서 내렸다. 기록을 세어 보니 위 칸이 안 열렸다**(06-status.md §39).
// 실제 대국 5판에서 물러진 국면 24개의 분포가 이랬다.
//
//	1회 16개 · 2회 6개 · 3회 1개 · 4회 1개 · 5회 0개
//
// 즉 3에서는 파란 테가 두 번 열렸고 **5에서는 파란 화살표가 한 번도 안 열렸다.**
// 세 번을 버티는 사람이 거의 없어서, 계단의 윗칸이 실전에서 닿지 않는 자리에 있었다.
//
// 2는 8개 국면(33%)이 닿고 4는 1개가 닿는다. **4는 관측 하나 위에 서 있다** — 5가
// 0이라는 것이 확실한 만큼 4가 옳다는 것은 확실하지 않다. 판이 더 쌓이면 다시 잰다.
const (
	HintPieceAfter = 2
	HintMoveAfter  = 4
)
...
// 네 번 실패해야 열리기 때문**이다. 기댈 수 있는 힌트가 아니고, 첫 칸은 어디로 갈지를
