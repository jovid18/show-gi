package game

import (
	"github.com/jovid18/show-gi/apps/server/internal/eval"
	"github.com/jovid18/show-gi/apps/server/internal/intervene"
	"github.com/jovid18/show-gi/apps/server/internal/shogi"
)

// Recorder 는 대국 이벤트를 기록한다. nil이면 기록 없이 대국을 진행한다.
//
// 모든 메서드는 즉시 돌아와야 한다. 세션 goroutine에서 불리므로, 여기서
// 기다리면 그동안 착수도 투료도 스냅샷도 받을 수 없다. 구현은 큐에 넣고 자기
// goroutine에서 쓴다.
//
// 기록 실패로 대국을 중단하지 않는다.
//
// game 은 DB를 모른다. 여기서 나가는 것은 전부 우리 타입이고, SQL로 옮기는 일은
// 구현 쪽이 한다.
type Recorder interface {
	// Started 는 대국 시작을 한 번 기록한다.
	Started(startSFEN string, humanColor shogi.Color)

	// Moved 는 확정된 수만 받는다.
	//
	// 개입으로 물러진 수는 기보에 넣지 않고 Retracted 로 기록한다.
	Moved(ply int, usi string, by Side)

	// Evaluated 는 그 手数의 국면 평가치를 나중에 채운다. 先手 관점이다.
	//
	// Moved 와 갈라져 있는 것은 값을 아는 시점이 다르기 때문이다. 판정이 사람의 수 뒤와
	// 그 직전 상대 수 뒤를 한꺼번에 주므로, 상대 수의 평가치는 한 수 늦게 채워진다.
	//
	// 관점을 先手로 고정하는 것은 edges 의 깊이별 값과 같은 규약이다. 「플레이어
	// 관점」으로 적으면 색이 다른 두 판을 함께 놓을 수 없다.
	Evaluated(ply int, sente eval.Score)

	// Good 은 확정된 사람의 수가 好手였다는 것이다. 그 手数의 Moved 뒤에만 온다.
	//
	// 대국 화면에는 띄우지 않고 되짚기가 읽는다(journal §141).
	Good(ply int)

	// Retracted 는 개입으로 물러진 수를 기록한다.
	//
	// 개입 전에 선택한 수이므로 실력 추정에 사용한다(01-core.md §5).
	//
	// 같은 ply에 여러 번 호출될 수 있으며 각 시도를 모두 기록한다(journal §17).
	//
	// 설명 문장은 저장하지 않고 explain.BaseMessage 로 재생성한다.
	// 그래야 문구를 수정한 뒤 지난 대국에도 같은 설명을 적용할 수 있다.
	Retracted(ply int, usi string, v intervene.Verdict)

	// Undone 은 수동으로 무른 수를 기록한다. ply 는 그 사람 수의 手数다.
	//
	// 개입으로 무른 수와 구분하기 위해 Retracted 와 별도로 기록한다(01-core.md §5).
	//
	// game_moves 에는 현재 수순만 남도록 무른 수와 상대 응수를 함께 제거한다(store.RecordUndo).
	Undone(ply int, usi string)

	// Named 는 사람이 그 판에서 처음 짜낸 囲い·전법·戦型의 코드다.
	//
	// styleTags 가 같은 囲い를 반복 감지해도 대국마다 코드별로 한 번만 기록한다.
	//
	// 手筋은 분류 정확도가 충분하지 않아 사용자 기록에 포함하지 않는다(journal §45).
	Named(code string)

	// Hinted 는 사람이 불러서 받은 최선수 힌트 한 번이다.
	//
	// 사용자가 요청한 힌트를 자동 개입 횟수에 포함하지 않도록 별도로 기록한다.
	//
	// key 는 shogi.PositionKey 다. 手数가 달라도 같은 국면이면 3회째 요청을 차단한다.
	Hinted(ply int, key string, stage int, bestUSI string)

	// HintTaken 은 알려준 그 수를 사람이 실제로 뒀는가다. 답까지 본 국면에만 온다.
	// 1단계는 駒만 짚으므로 「알려준 대로 뒀다」와 뜻이 같지 않다.
	//
	// 「알려줬는데도 못 찾은 좋은 수」가 그대로 약점이 된다(01-core.md §5).
	HintTaken(key string, taken bool)

	// Finished 는 대국이 끝날 때 한 번. 끝나지 않고 연결이 끊기면 오지 않는다.
	// 그 경우를 어떻게 남길지는 구현이 정한다.
	Finished(status Status, winner Side)
}
