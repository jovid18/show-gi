...
// goroutine에서 쓴다.
//
// 기록이 실패해도 대국은 계속된다. 대국이 본체이고 기록은 그 위에 얹힌다.
//
...
type Recorder interface {
	...
	// Moved 는 확정된 수만 받는다.
	//
	// 물러진 수는 Retracted 로 간다. 기보에 남으면 롤백이 롤백이 아니게 된다.
	...
	//
	// 개입에 오염되지 않은 하나뿐인 실력 신호다(01-core.md §5).
	//
	// 같은 ply에 여러 번 올 수 있다. 한 국면에서 몇 수를 시도하고 전부 물러지는 일이
	// 실제로 있고(journal §17), 그 반복 자체가 기록할 값이다.
	//
	// 화면에 나간 문장은 남기지 않는다. 카테고리에서 결정적으로 다시
	// 만들어지므로(explain.BaseMessage) 적어 두면 두 벌이 되고, 문구를 고친 날
	// 지난 판만 옛 문장을 말한다.
	Retracted(ply int, usi string, v intervene.Verdict)

	// Undone 은 사람이 스스로 무른 수다. ply 는 그 사람 수의 手数.
	//
	// Retracted 와 따로 둔다. 판이 되돌아간 것은 같지만 시작한 쪽이 반대이고, 저쪽에
	// 섞으면 그 칸의 정의가 그 자리에서 거짓이 된다(01-core.md §5).
	//
	...
	//
	// 手筋은 오지 않는다. 이름의 정확도가 아직 프로덕션 수준에 미치지
	// 못하고(journal §45), 「당신이 쓴 手筋」으로 적으면 그 오진이 사람의 기록으로
	// 굳는다.
	Named(code string)

	// Hinted 는 사람이 불러서 받은 최선수 힌트 한 번이다.
	//
	// 개입과 따로 둔다. 저쪽은 앱이 먼저 말을 건 자리이고 이쪽은 사람이 부른 자리라,
	// 섞으면 「개입 N회」가 사람이 스스로 물어본 횟수까지 세게 된다.
	//
	// key 는 그 국면의 shogi.PositionKey 다. 되돌아온 자리가 手数로는 갈리지만
	// 국면으로는 같아야 3회째가 막힌다.
