...
type Recorder interface {
	...
	//
	// Moved 와 갈라져 있는 것은 값을 아는 시점이 다르기 때문이다. 판정은 사람이 둔
	...
	//
	// 개입에 오염되지 않은 하나뿐인 실력 신호다(01-core.md §5). 개입이 막지 않았다면
	...
	// 지난 판만 옛 문장을 말한다. Evaluated 와 달리 나중에 오지 않는다: 되무르는
	// 그 자리에서 정해진다.
	Retracted(ply int, usi string, v intervene.Verdict)

	// Undone 은 사람이 스스로 무른 수다. ply 는 그 사람 수의 手数.
	//
	// Retracted 와 따로 둔다. 판이 되돌아간 것은 같지만 시작한 쪽이 반대다 —
	...
	// 굳는다. 나머지 셋은 판과 수순만으로 정해져 엔진 평가치에 걸리지 않는다.
	Named(code string)

	// Hinted 는 사람이 불러서 받은 최선수 힌트 한 번이다.
	//
	// 개입과 따로 둔다. 저쪽은 앱이 먼저 말을 건 자리이고 이쪽은 사람이 부른 자리라,
	// 섞으면 「개입 N회」가 사람이 스스로 물어본 횟수까지 세게 된다 — 待った를 따로
	// 둔 자리에서 같은 판단을 한다(010_game_hints.sql).
	//
