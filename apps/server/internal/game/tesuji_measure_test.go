...
func tesujiLoss(t *testing.T, an Analyst, moves []string) (int, []tag.Tag) {
	...
	return loss, namedTesuji(after, me, moves[len(moves)-1], j)
}
...
func TestMeasureTesujiGateOverAGame(t *testing.T) {
	...
	for p := 1; p <= len(playtestUpTo103); p++ {
		before, err := positionAfter(shogi.StartSFEN, playtestUpTo103[:p-1])
		if err != nil {
			t.Fatalf("국면: %v", err)
		}
		me := before.Turn

		// **새로 생긴 것만 센다.** 서 있는 형태를 매 수 세면 한 両取り가 여러 수에 걸쳐
		// 잡혀서, 「몇 번 뜨는가」가 아니라 「몇 수 동안 서 있었나」를 재게 된다.
		had := map[string]bool{}
		for _, tg := range tag.FindTesuji(before, me) {
			had[tg.Code] = true
		}
		after, err := positionAfter(shogi.StartSFEN, playtestUpTo103[:p])
		if err != nil {
			t.Fatalf("국면: %v", err)
		}
		last, err := shogi.ParseUSIMove(playtestUpTo103[p-1])
		if err != nil {
			t.Fatalf("수: %v", err)
		}
		fresh := []string{}
		for _, tg := range append(tag.FindTesuji(after, me), tag.DropTesuji(after, last, me)...) {
			if !had[tg.Code] {
				fresh = append(fresh, tg.Code)
			}
		}
		if len(fresh) == 0 {
			continue
		...
		loss, out := tesujiLoss(t, analyst, playtestUpTo103[:p])
		if len(out) > 0 {
			named++
			for _, tg := range out {
				seen[tg.Code] = true
		...
		t.Logf("%3d수 %-5s 형태=%v 낙폭=%+5dcp 통과=%v", p, playtestUpTo103[p-1], fresh, loss, codes(out))
	}
