...
// journal §34의 숫자를 만든 자리다. TesujiLossCp 가 [미확정] 이라 언젠가 다시 잰다.
//
// 판정하지 않고 값을 찍고 지나간다. 통과/실패로 만들면 엔진이 흔들릴 때마다 CI가
...
//
// 세션이 부르는 것과 같은 함수를 부른다. 형태를 따로 세면 그 어긋남이 문서의 숫자로만
// 나타나 아무 데서도 터지지 않는다.
...
// 게이트가 실제로 무엇을 끄는가. 실 기보 한 판을 수마다 훑는다(journal §34 ④).
//
// 세는 것은 셋이다: 手筋의 형태가 새로 생긴 수, 그중 이름이 붙은 수, 서로 다른 이름의
// 수. 마지막이 곧 화면에 뜨는 횟수다(useTagAnnounce).
func TestMeasureTesujiGateOverAGame(t *testing.T) {
	...
	for p := 1; p <= len(playtestUpTo103); p++ {
		...
		// 게이트 앞의 룰 층이다. 세션이 쓰는 것과 같은 함수라, 「몇 번 뜨는가」가
		// 프로덕션과 어긋날 수 없다.
		...
		//
		// 게이트가 묻는 것은 「이 수가 손해인가」인데 両取り의 손해는 형태를 만든 다음
		// 딸 때 난다. 그래서 飛로 銀 둘을 노렸지만 어느 쪽을 먹어도 飛가 죽는 국면이
		// 그대로 통과한다.
		//
		// 최선 수순을 같이 찍으면 눈으로 갈린다. 그 안에 대상을 따는 수가 있으면 형태가
		// 현금화되는 것이고, 없으면 이름만 선 것이다. 자동으로 재려면 대상 칸을 tag
		// 밖으로 꺼내야 한다.
		if len(out) > 0 {
...
// 따는가), 통과하지 않으면 사용자가 본 화면은 게이트가 붙기 전의 것이다(journal §34).
func TestMeasureDecorativeFork(t *testing.T) {
	...
	//
	// 재료를 맞춰 둔다. 한쪽이 압도적이면 무슨 수를 둬도 수천 cp가 움직여서 100cp
	// 게이트를 재는 의미가 없다(journal §45).
...
// 같은 국면·같은 깊이가 같은 값을 주는가(journal §34 ②).
//
// 앞선 탐색이 남긴 치환표 때문에 주지 않는다. 다른 엔진·다른 해시 크기로 갈아탈 때
// 제일 먼저 다시 재야 하는 값이다.
