...
// other 중 詰み이 아닌 것들이 「한 수 뒤에 죽는 자리」인지를 세는 자리다(journal §40 ⑦).
//
// 새 카테고리로 이어지지 않는다. 앱이 어차피 그 수를 되돌리므로 「詰まされる」와
// 「다음 수에 詰まされる」에서 플레이어가 할 행동이 같다.
//
// 종반의 어휘는 駒得보다 速度다(「終盤は駒の損得より速度」). 그 速度를 세는 단위가
// 手スキ이고 0手スキ가 詰み, 1手スキ가 詰めろ다.
//
// 판정은 패스로 한다. 「내가 아무것도 하지 않으면 詰むか」가 곧 정의라, 수번만 뒤집어
// 詰み 탐색에 물으면 답이 나온다.
//
// 어디서 묻는지가 갈림길이다. 물러진 수 직후는 상대 차례라 거기서 詰み을 묻는 것이
// 이미 앞 측정이다. 詰めろ는 상대가 벌하는 수를 두고 내 차례가 된 자리에서 묻고, 그
// 「벌하는 수」를 PV[0] 로 잡는다.
//
// 그 PV 첫 수가 이 측정의 한계다. 같은 국면·같은 깊이가 다른 값을 주므로(journal §39 ⑦)
// 아래 건수는 다시 돌리면 달라질 수 있다.
//
...
func TestMeasureBlunderTsumero(t *testing.T) {
	...
	for _, b := range all {
		...
		// 상대가 벌하는 수. 이미 손에 있는 탐색의 PV 첫 수라 추가 비용이 없다.
		...
		if !ok {
			// 王手를 받고 있다. 손을 뺄 수 없으니 詰めろ를 물을 수 없고, 이쪽은
			// 「王手가 이어진다」로 따로 말해야 한다.
			inCheck++
		...
		// 한계(DepthLimit=11) 안에서 결론을 내지 못한 것이고, 그것을 「詰めろ 없음」으로
		// 읽으면 안 된다. 섞어 세면 조용한 수의 몫이 실제보다 커 보인다.
