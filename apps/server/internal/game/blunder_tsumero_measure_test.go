...
//
// 새 카테고리로 이어지지 않는다. 재 본 뒤 이름을 붙이지 않기로 했다 — 앱이
// 어차피 그 수를 되돌리므로 「詰まされる」와 「다음 수에 詰まされる」에서 플레이어가 할
// 행동이 같다. 이 측정의 값은 남는 것이 무엇인지 정직하게 적을 수 있게 하는 것이다.
//
// 종반의 어휘가 駒得보다 速度라는 것은 그대로다 — 「終盤は駒の損得より速度」. 그
// 速度를 세는 단위가 手スキ이고 0手スキ가 詰み, 1手スキ가 詰めろ다.
//
// 판정은 패스로 한다. 「내가 아무것도 하지 않으면 詰むか」가 곧 정의라, 수번만 뒤집어 詰み
...
// PV 도 거기 포함되므로, 아래 건수는 다시 돌리면 달라질 수 있는 숫자다 — 단정으로 읽지
// 않는다.
//
...
func TestMeasureBlunderTsumero(t *testing.T) {
	...
	for _, b := range all {
		...
		if !ok {
			// 王手를 받고 있다. 손을 뺄 수 없으니 詰めろ를 물을 수 없다 —
