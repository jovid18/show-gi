...
// 두는 것」뿐이다 — 넘어야 할 선은 05-roadmap.md 의 6번 항목 주석에 있다.
//
...
//
// 판단은 전부 룰 엔진과 좌표뿐이다 — 엔진을 부르지 않는다. adaptive.go 의 자살수
// 필터도 같은 방식이고, 그래서 진형을 늘려 보는 데 엔진이 필요 없다.
func (o *bookOpponent) next(startSFEN string, moves []string) (string, bool) {
	...
	for _, u := range moves {
		...
		// 상관없이 자기 형태를 쌓는 것이 곧 국면을 보지 않는 것이 된다.
		//
		// 「한 번이라도」로 세는 것이 상태를 갖지 않고도 되돌리기와 맞는 자리다 — 물러진
		// 수는 moves 에서 사라지므로 그 잡기도 같이 없던 일이 된다.
