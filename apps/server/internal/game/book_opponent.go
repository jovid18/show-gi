...
// 넘긴다.
//
// 후보 생성도 두 안전 필터도 건드리지 않는다. adaptive.go 를 감싸기만 하므로 밴드
// 제어와 실력 추정은 그대로 돌고, 이 층이 하는 일은 「초반 몇 수를 정석으로 대신
// 두는 것」뿐이다(05-roadmap.md 6번 항목).
//
// 상태를 갖지 않는다. 지금 몇 수까지 뒀는지도, 넘길 때가 됐는지도 매번 (startSFEN,
// moves) 에서 다시 구한다. 개입이 수를 되무르는 이상 카운터를 갖고 있으면 롤백된
// 수를 센 채로 남는다(journal §48).
...
//
// 감싸는 자리는 배선이다(server/ws.go). Config 에 진형을 받지 않는 것은 세션이
...
// AdaptsToSkill 은 안쪽에 물어본다. 여기서 false 를 답하면 진형을 고른 판에서 강함
// 눈금이 경고 없이 사라진다(journal §47).
...
// ChooseBest 는 북을 건너뛴다(BestPlayer). 불리는 자리가 종반이라, 북이 거기서 수순을
// 이어 두면 「최선으로 버틴다」가 그 판에서만 경고 없이 지켜지지 않는다.
...
//
// 엔진을 부르지 않는다. 판단이 전부 룰 엔진과 좌표뿐이라 진형을 늘려 보는 데도
// 엔진이 필요 없다.
func (o *bookOpponent) next(startSFEN string, moves []string) (string, bool) {
	...
	for _, u := range moves {
		...
		// 駒를 잡은 적이 있으면 진형 만들기는 끝이다. 그 뒤로도 자기 형태를 쌓는 것이
		// 곧 국면을 보지 않는 것이 된다.
		//
		// 「한 번이라도」로 세면 상태를 갖지 않고도 되돌리기와 맞는다. 물러진 수는
	...
	if err := pos.ValidateMove(m); err != nil {
		// 사람이 그 칸을 먼저 쓴 경우다. 수순을 건너뛰지 않고 전부 넘긴다. 한 수만
		// 빼고 이어 두면 그때부터는 어느 진형도 아닌 모양이 된다.
		return "", false
	}
