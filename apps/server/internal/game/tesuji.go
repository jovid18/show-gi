...
//
// tag 가 엔진을 모른 채 이름만 짚는 것은(엔진 없이 테스트가 돈다) intervene 이 엔진을
// 모르는 것과 같은 구조다. 이름 없는 수를 알리지 않는 잠금은 01-core.md §7.1,
// 둘을 나눈 근거는 09-tags.md §5.
...
//
// 이름으로 견준다. 같은 이름이 다른 자리에 하나 더 생긴 것은 새 소식으로
// 세지 않는다 — 화면이 어차피 이름 단위로 한 번만 띄운다.
//
...
func freshTesuji(before, after shogi.Position, c shogi.Color, lastUSI string) []tag.Tag {
	...
	// 넘긴다 — 이름 하나가 뜨지 않는 것과 대국이 도는 것의 값이 다르다.
...
//
// 견주는 두 값은 intervene 이 개입을 판정할 때 쓰는 그 둘이다. 새 기준을 만들지 않는다 —
// 그쪽이 「낙폭」이라고 부르는 것을 여기서는 cp로 보고, 임계치만 다르다.
//
// 詰み이 한쪽에라도 있으면 이름을 붙이지 않는다. 이 게이트가 cp 뺄셈이라 詰み을
// 섞으면 자가 다른 두 값의 차가 되고, 그때 「모르면 이름을 붙이지 않는다」가
// 하나뿐인 정직한 답이다. 종반의 手筋 이름을 그만큼 잃는다. 詰み이 보이는 국면에서
// 사람은 手筋 이름을 찾지 않는다.
