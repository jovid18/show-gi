...
//
// 깊이로만 탐색한다(go depth). 시간 제한도 중간 절단도 없다 — 근거는 01-core.md §4.
...
// 추정치를 보지 않는다. 기준선이 곧 최선수인 상대라 옮길 밴드가 없다 — 그래서
// 이 구현이 있고(위 주석), 실력에 따라 움직이는 쪽은 adaptive.go 뿐이다.
...
// 그래도 인터페이스를 만족시켜 두는 것은, 하지 않으면 세션이 「조절을 끄지 못한
// 상대」로 보고 다른 경로를 타기 때문이다. 답이 같아도 부르는 이유가 갈리면 안 된다.
