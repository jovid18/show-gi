...
//
// 사람이 끝까지 둔 두 판에서 제안형 手筋(kind='tesuji')이 0건이었고, 「후보가 없어서」
// 「엔진이 다 떨어뜨려서」 「시한을 넘겨서」가 갈려 있지 않았다(journal §45 · §56).
//
//	SHOWGI_MEASURE=1 SHOWGI_USI_CMD=/opt/yaneuraou/run \
//	  go test ./internal/game/ -run MeasureTesujiHintGate -v -timeout 60m
//
// 세션을 만들지 않고 maybeTesujiHint 의 조건을 그대로 다시 쓴다. 세션으로 돌리면
// 기보에 없는 착수 시각을 흉내내야 하고, 그 흉내가 곧 결론이 된다.
//
...
// journal §74 뒤에 사람이 처음 둔 판이다. 그 판에서 후보가 전부 「모르는 채」로
// 침묵했고(§76 ④), 갈라야 할 것이 하나 남았다. 통과할 手筋이 애초에 없었는가,
// 아니면 k 줄 밖이라 묻지 못했는가.
//
// k 를 올리면 판정을 받기는 한다. 받는 것과 통과하는 것은 다르고, 앞 판에서는 그
// 차이가 0이었다(§74).
//
