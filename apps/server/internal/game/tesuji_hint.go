...
// 제안형 手筋 힌트. 아직 두지 않은 수가 만들 이름을 찾는다(tesuji.go 는 착수 後로
// 방향이 반대).
//
// 프로덕션에서는 꺼져 있다. server/ws.go 가 Config.TesujiHint 에 nil 을
// 넘긴다(journal §78). 배선만 끊은 것이라 여기 있는 것은 전부 지금도 도는 코드다.
//
// 게이트는 저쪽 것을 그대로 쓴다. freshTesuji·enginePaidOff 를 나눠 쓰고 이 파일은
// 부르는 순서만 갖는다. 두 벌이 되면 힌트와 착수 뒤가 어긋난다(journal §34 ⑦).
...
// 있어야 한다(journal §45 의 kaku_ryodori).
//
// 이 값이 정하는 것은 「어디까지가 확정 탈락인가」다(gateTesujiOptions 의 decided).
//
// 3은 실측이다(journal §74). MultiPV 는 가지치기가 k 마다 달라서 k 자체가 정의의
// 일부다.
...
// tesujiOptions 는 합법수를 전부 둬 보고 새 이름이 생기는 것만 남긴다. 엔진을 부르지
// 않는다.
//
// 싸지 않다. 비용이 합법수 × FindTesuji 라 종반에는 초 단위로 간다(journal §56).
// 그래서 세션 goroutine 밖에서 돈다. 안에서 부르면 그동안 스냅샷도 投了도 받을 수
// 없다(session.go 의 maybeTesujiHint).
//
// freshTesuji 를 후보마다 다시 부르므로 착수 전 국면의 이름을 매번 새로 센다. 한 번
// 구해 돌려쓰면 이 파일이 「새 이름인가」의 규칙을 자기 손으로 다시 쓰게 된다.
func tesujiOptions(pos shogi.Position, c shogi.Color) []TesujiOption {
	// LegalMoves 는 pos.Turn 쪽의 수만 돌려준다. 상대 차례에 물으면 에러 없이 빈 결과가
	// 와서 「手筋이 없다」로 보인다.
	if pos.Turn != c {
		return nil
	}
...
//
// k를 상수로 두지 않고 인자로 받는 것은 depth 와 같다. 프로덕션이 넣는 값은
// TesujiHintRootK.
//
// dropped 는 모르는 채로 남은 후보 수다. 탐색이 실패하면 전부 빈 결과다.
func gateTesujiOptions(
	...
	// Ranked 를 쓴다. Lines[0] 은 1위가 아닐 수 있고, 그것을 최선으로 읽으면 낙폭
	// 전체가 어긋난다(usi.SearchResult.Ranked).
	lines := root.Ranked()
	if len(lines) == 0 {
	...
	// 줄 밖의 후보는 마지막 줄보다 나쁘다. 그 마지막 줄이 이미 상한 밖이면 밖은 전부
	// 탈락이 확정이고, 안이면 모르는 것이다. 둘을 같은 침묵으로 섞지 않는다.
	//
	// k줄을 다 받았을 때만 그렇게 말할 수 있다. 중간 순위 하나가 오지 않으면 Ranked 가
	// 그것을 빼고 주므로 「밖」에 그 순위도 섞이고, 그것은 마지막 줄보다 나쁘지 않다.
	//
	// 詰み이 섞이면 「모른다」다. 자가 다른 두 값의 차를 임계치와 견줄 수
	// 없다(enginePaidOff 와 같은 판단이다).
	decided := false
	if len(lines) == k {
	...
	for _, o := range opts {
		...
		// 두 값이 한 탐색의 형제 줄이라 뿌리가 같다(enginePaidOff).
		j := Judgement{
...
// 알릴 것은 하나다. 수를 짚지 않는 것이 계단 ①의 성질이다(buildHint).
