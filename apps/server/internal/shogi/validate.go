...
//
// 실사용자한테서는 거의 안 나온다. 클라이언트가 서버에서 받은 합법수만 고르게 하므로
// 반칙은 UI 단계에서 이미 막힌다. 여기까지 오는 것은 둘 중 하나다 —
// API를 직접 부른 경우이거나, 클라이언트와 서버의 국면이 어긋난 경우.
// 뒤쪽은 우리 버그이고, 그래서 사유가 정확할수록 원인을 빨리 찾는다.
//
// 실력 추정과는 무관하다. 블런더는 "합법이지만 나쁜 수"라 판정이 엔진 평가치에서 나온다.
type Reason int
...
// reasonMessages 는 화면에 나갈 수 있는 문구다. 타깃 사용자가 일본인이므로 일본어다.
//
// 정상 경로에서는 쓰이지 않는다(위 Reason 주석 참조). 국면이 어긋났을 때의 마지막 방어선이라
// 비어 있거나 영어이면 안 될 뿐이다. 그래도 읽고 무엇이 문제인지는 알 수 있게 적는다 —
// 반칙 이름만 던지면 "二歩ってなに" 에서 막힌다.
