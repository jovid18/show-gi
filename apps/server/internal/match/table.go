...
// Recorder 는 한 사람의 몫을 기록한다. 판 하나에 둘이 붙어 games 행 두 개로
// 남는다(journal §83).
//
// game.Recorder 와 따로 둔 것은 여기서 벌어지는 일이 셋뿐이라서다. 같은 인터페이스를
// 쓰면 이 패키지가 「부르지 않는 메서드 여섯」을 갖게 된다.
//
// 세 메서드 다 즉시 돌아와야 한다. 테이블 goroutine 이 부른다.
type Recorder interface {
	// Started 는 대국이 시작될 때 한 번. myColor 는 이 기록기가 맡은 사람이 잡은 쪽이다.
	Started(startSFEN string, myColor shogi.Color)
	// Moved 는 확정된 수다. 대인전에는 물러지는 수가 없으므로 둔 수가 곧 확정이다.
	Moved(ply int, usi string)
	// Finished 는 결과다. 이 기록기가 맡은 사람 관점이라 같은 판에서 둘이 반대로 온다.
	Finished(r Result)
}

// Result 는 한 사람 관점의 결과다. store.GameResult 와 같은 어휘를 쓴다. 옮기는
// 자리를 하나 더 두면 그 자리가 경고 없이 어긋난다.
type Result string
...
const (
	...
	// ResultAbandoned 는 승부가 나지 않은 채로 끝난 판이다. 두 자리에서 온다: 서버가
	// 내려갔을 때(StatusAborted)와 한 수도 두지 않은 채 시간이 다 됐을 때(StatusExpired).
	//
	// 수를 두고 나서의 시간패는 승부가 난 것이라 win/loss 로 간다.
	ResultAbandoned Result = "abandoned"
)

// Config 는 테이블 하나의 설정이다.
type Config struct {
	// Black·White 는 두 대국자다. 先手·後手가 여기서 확정되고 대국이 이어지는 동안
	// 바뀌지 않는다.
	Black, White Player
	// Recorders 는 先手·後手마다 하나씩이다. nil 이면 그쪽을 기록하지 않는다. DB 가 없는
	// 배포를 위한 자리이고, 익명 대국을 뜻하지 않는다.
...
//
// 先手·後手마다 한 벌씩 뿌리면 한쪽에만 보내고 끝나는 경로가 생긴다.
type viewSnapshot struct{ st *snapshotData }

// Table 은 대국 하나다. 모든 메서드는 안전하게 동시 호출할 수 있다. 하는 일이
// goroutine 에 명령을 보내고 답을 기다리는 것뿐인 점이 game.Session 과 같다.
type Table struct {
	cmds chan command
	// finished 는 승패가 정해진 순간 닫힌다. done 은 finishedGrace 만큼 늦게 닫히므로,
	// 「振り返り」 링크를 그쪽에 걸면 사람은 이미 화면을 떠나 있다.
...
type state struct {
	...
	//
	// 0이어도 판을 멈추지 않는다. 멈추면 지고 있는 쪽이 탭을 닫아 얼릴 수 있다.
...
func NewTable(ctx context.Context, cfg Config) (*Table, error) {
	...
	// 기록 행은 대국이 시작되는 자리에서 만든다. 행이 첫 Moved 보다 먼저 있어야 한다 —
	// 기록기가 행 없이 온 수를 그냥 버린다(server/recorder.go 의 gameID == 0).
	//
	// 그래서 한 수도 두지 않은 판에도 행 둘이 남는다. 어느 목록에도 뜨지 않는다 —
...
//
// 끝나는 그때에 문을 닫으면 하필 그때 끊겼다 다시 붙은 사람이 결과 대신 오류를 본다.
// 投了를 받은 쪽이 새로고침하는 일은 흔하다.
//
// 방보다 길어야 한다(FinishedTTL). 같게 두면 닫히는 순서가 정해지지 않아 「방은 열려
// 있는데 테이블은 닫힌」 창이 생긴다.
const finishedGrace = FinishedTTL + time.Minute

// run 은 명령과 시계를 한 자리에서 받는다. 시계는 반드시 select 안에 있어야 한다.
...
func (t *Table) run(ctx context.Context, st *state) {
	...
	// grace 는 판이 끝난 뒤에만 걸린다. 그 전에는 nil 채널이라 select 가 그 갈래를
	// 고르지 않는다.
	...
	for {
		...
		case <-timer.C:
			...
			// 착수로 timer 를 다시 걸기 직전에 이 case 가 준비될 수 있다. 남은 시간을
			// 다시 보지 않으면 방금 제때 둔 사람이 시간패로 진다.
			if st.leftForTurn() > 0 {
				resetTimer(timer, st.leftForTurn())
				continue
			}
...
func (st *state) play(by shogi.Color, usi string) (Snapshot, error) {
	...
	// 시계는 착수와 같은 자리에서 다시 시작한다. 따로 두면 느린 DB 쓰기 하나가
	// 상대의 시간을 먹는다.
	st.turnFrom = st.now()
	...
	// 상대 쪽 기록기도 같은 수를 받는다. 한쪽에만 넣으면 그 판이 두 사람에게 다른
	// 판으로 남는다.
...
// timeout 은 수번 쪽의 시간이 다 됐을 때다. 대개 승패가 난다.
//
// 한 수도 두지 않았으면 예외다. 누구도 두지 않았으면 판이 없었던 것이다.
...
func (st *state) resultFor(c shogi.Color) Result {
	...
	case st.status == StatusRepetition:
		return ResultDraw
	case !st.hasWin:
		// 서버가 내려갔거나 한 수도 두지 않은 채 시간이 다 됐다. 되짚기가 결과 있는
		// 판만 열기 때문에(journal §51) 어느 쪽이든 목록에 뜨지 않는다.
...
// notify 는 막히지 않게 보낸다. 느린 구독자가 테이블을 붙들면 그동안 상대의 시계가
// 흐른다.
func notify(ch chan viewSnapshot, snap viewSnapshot) {
	select {
	case ch <- snap:
	default:
		// 앞의 것을 버리고 최신을 넣는다. 스냅샷이 언제나 통째라 중간을 놓쳐도 맞다.
		select {
		case <-ch:
		default:
		}
...
// Play 는 한 수 둔다. by 는 서버가 쿠키에서 정한 쪽이고 클라이언트가 보내지 않는다.
...
//
// 접속 표시가 여기 붙어 있다. 따로 두면 끊긴 연결이 붙어 있는 것으로 남는 경로가 생긴다.
func (t *Table) Subscribe(ctx context.Context, by shogi.Color) (<-chan Snapshot, func(), error) {
	...
	if _, err := t.send(ctx, command{kind: cmdPresence, color: by, on: true}); err != nil {
		// 구독을 되돌린다. 그냥 나가면 raw 가 구독 목록에 남은 채 누구도 읽지 않고,
		// 돌려줄 정리 함수도 없다.
	...
	// 관점을 붙이는 자리다. 테이블은 先手·後手를 모르는 스냅샷 하나만 뿌린다
	// (viewSnapshot).
	...
	return out, func() {
		once.Do(func() {
			// 정리에 ctx 를 쓰지 않는다. 떨어지는 이유가 대개 그 ctx 의 종료라, 그대로
			// 넘기면 구독이 남고 그 사람은 영영 접속 중으로 보인다.
			off, cancel := context.WithTimeout(context.WithoutCancel(ctx), time.Second)
			defer cancel()
...
// errorsIsClosed 는 「테이블이 이미 닫혔다」를 경고 없이 넘기는 자리다. 판이 끝나는
// 순간과 연결이 떨어지는 순간은 흔히 겹치고, 로그를 남기면 정상 종료마다 줄이 쌓인다.
