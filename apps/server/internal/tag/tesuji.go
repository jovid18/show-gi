...
// 종반의 龍에 쏠려 「이 수가 手筋이다」 대신 「龍이 강하다」를 말했다(journal §67).
//
// 게이트로는 막을 수 없다 — 엔진은 「이 수가 손해인가」를 묻지 「両取り가 이득인가」를
// 묻지 않는다. 자르는 방향이 안전한 쪽이다: 뜨지 않으면 힌트가 없을 뿐이지만, 틀린 이름은
...
// 대각이다. 桂만 방향을 말하지 않는다 — 利き가 두 칸뿐이라 낄 방향이 하나다. 그래서 桂는
...
// Fork 는 sq 의 駒가 両取り의 형태를 이루는지만 본다 — 「그래서 得인가」는 묻지 않는다.
...
// 없으면 구할 수 없다. 그래서 조건이 「한 줄에 둘이 있다」다.
//
// 앞뒤를 나눠 보는 것은 남는다. 앞은 무엇이든 되지만 뒤는 歩가 아니어야 한다 —
// 香를 던져 얻는 것이 뒤의 駒라서, 뒤가 歩면 꿰어도 이름을 붙이지 않는다.
...
//
// 「둘 수 있는 手筋」 대신 「이미 걸린 手筋」을 짚는다. 후보 수를 전부 둬 보며 찾는
// 쪽이 제안형 힌트가 원하는 것이지만(착수 前에 알려야 한다), 그것은 합법수마다 판을
// 만들어 술어를 돌리는 것이라 비용이 다르다. 지금 화면에 나가는 것은 방금 만든 형태에
// 이름을 붙이는 것이라 이쪽이면 된다 — 붙일지 말지는 엔진이 정한다(game/tesuji.go).
//
// 답이 국면에만 매여 있어서 같은 국면이면 같은 결과가 온다. 부르는 쪽이 갖고 있어도
// 되고, 세션이 그렇게 한다 — 이름을 통과시키는 엔진 값이 국면과 함께만 유효해서다.
// 호출 자체는 싸다(初期配置에서 90µs 남짓).
