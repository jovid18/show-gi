...
// 手筋은 좌표 대신 관계로 정해져서 룰 엔진에 합법수로 묻는다. 이 패키지가 정하는 것은
// 형태와 이름뿐이고 「그래서 得인가」는 game 의 엔진 게이트가 건다 — 경위는
// 09-tags.md §5.
...
//
// 龍·馬는 세지 않는다. 이름이 「飛車」·「角」을 말하는데 龍·馬는 다른 駒이고, 실측에서도
// 종반의 龍에 쏠렸다(journal §67). 엔진은 「이 수가 손해인가」를 묻지 「両取り가
// 이득인가」를 묻지 않으므로 게이트로는 막을 수 없다.
...
// targetSquares 는 그 駒가 딸 수 있는 상대 駒가 선 칸들이다. 수를 세면 成·不成이 둘로
// 샌다. 歩·玉은 이름의 관례로 뺀다(journal §45).
//
// 手番을 c 로 맞추지 않으면 LegalMoves 가 상대 수만 내보내서 경고 없이 빈 결과를 준다.
...
// forkShape 는 그 이름이 말하는 형태인가를 본다. 이름이 방향을 말하면 그 방향을
// 요구한다. 「十字」는 縦과 横의 교차, 「角」은 斜め, 「割打ち」는 출처가 정한 뒤쪽 두
// 대각이다. 桂는 利き가 두 칸뿐이라 낄 방향이 하나이고, 그래서 「둘」이면 된다.
//
// 이 조건이 없을 때 무엇이 잘못 떴는지는 journal §34 ⑤ · §45.
...
// Fork 는 sq 의 駒가 両取り의 형태를 이루는지만 본다. 이 결과를 그대로 화면에 실으면
// 안 된다. 엔진과의 AND는 game/tesuji.go 가 건다.
...
// Skewer 는 田楽刺し를 본다. 香가 한 筋의 상대 駒 둘을 串刺し로 꿰는 형태다.
//
// 両取り와 다르다. 串刺し는 앞의 駒를 치우면 뒤가 그대로 드러나므로 조건이 「한 줄에
// 둘이 있다」다.
//
// 앞뒤를 나눠 본다. 앞은 무엇이든 되지만 뒤는 歩가 아니어야 한다. 香를 던져 얻는 것이
// 뒤의 駒이기 때문이고, 両取り가 歩를 대상에서 빼는 것과 같은 관례다(targetSquares).
func Skewer(pos shogi.Position, sq int, c shogi.Color) (Tag, bool) {
	...
	if found[1].Type() == shogi.Pawn {
		return Tag{}, false // 뒤가 歩면 꿰는 형태에 이름이 붙지 않는다
	}
	return dengaku, true
}
...
// FindTesuji 는 그 색이 지금 걸고 있는 手筋의 이름 전부를 짚는다.
//
// 「둘 수 있는 手筋」 대신 「이미 걸린 手筋」이다. 착수 前에 알리는 제안형은 합법수마다
// 판을 만들어 술어를 돌려야 해서 비용이 다르다. 붙일지 말지는 엔진이 정한다
// (game/tesuji.go).
//
// 답이 국면에만 매여 있어서 같은 국면이면 같은 결과가 온다. 부르는 쪽이 갖고 있어도
// 되고 세션이 그렇게 한다. 호출 자체는 싸다(初期配置에서 90µs 남짓).
