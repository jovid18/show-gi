...
//
// 龍·馬는 안 센다. 이름이 「飛車」·「角」을 말하는데 龍·馬는 다른 駒이고, 실측에서도
// 종반의 龍에 쏠려 「이 수가 手筋이다」 대신 「龍이 강하다」를 말했다(journal §67).
//
// 게이트로는 못 막는다 — 엔진은 「이 수가 손해인가」를 묻지 「両取り가 이득인가」를
// 묻지 않는다. 자르는 방향이 안전한 쪽이다: 안 뜨면 힌트가 없을 뿐이지만, 틀린 이름은
...
// 대각이다. 桂만 방향을 안 말한다 — 利き가 두 칸뿐이라 낄 방향이 하나다. 그래서 桂는
...
// Fork 는 sq 의 駒가 両取り의 형태를 이루는지만 본다 — 「그래서 得인가」는 안 묻는다.
...
// 없으면 못 구한다. 그래서 조건이 「한 줄에 둘이 있다」다.
//
// 앞뒤를 나눠 보는 것은 남는다. 앞은 무엇이든 되지만 뒤는 歩가 아니어야 한다 —
// 香를 던져 얻는 것이 뒤의 駒라서, 뒤가 歩면 꿰어도 이름을 안 붙인다.
...
// FindTesuji 는 그 색이 지금 걸고 있는 手筋의 이름 전부를 낸다.
//
// 「둘 수 있는 手筋」 대신 「이미 걸린 手筋」을 낸다. 후보 수를 전부 둬 보며 찾는
