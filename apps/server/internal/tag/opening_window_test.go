...
// 戦法·戦型은 序盤 분류다. 시간 경계가 없으면 종반의 우연이 이름을 받는다 — floodgate
// 341판에서 이름이 붙은 手数의 분포는 journal §44.
//
...
// 경계는 「지금 몇 수째인가」 대신 「振った 것이 몇 수째였나」에 건다.
//
...
// 경계를 넘은 판에도 이름이 남는다. 囲い는 상태라 그대로 뜨고, 잘라낸 것은
// 戦法·戦型 축뿐이라는 것을 짚어 둔다.
