...
// 한 개만 노리는 것에는 両取り가 붙지 않는다. 이 음성 테스트가 없으면 술어가 그냥
...
// 歩 둘을 노려도 両取り가 붙지 않는다. 歩는 세지 않는다.
...
// 내므로, 手番을 맞추지 않으면 경고 없이 「両取り가 없다」가 된다 — 에러가 나지 않는 버그다.
...
// FindForks 는 판 전체를 훑고 이름을 겹쳐 짚지 않는다.
...
// 十字飛車의 형태를 이루지 못하고, 그 이름을 붙이면 초심자는 다음에 그 형태를 알아볼 수 없다.
...
// 龍·馬가 덤으로 얻은 한 칸은 이름이 말하는 방향에 들지 않는다.
//
...
func TestPromotedPiecesDoNotBorrowTheBaseNames(t *testing.T) {
	for _, tc := range []struct {
		...
		// 형태는 그대로 縦横·斜め다 — 이름 때문에 붙지 않는다는 것을 고정하려고,
...
// 相手の飛車または金」로 정의한다 — 앞대각으로 둘을 낀 것에는 붙지 않는다. 방향을 보지 않으면
...
func TestSilverForkDoesNotCountTheKing(t *testing.T) {
	// 5五 銀의 뒤 대각 4六에 後手 玉, 6六에 後手 金 → 王手銀取り다. 割打ち로 뜨지 않는다
