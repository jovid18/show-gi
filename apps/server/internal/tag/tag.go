...
//
// **이 맵은 段을 안 본다.** 처음에는 `{6, 8, Rook}` 처럼 칸으로 적었는데, 출처가 전부 筋으로
...
// **전법을 짜는 순간**이었다는 뜻이다.
//
// **段은 한 군데서만 본다** — `DetectFormation` 이 振った **착지가 자기 2段(八/二)인지**만
// 가른다(敵陣 3四로 뛰는 ▲3四飛 横歩取り를 袖飛車에서 뺀다). 그건 「振る 순간」 한 번의
// 판정이라 위의 「飛를 6五로 올리면 이름이 꺼진다」와 다르다 — 이름이 정해진 뒤 段이
// 움직이는 것은 여기서 안 보므로 라벨은 안 꺼진다.
//
