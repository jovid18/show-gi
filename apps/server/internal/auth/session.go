...
//
// 세션을 DB에 두지 않는다. 담는 것이 user_id·이름·만료 셋뿐이고 서버가 세션을 골라 끊을
// 일도 없어서, 표 대신 쿠키 자체를 HMAC로 서명한다.
//
// 이 패키지는 HTTP 핸들러를 갖지 않는다. 표면은 internal/server 가 갖고(auth.go), 여기
// 있는 것은 서명과 Google 왕복 둘뿐이다.
package auth
...
//
// 서명 쿠키는 발급한 뒤에 끊을 수 없다. 그래서 이 값이 곧 기기를 잃어버렸을 때 남의
// 손에 로그인이 살아 있는 시간이고, 새는 것은 학습 기록에 한정된다.
const SessionTTL = 30 * 24 * time.Hour

// ErrNoSession 은 쿠키가 없거나, 서명이 맞지 않거나, 만료됐을 때다.
//
// 셋을 구별해서 돌려주지 않는다. 부르는 쪽이 할 일이 어느 경우든 같고, 구별해서 화면에
// 말하면 서명 위조를 시도하는 쪽에 힌트가 된다.
var ErrNoSession = errors.New("auth: no valid session")

// Session 은 쿠키에 담기는 내용이다.
//
// 이름을 같이 담아서 화면 한 줄 때문에 매 요청이 users 를 읽지 않게 한다. 낡는 것은
// 사용자가 Google 쪽에서 이름을 바꿨을 때뿐이고, 그때는 다음 로그인에 따라온다.
type Session struct {
...
// NewCodec 은 서명 키를 받는다. 키가 비어 있으면 nil을 돌려주고 로그인이 꺼진다.
// 서명이 없으면 쿠키 한 줄로 아무 user_id 나 될 수 있다.
