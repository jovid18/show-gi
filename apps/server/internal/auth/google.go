...
// 엔드포인트는 상수로 둔다. Google의 discovery 문서를 읽어 오면 기동할 때마다 외부
// 호출이 하나 더 생기고, 그 호출이 실패하면 로그인 전체가 꺼진다.
const (
...
//
// 암묵 흐름(id_token 을 브라우저로 직접 받는 것)을 쓰지 않는다. 그쪽은 토큰이 주소창과
// 브라우저 기록에 남는다.
type Google struct {
	clientID     string
	clientSecret string
...
// NewGoogle 은 클라이언트를 만든다. 둘 중 하나라도 비면 nil이고, 그러면 로그인 표면
// 전체가 꺼져 익명 대국으로 남는다. 엔진·DB가 없을 때와 같은 판단이다.
//
// 자리표시자 unset 도 빈 것으로 본다. SSM이 빈 문자열을 거부해서 아직 발급되지 않은
// 키가 그 값으로 들어가 있고(06-status.md §3), 그대로 켜면 Google이 400을 준다.
...
//
// state 는 CSRF 막이다. 같은 값을 쿠키에도 심어 두고 콜백에서 맞춰 본다. 남이 자기
// 계정의 콜백 URL을 우리 사용자에게 열게 하는 공격이 이것으로 막힌다.
...
//
// 서명을 검증하지 않는다. 이 토큰은 브라우저를 거치지 않고 우리가 방금 TLS로 직접 부른
// Google의 토큰 엔드포인트에서 왔다. 가운데에 낄 수 있는 것이 없어서 검증할 것이 없고,
// Google 문서도 그 경우를 예외로 고정해 둔다. 브라우저가 들고 온 토큰이면 정반대다.
//
// 그래서 JWT 라이브러리를 끌어오지 않는다. 하는 일은 base64 한 번과 JSON 한 번이고,
// 설정 실수를 잡으려고 aud 만 확인한다.
