...
// 대국 후 총평에서 세는 쪽이다. 문장으로 바꾸는 일은 explain.RenderSummary 가 하고, 여기는
// 기록에서 사실을 결정적으로 뽑는다.
//
// 화면에 나가는 것이 숫자(summaryStats)와 문장 둘이고, 같은 수를 두 벌로 두지 않으려고
// explain.GameFacts 에는 숫자를 두지 않았다.
...
type focusPoint struct {
	// Ply 는 물러진 수의 手数다. 화면은 이 수를 두기 직전 국면을 연다. 물러진 수는 기보에
	// 없으므로(game.Recorder) 그 자리가 「다시 생각할 국면」이다.
	Ply      int    `json:"ply"`
...
// 화면이 Step 에서 이름을 만들면 어휘가 두 벌이 된다(skill.Rank).
...
//
// 대국 중에는 보내지 않는다. 자기 실력이 매 수 흔들리는 것을 보여 줄 이유가 없다
// (skill.RiseRate 가 비대칭이라 블런더 하나에 몇 계단이 움직인다).
...
type gameSummaryPayload struct {
	...
	// GameID 는 이 판이 기록에 남은 번호다. 화면이 되짚기로 건너가는 데 쓴다. 대국 화면은
	// 그때까지 자기 판의 번호를 모른다(기록은 WS 밖에서 비동기로 쓰인다).
	//
	// 되짚기가 부르는 쪽에서는 0이다. 이미 그 판을 열고 있는 화면이라 쓸 데가 없다.
	GameID int64 `json:"gameId,omitempty"`
	// Skill 은 이 판의 段級 변화다. 되짚기에서는 언제나 nil 이다. 추정치는 사람에게 붙는
	// 값이라 지난 판을 여는 지금은 이미 다른 값이고, 그때의 값을 남겨 두지 않았다.
...
func factsOf(rec store.GameRecord, level intervene.Level) (explain.GameFacts, summaryStats) {
	f := explain.GameFacts{
		...
		// 가져온 판에서는 누구도 그 수를 막지 않았다. 문장이 「戻す」로 말하면 없던 일을
		// 있었다고 말하게 된다(explain.GameFacts.Intervened).
	...
	stats.Accuracy = accuracyOf(rec, humanColor)
...
// accuracyOf 는 기보에 남은 평가치로 그 사람의 精度를 센다. 물러진 수는 기보에 없으므로
// 세지 않는다. 확정한 수순의 精度다.
func accuracyOf(rec store.GameRecord, human shogi.Color) *int {
	last := 0
	for _, m := range rec.Moves {
		last = max(last, m.Ply)
	}
	...
	acc, ok := accuracy.Game(startSFENOf(rec.StartSFEN), human, scores)
	if !ok {
		return nil
	}
...
//
// 카테고리를 보지 않는다. 어느 종류가 더 배울 것이 많은지를 정하는 순간 순위표가 하나 더
// 생긴다(journal §15).
//
...
// rankCategories 는 많은 순으로 줄 세운다. 같은 수면 코드 순이다. 무작위면 같은 판을 두 번
// 요약할 때 다른 문장이 나온다.
...
// 序盤·中盤·終盤의 경계다. 手数의 절대값으로 가른다. 판의 길이로 나누면 짧은 판에서 첫 수가
// 종반이 되는데, 20手에 끝난 판에는 종반이 없는 것이 맞다.
//
...
// trendOf 는 후반이 나아졌는가다. 개입 밀도로 본다. 낙폭의 평균은 물러진 수에만 있는 값이라
// (journal §39 ⑥) 통과한 수가 늘어난 것을 보지 못한다.
...
//
// 마지막으로 채워진 평가치를 쓴다. 평가치는 수보다 늦게 오므로 마지막 몇 수가 비어 있을 수
// 있고(store.RecordedMove), 그래서 끝에서 StandingMaxLag 手 안의 것만 받는다.
//
// 부호를 뒤집는 자리다. EvalCp 는 先手 관점이고 여기서 필요한 것은 사람 관점이다. 手合割의
// 기준점도 여기서 뺀다.
