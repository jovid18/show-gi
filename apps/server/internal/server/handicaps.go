...
//
// SFEN도 기준점도 보내지 않는다. 시작 국면은 세션이 스냅샷으로 보내고
// (game.Snapshot.SFEN) 기준점은 판정의 상수라, 여기 있는 것은 고르는 데 필요한 이름과
// 한 줄 설명뿐이다(openingItem 과 같은 판단).
//
// 平手가 목록에 없다. 「접지 않는다」는 서버에 묻지 않는 기본값이고, 그 자리는 화면이
// 직접 그린다(internal/handicap).
