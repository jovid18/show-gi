...
// 자리마다 판 번호와 사람이 짝지어 나간다. 분석기가 실력 추정을 그 짝으로 나누므로
...
func TestCollectPairsEachSeatWithItsPlayer(t *testing.T) {
	...
	entry := &roomRecord{
		at: time.Now(),
		rec: map[shogi.Color]*dbRecorder{
			shogi.Black: {events: make(chan recordEvent, 1), done: make(chan int64, 1)},
			shogi.White: {events: make(chan recordEvent, 1), done: make(chan int64, 1)},
		},
		id: map[shogi.Color]int64{},
		ready: map[shogi.Color]chan struct{}{
			shogi.Black: make(chan struct{}), shogi.White: make(chan struct{}),
		},
		player: map[shogi.Color]match.Player{
			shogi.Black: {UserID: 11, Name: "先手"},
			shogi.White: {UserID: 22, Name: "後手"},
		},
		// 결과는 안 채운다 — 레이팅은 이 테스트의 관심이 아니고, 비면 그 자리가
		// 그냥 돌아간다(updateRatings).
		result: map[shogi.Color]match.Result{},
	}
	entry.rec[shogi.Black].done <- 101
	entry.rec[shogi.White].done <- 202
	...
	want := map[shogi.Color]analysisSeat{
		...
		shogi.White: {gameID: 202, userID: 22, color: shogi.White},
	}
	if len(seats) != len(want) {
		t.Fatalf("자리 %d개, want %d", len(seats), len(want))
	}
