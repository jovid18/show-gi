...
//
// 로그인 없이도 대국이 된다(games.user_id 가 nullable 인 이유, journal §18). 로그인이
// 바꾸는 것은 그 판이 누구의 것으로 남느냐 하나다.
...
const (
	...
	// callbackPath 는 Google에 등록한 리디렉션 경로다. /api 아래여야 한다. Caddy와 Vite
	// 둘 다 그 접두사만 서버로 넘긴다(apps/web/Caddyfile).
...
// enabled 는 로그인 표면을 열 수 있는지다. store 까지 있어야 한다. 사용자를 남길 곳이
// 없으면 로그인해도 그 판이 익명으로 남는다.
...
//
// 실패해도 화면으로 돌려보낸다. 브라우저가 주소창으로 들어오는 자리라 JSON을 쓰면 사람이
// 그 JSON을 보게 된다. 무엇이 틀렸는지는 로그가 갖는다.
func (h *authHandler) callback(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || state.Value == "" || state.Value != q.Get("state") {
		// 우리가 시작하지 않은 콜백이다. 남이 자기 계정으로 로그인시키려는 시도가 이
		// 모양으로 온다.
...
// logout 은 쿠키를 지운다. 서버에는 지울 것이 없다(서명 쿠키의 값이자 한계다).
...
//
// 핸들러가 이 함수 하나로만 사람을 안다. 쿠키 이름과 서명 검증이 여기 한 곳에 있어야
// 「어떤 경로는 만료를 보지 않는다」가 생기지 않는다.
...
//
// enabled 를 같이 준다. 화면은 「로그인하지 않음」과 「로그인이 이 배포에 아예 없음」을
// 구분해 그려야 한다.
//
// skill_profile 은 여기 싣지 않는다. 본인만 보는 민감 정보이고(02-architecture.md §7
// 위협 2) 이 응답은 화면이 늘 부르는 자리다.
...
//
// PUBLIC_ORIGIN 이 있으면 그것이 정본이다. 없으면 요청에서 되짚되 스킴을
// X-Forwarded-Proto 로 정하지 않는다. 앞단이 ALB → Caddy 두 겹이고 Caddy는 신뢰 설정
// 없이는 그 헤더를 평문 http 로 덮어, Google이 redirect_uri 불일치로 거부한다.
//
// 대신 호스트로 정한다. 배포된 모든 자리가 HTTPS이고 평문인 것은 로컬뿐이라, 규칙이
// 「localhost 면 http, 아니면 https」로 끝난다.
