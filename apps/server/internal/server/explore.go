...
// **手合割 표**에서(internal/handicap) 뿌리를 얻는다(journal §85).
//
...
// 무엇이든 물을 수 있다. 그래서 벽이 둘이다 — **로그인**과 **엔진 슬롯 하나**.
...
const (
	...
	// exploreSlots 는 이 표면이 **동시에 잡을 수 있는 엔진 수**다.
	//
...
type exploreRequest struct {
	// Handicap 은 手合割 id다. **빈 값이 平手**다 — 화면의 기본값이고 표에 없다
...
//
// **화면이 그 값을 만들 수 없다.** 二枚落ち의 0手目가 +1490인데(handicap.BaselineCp) 그것을
// 안 말해 주면 「+1490」이 「압승 중」으로 읽히고, 후보 목록의 색도 한 줄도 빠짐없이 최대
...
// 뒤집는데(detailOf), 검토의 관점은 언제나 先手(下手)로 못박혀 있다(exploreRoot).
...
//
// **관점을 先手(下手)로 못박는다.** 되짚기의 뿌리는 사람이 어느 쪽으로 뒀는가를 들고 있어서
// (`whatifRoot.Human`) 노드의 cp가 그 사람 관점인데, 검토에는 플레이어가 없다. 先手인 것은
// 手合割이 정한다 — 駒落ち는 上手(後手)의 駒를 빼고 **언제나 下手부터** 두고, 기준점 표도
// 下手 관점 cp라(internal/handicap) 관점을 여기로 맞추면 화면의 숫자와 「互角ライン」이
// 같은 자를 쓴다.
//
