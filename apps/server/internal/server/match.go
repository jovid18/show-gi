...
// 대인전의 HTTP 표면. 방을 만들고 들여다보는 데까지이고, 실제로 두는 것은
...
// roomPayload 는 방 하나다. id 말고는 아무것도 주지 않는다 — 상대의 段級도 전적도
...
type roomPayload struct {
	...
	//
	// Waiting 과 함께 「아직 앉지 않은 손님」을 가른다. 그 사람에게만 확인 화면이
...
// 받아야 방 id 를 훑어보는 것이 성립하지 않고(match.ErrNoRoom), 로그인하지 않은 요청까지
...
func (h *matchHandler) create(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		...
		// 아직 방이 없으므로 「있다/없다」를 말할 대상부터 없고, 화면은 이 답을 보고
		// 「로그인하고 다시」를 그려야 한다.
