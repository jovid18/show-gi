...
type dbRecorder struct {
	...
	// done 은 마지막 이벤트까지 쓴 뒤 대국 id를 한 번 실어 보낸다. 버퍼가 1이라 받는 쪽이
	// 없어도 막히지 않는다.
	//
	// 총평이 이것을 기다린다(ws.go sendSummary). 「id가 있다」와 「기록이 다 됐다」를 한
	// 신호로 둔다. 갈라 두면 부르는 쪽이 둘을 따로 기다려야 한다.
	done chan int64
}
...
type recordEvent struct {
	...
	stage int
	taken bool
	// code 는 evNamed 의 태그 코드이자 힌트 두 이벤트의 국면 키다. usi 를 돌려쓰지 않는다.
	// 저 칸은 수이고 이건 이름이라, 같은 칸에 넣으면 뜻이 이벤트마다 달라진다.
	code    string
	by      game.Side
	score   eval.Score
	...
	status  game.Status
	winner  game.Side
	// result 는 evFinished 가 그대로 적을 결과다. 대인전이 채운다. 그쪽은 승패를 테이블이
	// 이미 先手·後手 기준으로 정해 놨고(match.state.resultFor), status·winner 로 되돌려
	// 넣으면 같은 변환이 두 곳에 생긴다.
	result store.GameResult
}

// recordQueue 는 이벤트 버퍼 크기다. 한 판이 200수 남짓이고 쓰기 한 번이 밀리초 단위라,
// 이 크기가 차는 것은 DB가 멈췄을 때뿐이다. 그때는 넘치는 것을 버린다.
const recordQueue = 256

// recordTarget 은 이 판을 어느 행에 남기나다. 연결이 열릴 때 한 번 정해지고 그 판 내내
// 바뀌지 않는다. 두는 중에 다른 탭에서 로그아웃해도 시작할 때의 주인으로 끝난다.
...
func (r *dbRecorder) send(ev recordEvent) {
	...
	case r.events <- ev:
	default:
		// 버리고 계속한다. 기록은 부가 기능이고 대국이 본체다. 경고 없이 버리지는 않는다.
...
// Named 는 사람이 처음 짜낸 이름 하나다. 한 판에 코드마다 한 번이라 세션이 이미 걸러서
// 보내고(game.recordStyleTags), 여기서 또 세지 않는다.
...
func (r *dbRecorder) run(ctx context.Context, st *store.Store, level intervene.Level, target recordTarget) {
	...
	// abandoned 로 닫는다. 되열린 채로 남으면 되짚기에도 이어하기에도 걸리지 않는다.
	...
	drain := func(ev recordEvent) {
		switch ev.kind {
		case evStarted:
			// 이어하는 판은 행을 새로 만들지 않는다. 만들면 한 대국의 기보와 개입이 두
			// 행으로 갈린다(journal §46). 시작 국면도 진형도 그 행에 있는 그대로 둔다.
		...
		case evFinished:
			// 행이 없어도 신호는 보낸다. 행 만들기가 실패한 판(DB가 흔들린 경우)에서 경고
			// 없이 나가면 이 채널을 기다리는 쪽이 영원히 기다린다. 대인전은 그 기다림이 곧
			// 기록기 goroutine 둘의 수명이라(server/match_records.go 의 collect) 그때부터
			// 프로세스가 끝날 때까지 남는다.
			//
	...
	for {
		...
		case <-ctx.Done():
			...
			// 끝나지 않고 연결이 끊긴 판은 그렇게 남긴다. 빈 result 로 두면 「아직 두는
			// 중인 판」과 구별할 수 없다.
