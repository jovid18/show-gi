...
type positionHandler struct {
	auth *authHandler
	// read 는 그림을 읽는 창구다. 키가 없으면 nil 이고, 그때 이 표면은 열리지 않는다.
...
func (h *positionHandler) readImage(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(http.MaxBytesReader(w, r.Body, positionBodyMax)).Decode(&req); err != nil {
		// 넘쳐서 끊긴 것은 「읽지 못했다」와 다르다. 고칠 것은 형식 대신 크기다.
	...
	if err != nil {
		...
		// 저쪽 API 의 사정이므로 로그로 남기고 「다시 눌러 볼 수 있는 실패」로 내보낸다.
...
// maxKept 는 이 폴더가 담는 그림 수의 상한이다. 번호가 두 자리를 넘으면 이름 순서가
...
func decodeImage(s string) ([]byte, error) {
	...
	// 줄바꿈이 섞여 오는 base64 가 있다. 떼지 않으면 그림 전체가 「형식이 틀렸다」로 죽는다.
...
// boardReadUnavailable 은 키가 없어 이 표면이 열리지 않은 자리다.
