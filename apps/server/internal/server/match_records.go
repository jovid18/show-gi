...
// (012_match_games.sql) 그 번호를 아는 것이 기록기뿐인데, 판이 끝난 뒤 「振り返り」
// 링크가 그 값으로 만들어진다.
//
// 방 자체는 match.Hub 가 소유한다. 여기는 그 옆에 붙는 곁장부라 만료도 따로 판다. Hub 가
// 방을 걷어갈 때 이쪽에 알려주는 길을 내면 두 패키지가 서로를 알게 된다.
type matchRecords struct {
	store *store.Store
	// level 은 개입 임계치다. 대인전에는 개입이 없어 기록기가 그 값을 쓸 행
	// (interventions)이 생기지 않는다. 넘기는 것은 기록기를 한 벌만 두기 위해서다.
...
type roomRecord struct {
	...
	//
	// dbRecorder.done 은 값 하나짜리 채널이라 먼저 읽은 쪽이 가져가 버린다. 같은 쪽으로 탭을
	// 둘 열면 두 번째 연결이 5초를 기다린 끝에 링크를 그리지 못한다. 그래서 받는 쪽을 하나로
	// 모으고(collect) 여기 옮겨 둔다.
	//
	...
	// start·moves 는 미리 재는 데 넘길 수순이다. 두 자리가 같은 수를 다 적으므로 手数로
	// 거른다(counting).
	//
	...
	// plies 는 그 판에 둔 手数다. 큐에 세울 때(analysisJob) 이 값이 필요하고, 기록에서
	// 되읽으면 판이 끝나는 경로에 질의가 하나 는다.
	plies int

	// player·result 는 자리마다의 대국자와 결과다. 레이팅을 옮기는 데 쓴다(match_rating.go).
	//
	// 결과를 기록기 채널과 별개로 여기서 한 벌 더 받는다. 레이팅은 두 사람을 같이 옮기므로
	// 한쪽 관점의 행 하나로는 짝을 맞출 수 없다.
...
//
// 판이 시작한 시각부터 센다. 그래서 한 판이 걸릴 수 있는 최대 시간보다 길어야 한다.
// 짧으면 오래 두는 판이 끝나기 전에 항목이 사라지고 「振り返り」 링크가 그려지지 않는다.
// 방의 만료(40분)로는 모자란다. 1手 60초라 100手만 둬도 그 값을 넘긴다.
...
//
// ctx 는 서버의 것이다. 연결에 매달면 한쪽이 탭을 닫는 순간 그 사람의 기록이 abandoned 로
// 닫히는데, 대인전은 그때도 판이 계속 돈다.
//
// 그래서 판마다 ctx 를 한 겹 더 판다. 서버 ctx 를 그대로 주면 기록기 goroutine 이 판이
// 끝난 뒤에도 이벤트 채널에 영원히 매달려, 끝난 판마다 goroutine 둘과 256칸짜리 채널 둘이
// 배포 전까지 쌓인다.
func (m *matchRecords) new(
	...
	for c, p := range map[shogi.Color]match.Player{shogi.Black: black, shogi.White: white} {
		// 사람마다 변수를 새로 두고 그 주소를 넘긴다. 반복 변수의 주소를 그대로 넘기면 두
		// 기록기가 같은 값을 가리키고, 한 판이 한 사람의 행 두 개로 남는다.
		userID := p.UserID
...
//
// 번호는 여기 한 곳에서 받는다(roomRecord.id).
//
// 先手·後手마다 goroutine 을 따로 둔다. 한 자리에서 차례로 기다리면 한쪽이 늦는 것이 다른
// 쪽의 신호를 막고, 멀쩡히 기록된 사람이 「振り返り」 링크를 받지 못한다.
func (m *matchRecords) collect(ctx context.Context, cancel context.CancelFunc, entry *roomRecord) {
	...
	for c, rec := range entry.rec {
		...
		go func(c shogi.Color, rec *dbRecorder) {
			defer wg.Done()
			// 번호를 받지 못해도 신호는 연다. 그때는 entry.id 가 비어 있어서 gameIDOf 가
			// false 를 준다.
			defer close(entry.ready[c])

			select {
			case id := <-rec.done:
				// 0은 「행이 없다」다. 없는 판으로 링크를 그릴 수 없다.
				if id == 0 {
					return
				}
				m.mu.Lock()
				entry.id[c] = id
				m.mu.Unlock()
				// 번호가 나가기 전에 표시한다. 아래 ready 가 닫히는 순간 화면이 되짚기를
				// 열 수 있고, 그때 이미 「분석 중」이라야 한다. 판 하나에 한 행이라 두 자리가
				// 같은 것을 세운다(멱등이다).
	...
	// 번호가 둘 다 있어야 큐에 세운다. 한쪽만 있으면 그 판은 반쪽이라, 채운 평가치가 한
	// 사람에게만 보인다.
	//
	// 사람과 색을 같이 실어 보낸다. 그 짝을 아는 것이 여기 곁장부뿐이다(analysisSeat).
	//
	// 색 순서를 고정한다. 맵을 그냥 훑으면 자리 순서가 실행마다 달라진다.
	m.mu.Lock()
	...
	if len(seats) != len(entry.ready) {
		// 반쪽이라 분석하지 않는다. 표시는 걷는다. 걷지 않으면 그 판이 영영 「분석 중」이다.
...
//
// 즉시 돌아온다. 테이블 goroutine 이 부르는 자리다(match.Recorder).
//
// 기록기의 done 보다 반드시 먼저 적힌다. FinishedWith 가 이벤트를 큐에 넣은 뒤 여기가
// 불리고, done 은 그 이벤트를 DB 에 쓴 뒤에야 열린다.
...
// 적으므로 앞선 자리만 세운다.
//
// 즉시 돌아온다. 테이블 goroutine 이 부르는 자리이고, 세우는 것이 논블로킹이라
// (matchAnalyzer.prefetch) 큐가 차 있어도 착수가 늦지 않는다.
