...
//
// analysis_jobs.match_id 를 「방 id」 대신 분석 키로 읽는다(journal §126).
//
...
//
// 이 갈래는 手를 한 번에 다 세운다. 수순 전부를 이미 알기 때문이고, 워커가 몇이든 手들이
// 병렬로 재어진다.
//
// 手를 먼저 세우고 판을 나중에 세운다. 반대로 하면 아직 手가 하나도 없는 판이 집혀 그
// 자리에서 手数만큼을 혼자 재게 된다.
...
// 「崩れやすいところ」도 한 줄 고치지 않고 가져온 판을 같이 센다(journal §126).
//
// retracted_usi 를 적지 않는다. 그 칸은 「개입이 막지 않았다면 뒀을 수」인데 가져온 판에서는
// 누구도 막지 않았고 그 수가 기보에 그대로 남아 있다. 화면은 그 手数의 수를 기보에서
// 찾는다(web 의 ReviewDetail).
