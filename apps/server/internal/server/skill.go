...
// 실력 추정치를 판 사이로 옮기는 두 자리(journal §47). 로그인했고 DB가 있어야 붙고,
// 하나라도 없으면 판마다 초기화된다.

// priorSkill 은 지난 판까지의 추정치다. 없으면 skill.Unknown 이고 기준선 밴드로 시작한다.
//
...
// skillEstimateOf·storeSkillEstimate 는 같은 네 값을 두 어휘 사이로 옮긴다. 추정기가 DB 를
// 모르기 때문에 생기는 자리다(skill 패키지).
...
// saveSkill 은 판정마다 추정치를 덮는 콜백이다. 붙일 자리가 없으면 nil을 돌려주고, 추정기가
// 그때 아무것도 부르지 않는다(skill.NewWorkerFrom).
//
...
func (h *gameHandler) saveSkill(ctx context.Context, userID *int64) func(skill.Estimate) {
	...
	return func(e skill.Estimate) {
		// 연결의 ctx 를 그대로 쓴다. 대국이 끝나면 그 뒤의 쓰기는 취소되는데, 매 수 쓰므로
		// 그때는 이미 마지막 판정까지 저장된 뒤다.
		err := st.SaveSkillEstimate(ctx, id, storeSkillEstimate(e))
		switch {
		case err == nil:
		case errors.Is(err, context.Canceled):
			// 연결이 끊긴 것이다. 잃은 것이 없는데 「저장 실패」가 판마다 한 줄씩 쌓이면
			// 진짜 실패를 그 안에서 찾지 못한다.
		default:
			// 추정이 쌓이지 않으면 다음 판의 첫 몇 수가 기준선이 된다. 판은 그대로 된다.
...
// observing 은 저장 콜백을 감싸 마지막 값을 붙잡는다. save 가 nil이어도(익명) 붙잡는 일은
// 그대로 하고, 돌려주는 함수는 언제나 부를 수 있다(nil 을 주지 않는다).
...
// change 는 총평에 실을 段級 변화다. 표본이 모자라 이름을 붙이지 못하면 nil이다
// (skill.RankOf).
