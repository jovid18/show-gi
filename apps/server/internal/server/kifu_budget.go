...
//
// 하루 몫(maxImportsPerDay)이 이 자리를 막지 않는다. 그쪽은 판이 만들어지는
// 것을 세는데, 옮겨 적는 일은 판을 만들기 전에 일어난다 — 읽기만 반복하는 사람은
// 그 상한에 영영 닿지 않으면서 토큰을 계속 쓴다.
//
// 표에 적지 않는다. 지키려는 것이 돈이라서다 — 배포에 값이 사라지면 그 사람의
// 몫이 한 번 초기화될 뿐이고, 인스턴스가 여럿이면 상한이 그만큼 느슨해진다. 그 둘을
// 고치려면 표가 필요한데, 표를 두면 이 상한이 「기록」이 되어 지우는 규약이 하나 더 생긴다.

// maxTranscribesPerHour 는 한 사람이 한 시간에 정규화를 부를 수 있는 횟수다.
//
// 하루 몫(10판)보다 넉넉하다. 읽기는 여러 번 해 보는 것이 정상이고(형식을 고쳐 가며
// 다시 붙여 넣는다) 막으려는 것은 무한히 도는 쪽뿐이다.
//
...
//
// 사람이 미리보기에서 확인한 판이 가져오는 판과 같아야 한다. 원문을 두 번 받는
