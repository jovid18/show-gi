...
//
// 부르는 데가 둘이다. cmd/importkifu 는 태그가 남의 대국에서도 맞게 붙는지를 보고
// (journal §44) 상수를 기록으로 다시 채점한다(§39). 서버는 사람이 자기 기보를 가져올 때
...
//
// 기보 파일은 레포에 커밋하지 않는다. 받는 법과 env 게이트는 apps/server/README.md.
package kifu
...
// shogi.SquareJa 의 모양이다. 둘 다 받아야 이 패키지가 자기 렌더러의 출력을 다시 읽는다.
...
//
// 콜론이 두 가지다. 전각(：)이 KIF 의 것이지만 반각(:)으로 쓰는 도구가 있다.
...
// startOf 는 手合割 이름으로 0手目를 만든다. 모르는 手合을 平手로 읽으면 첫 수부터
// 반칙이 되고, 그 오류가 手合 때문이라는 것을 누구도 볼 수 없다.
...
func ParseKIF(input string) (ParsedGame, error) {
	...
	for sc.Scan() {
		...
		// 판 위의 색은 그대로다(internal/handicap).
		...
		// 手合割은 시작 국면을 정한다. 읽지 않으면 駒落ち 기보가 平手 위에서 읽히다가
		// 엉뚱해 보이는 반칙으로 죽는다. 수를 하나라도 읽은 뒤에 오면 무시한다. 그때
		// 국면을 갈아 끼우면 앞의 수들이 다른 판의 것이 된다.
		...
		// 그래서 KIF 의 変化 블록은 헤더 줄만 걸러지고 아래 수들이 본선에 이어 붙는다.
...
func parseKIFMove(text string, pos shogi.Position, prevTo int) (shogi.Move, error) {
	...
	// 成香·成桂·成銀 이 2글자라 2글자 조회가 먼저다. 뒤집으면 ４二成銀 이 「成 + 銀」으로
	// 갈려 조회가 깨진다. 銀成(승격 동작)은 표에 없어서 1글자 쪽으로 흘러간다.
	...
	// 수식어(右左直寄引上)를 먼저 걷는다. 成 을 먼저 보면 「３三銀右成」에서 승격이 경고
	// 없이 빠지고, 결과가 합법수라 ValidateMove 도 잡지 않는다.
	//
	// 걷은 것은 버리지 않는다. 원위치가 없는 표기에서는 이것이 출발칸의 하나뿐인 단서다.
	modStart := idx
	...
	if err != nil {
		// 打 를 적지 않은 投入이다. KI2 는 반상의 같은 駒가 그 칸에 갈 수 없으면 打 를 뺀다.
...
// dropAt 은 投入 수를 만든다. 玉과 成한 駒는 持ち駒로 들 수 없다.
...
func ParseCSA(input string) (ParsedGame, error) {
	...
	for sc.Scan() {
		...
		case strings.HasPrefix(line, "'"), strings.HasPrefix(line, "V"),
			...
			continue
		// 投了와 시간초과는 같은 결과다. 手番 쪽이 진다. %TIME_UP 을 받지 않으면 그 판이
		// K 실측의 승패 표본에서 빠진다(journal §44 의 341판에 3판). %KACHI·%TSUMI·
		// %ILLEGAL_* 는 아직 받지 않는다.
...
func parseCSAMove(line string, pos shogi.Position) (shogi.Move, error) {
	...
	from := shogi.SquareOf(fromFile, fromRank)
	// CSA 는 착수 후의 駒를 적어(+8822UM) 표기에 成 이 없다. 출발 칸에 서 있던 駒와 견줘
	// 역산하고, 그래서 CSA 는 앞에서부터 순서대로만 읽을 수 있다.
