...
//
// 상수는 이 테스트가 고치지 않는다. 재는 것과 정하는 것이 따로다(journal §39, §94).
//
...
//
// DB가 필요한 것은 positions 캐시 때문이다. 검토 화면(/explore)이 지나간 국면이 같은
// 깊이로 남아 있어서, 그 수순을 재생하면 판당 탐색 200번이 대부분 캐시로 답한다.
func TestMeasureRankAnchors(t *testing.T) {
	manifest := os.Getenv("SHOWGI_RANK_KIFU")
	dbURL := os.Getenv("SHOWGI_TEST_DATABASE_URL")
	// 엔진 경로는 두 이름을 다 받는다. 임포트 테스트만 SHOWGI_TEST_ENGINE_PATH 를 쓰고
	// 나머지 측정은 SHOWGI_USI_CMD 다(README 의 표).
	...
	// 앵커를 어느 手数 창에서 잴지. 초반은 定跡이라 급수 신호가 없고, 판마다 길이가 달라
	// 끝까지 세면 자가 판 길이의 함수가 된다. 기본값은 런타임과 같아야 한다
	// (skill.AnchorFromPly).
	...
	// 詰み solver 는 없으면 없이 돈다. 종반 판정이 빠져 절대 낙폭이 낮게 나오고, 그것은
	// 급수 사이의 차이를 좁히는 방향이다.
	...
	// 프로덕션과 같은 레벨로 판정한다. 임계치가 갈리면 이 표가 급수 대신 레벨을 잰다
	// (journal §92).
	...
	for _, e := range entries {
		...
		// 手가 0인 쪽은 세지 않는다. 낙폭 0은 「매 수 최선」이라 표를 센 쪽으로 끌고 간다.
		if sente.count(w) > 0 {
	...
	// 한 手도 재지 못했으면 표가 머리만 찍히고 초록으로 끝난다. 사람이 이 표에서 상수를
	// 옮겨 적는 자리다.
	if measured == 0 {
...
// shortSource 는 표에 실을 기보 이름이다. 수순 전체를 찍으면 한 줄이 수백 자가 된다.
...
// rankEntry 는 목록의 한 줄이다. 라벨은 출처의 사실이라 파일 안에 없다. KIF 헤더의
// 이름 칸은 사이트마다 다르다.
...
// rankSide 는 한 판의 한쪽 몫이다. 段級이 절대 낙폭에서, 밴드가 EMA 에서 나오므로
// (skill.Estimate) 두 축을 같이 갖고 있다.
//
// 手를 하나하나 갖고 있다. 「몇 手부터」를 나중에 정할 수 있어야 한다(rankWindow).
...
type rankMove struct {
	...
	drop    float64
	blunder bool
	// decided 는 그 수를 두기 전에 이미 승패가 갈려 있었나다. 표본에서 뺀다. 승률이
	// 포화한 구간이라 나쁜 수도 낙폭이 0에 가깝다(01-core.md §2).
	decided bool
}

// rankDecided 는 「이미 갈렸다」의 기본 경계다. 앵커가 이 경계 위에서 나왔으므로 런타임과
// 같은 값이어야 한다(game.DecidedWinRate).
//
// SHOWGI_RANK_DECIDED 로 옮긴다. 1이면 아무것도 빼지 않는다. 실제로 이 값 하나가
// 「갈린다/아니다」를 뒤집었다.
...
// mean 은 from手 이후의 절대 낙폭 평균이다. 그 구간에 手가 없으면 0 대신 NaN 이다.
...
// rankBigDrops 는 「큰 실수」의 절대 경계들이다. 개입률과 재는 것이 같은데 레벨을 보지
// 않는다. 개입은 임계치가 분모라(intervene.Level.Threshold) 그 값이 갈리면 따라 갈린다.
...
//
// 개입을 걸지 않는다. 물러진 수가 판에서 빠지면 그 수의 낙폭이 표본에서 사라진다.
func measureGame(ctx context.Context, analyst game.Analyst, g ParsedGame, decided float64) map[shogi.Color]rankSide {
	...
	for i := range g.Moves {
		...
		if err != nil {
			// 한 수를 재지 못하면 그 수만 빠진다. 手数가 표에 있어 줄어든 것이 보인다.
			continue
		}
...
// absDrop 은 한 수의 절대 낙폭이다. skill 과 같은 계산인데 그쪽은 판마다의 평균을
// 내놓지 않아 여기서 다시 쓴다(Estimate.AbsLoss 는 누계다).
...
// moverAt 은 그 手数를 둔 쪽이다. Judgement 이 手番을 내놓지 않아 여기서 센다.
...
// reportRankLabels 는 급수마다의 앵커 후보를 출력한다. SD 가 급수 사이의 차이보다 크면
// 눈금을 촘촘히 하는 대신 굵게 해야 한다(journal §39).
func reportRankLabels(byLabel map[string][]rankSide, w rankWindow) {
	...
	for _, label := range sortedRankLabels(byLabel, w) {
		...
		// EMA 도 같이 출력한다. 밴드가 그 값을 본다(skill.Estimate.Loss).
		ema := 0.0
		for _, s := range sides {
			ema += s.ema
		}
...
// 라벨과 이름이 어긋나면 상수가 오래된 것이다(skill.rankAnchors).
...
// reportRankSeparation 은 라벨 둘이 실제로 갈리는지를 출력한다. 갈리지 않으면 경계를
// 어디에 놓아도 표본이 그것을 뒷받침하지 못한다.
...
// 같은 판 수로 더 예민하다(journal §39).
//
// 대신 교란이 하나 남는다. 약한 쪽이 실수하면 상대의 국면이 쉬워져서, 차이가 급수
// 때문인지 국면 때문인지를 이 표로는 가를 수 없다.
...
// meanSD 는 한 라벨의 평균과 표준편차다. 판마다의 평균을 표본 하나로 센다. 手를 전부
// 모아 세면 긴 판이 표를 끌고 간다(journal §39).
...
// labelBigRate 는 라벨 전체에서 큰 실수의 비율이다. 측면마다 재서 평균한다
// (meanSD 와 같은 규약).
...
// rankSegments 는 구간 표의 경계다. 초반의 낙폭이 뒤 구간보다 뚜렷하게 낮으면 그
// 구간은 책이 둔 것이다.
var rankSegments = [...]int{1, 21, 41, 61}

// reportRankPhases 는 급수마다 手数 구간별 평균을 출력한다. 앵커를 몇 手부터 잴지가
// 이 표로 정해진다(SHOWGI_RANK_FROM_PLY).
...
//
// 목록은 레포 밖에 둔다. 제3자 기보를 testdata 에 커밋하지 않고 파생 상수만 넣는다.
//
...
// 한 수도 대신 두지 않아 주소 하나가 기보 하나다(server/explore.go).
