...
func TestParseCSA_Floodgate(t *testing.T) {
	...
	// 결과를 모르는 판도 통과한다. floodgate 실 코퍼스에 종국 표시가 없는 판이 섞여
	// 있다(341판 중 16판). 에러로 두면 기보를 늘릴 때마다 빨개진다.
	//
	// 대신 센다. 「파서가 다 읽었다」와 「절반이 결과 없이 지나갔다」가 같은 초록으로
	// 보이지 않게 한다.
	...
	// 여기서 보는 것은 비율이다. 결과 없는 판이 절반을 넘으면 코퍼스가 이상하거나
	// 파서가 종국 표시를 놓치고 있다.
