...
func TestParseCSA_Floodgate(t *testing.T) {
	...
	// 결과를 모르는 판도 통과한다. floodgate 실 코퍼스에는 종국 표시가 아예
	// 없는 판(끊긴 대국)이 섞여 있다 — 341판 중 16판이었다. 그걸 에러로 두면 이 테스트가
	// 파서 검사 대신 코퍼스 크기 감지기가 되어, 기보를 늘릴 때마다 빨개진다.
	//
	// 대신 센다. 경고 없이 넘기면 「파서가 다 읽었다」와 「절반이 결과 없이 지나갔다」가
	// 같은 초록으로 보인다.
	...
	// 읽히지 않는 판이 하나라도 있으면 위에서 이미 실패했다. 여기서 보는 것은 비율이다:
