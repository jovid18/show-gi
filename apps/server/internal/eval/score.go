...
//
// 35281 이 더 큰데도 눌린 값이 성립하는 것은, 받는 쪽이 크기를 안 견주기 때문이다.
// WinRate(K=600) 는 둘 다 1.0 으로 포화하고 평가치 그래프는 ±1200 에서 자른다.
//
// 숫자를 바꾸지 않는다. 이미 이 자로 적힌 행이 있고(edges.eval_by_depth ·
// game_moves.eval_cp) 섞여도 에러가 안 난다. 그 칸들에 태그가 붙는 날 같이 사라진다.
const MateCp = 30000
