...
//
// 리프 패키지다. usi 도 store 도 여기를 들여오고, 여기는 이 레포의 어느 패키지도
// 들여오지 않는다 — 그래서 순환이 생기지 않는다.
package eval
...
//
// 제로값은 Cp(0) 이고 「호각」을 뜻한다 — 「점수를 아직 모른다」는
// 부르는 쪽이 ok 플래그로 받는다(usi.SearchResult.ScoreAtDepth).
type Score struct {
	mate bool
	v    int
}
...
//
// n 에 0 을 넣지 않는다. 부호가 없어서 Neg 가 관점을 옮기지 못하고(-0 = 0), 그래서 「내가
// 詰んでいる」과 「상대가 詰んでいる」이 한 값이 된다 — 여기서 뜻을 정하면 어느 쪽으로
// 정하든 절반이 거짓이다. 그래서 값이 들어오는 자리에서 막는다(usi.parseScore).
//
// 엔진이 그 값을 내보내지 않는다. 이미 詰んでいる 국면에 물으면 score mate -1 에 bestmove
...
//
// 있는 것은 눌러 적을 자리를 남기지 않기 위해서다 — %d 로 찍으려고 환산 함수를 부르는
// 순간 그 숫자가 다시 돌아다닌다.
...
// 흘려보내고 이쪽은 맨 아래로 보내는데, 그 값은 애초에 만들지 않는다(Mate).
