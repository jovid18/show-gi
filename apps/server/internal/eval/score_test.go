...
// 엔진의 생 cp 가 詰み을 눌러 담던 값(30000 − 10×手数)을 넘어와서(±35281), 숫자 하나로
// 줄을 세우면 1手詰み이 그 뒤로 밀렸다(journal §131).
func TestAWinningMateOutranksTheEnginesRawCeiling(t *testing.T) {
	...
	// 늦게 이기는 詰み도 마찬가지다. 手数가 아무리 커도 cp 뒤로 가지 않는다.
...
func TestCompareOrdersMatesAroundEveryCp(t *testing.T) {
	// 위에서 아래로 좋은 순. 화면 쪽 rankOf 와 같은 규칙이다.
	order := []Score{
...
// 태그가 갈려 있어야 부르는 쪽이 분기한다. 합성값이 다시 생기지 않게 막는 장치가
// 이것뿐이다.
...
// 로그에 적히는 모양. 이 함수가 없어지면 측정 코드가 다시 환산 함수를 찾는다.
