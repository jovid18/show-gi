...
//
// 만드는 것은 판마다 한 번이고, 결과 전체는 저장된다. 판이 끝나면 큐에 들어가고 분석
// 워커가 만든다(server/quiz_jobs.go). 되짚기가 열 때는 읽기만 한다 — 되짚기에서 만들면 그 탐색이
// 진행 중인 다른 대국의 착수를 기다리게 한다.
//
// # 詰み 문항이 왜 완전한 트리인가
//
// 실전 국면은 詰将棋 작품과 다르다. 玉方의 持駒에 「나머지 전부」가 없고, 같은 手数의
...
// 문항은 새 기준을 거치지 않고 만든 것이라 채점 규약이 어긋난다(migrations/007).
const Version = 1

const (
	// MateMaxPlies 는 문항으로 삼을 詰み의 상한이다. 문제는 사람 차례 국면을 手数 순으로
	// 훑어 처음으로 이 안에 들어온 것이다.
	//
	// 「가장 어려운 것」 대신 「최초」인 것은 그것이 판에서 詰み이 처음 성립한 자리이고,
	...
	// 決めた 1手詰め을 다시 내는 것은 문항 대신 되풀이가 된다.
	MateMinPliesIfConverted = 3

	// MateSearchBudget 는 詰み 탐색 횟수의 상한이다. 바깥의 quizTimeout 이 여전히
	// 마지막에 걸리도록 그 아래로 잡았다(journal §53).
	//
	// 탐색을 자르는 자리를 하나로 두려고 이 값이 있다. 예산이 먼저 걸리면
	// 「몇 번까지」와 「몇 분까지」 둘이 서로를 가려서, 무엇 때문에 잘렸는지가 로그에서 갈리지 않는다.
	//
	// 실전 종반의 트리가 몇 번을 쓰는지는 아직 재지 않았다 [미확정]. 잰 것은 시험의 합성
	// 국면뿐이고(3手·11회, §53) 그쪽은 駒가 거의 없어 바닥값이다 — 持駒가 있으면 王手 후보와
	// 合駒가 둘 다 늘고 그 둘이 곱해져서, 뿌리 노드 하나가 이 값 전체를 쓸 수도 있다.
	//
	...
	// BestMultiPV 는 gap을 재는 MultiPV다. 쓰는 것은 1·2위뿐인데 5로 둔 것은 둘 때문이다 —
	...
	//
	// 이 값이 문항의 정의다. 차가 작으면 정답이 사실상 여럿이고, 그런 국면을 내면
	...
	//
	// 추가 탐색이 0이다 — gap을 재면서 이미 손에 들어온 PV를 자를 뿐이다.
	//
	// 짧게 자르는 근거가 §20 · §25다. 개입 카드가 PV를 싣던 때 「어디서 자를지가 국면마다
	// 달라」 두 수에서 끊기기도 했고, 그래서 그쪽은 화면에서 뺐다. 여기가 그때와 다른 것은
	// 묻는 것이 다르다는 하나다 — 저쪽은 「상대가 어떻게 벌하나」였고 여기는 「왜 이 수가
	// 최선인가」라, 한 왕복이면 대개 답이 된다(取り返す·逃げる가 거기 나온다).
	//
	// [미확정] — 사람이 이 문항을 실제로 푸는 것을 아직 보지 못했다(§61: 새 판이 필요하다).
	BestLinePlies = 4
)
...
type MateItem struct {
	...
	// 늘리고, solver 는 그것도 手数로 센다. 늘린 쪽이 실제로 강제되는 手数라 화면에
	// 그대로 적어도 참이다(§53).
...
type BestItem struct {
	...
	// Line 은 정답 뒤에 서로 최선으로 뒀을 때의 수순이다(정답 자신은 없다).
	//
	// 옛 판에는 없다. 이 칸이 생기기 전에 만들어진 문항은 영영 비어 있고, 채우려면
	// 다시 재야 한다 — 하지 않는다(journal §66). 화면은 없으면 그 줄을 그리지 않는다.
	//
	// 정답과 같은 취급이다: 맞히기 전에는 응답에 실리지 않는다(server/quiz.go). 최선 수순의
	// 첫 수가 곧 정답이라, 이것만 내보내도 정답을 말한 것이 된다.
...
// checkingMoves 는 王手가 되는 합법수만 준다. 엔진을 부르지 않는다.
//
