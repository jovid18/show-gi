...
// 문항은 새 기준을 안 거쳐 만든 것이라 채점 규약이 어긋난다(migrations/007).
const Version = 1
...
const (
	...
	//
	// 「가장 어려운 것」 대신 「최초」인 이유는 그것이 판에서 詰み이 처음 성립한 자리이고,
	...
	//
	// 회차를 자르는 자리를 하나로 두려는 것이 이 값의 뜻이다. 예산이 먼저 걸리면
	...
	// BestLinePlies 는 정답 뒤에 적어 둘 수순의 길이다(정답 자신은 안 센다).
	//
	...
	//
	// [미확정] — 사람이 이 문항을 실제로 푸는 것을 아직 못 봤다(§61: 새 판이 필요하다).
	BestLinePlies = 4
)
...
type BestItem struct {
	...
	// 다시 재야 한다 — 안 한다(journal §66). 화면은 없으면 그 줄을 안 그린다.
	//
	// 정답과 같은 취급이다: 맞히기 전에는 응답에 안 실린다(server/quiz.go). 최선 수순의
