...
const (
	...
	// MateSearchBudget 는 트리 하나에 쓸 詰み 탐색 횟수의 상한이다. 탐색 하나가 실측
	// 107ms라(§31) 이 값은 약 2.7분이다.
	//
	// **실전 종반의 트리가 몇 번을 쓰는지는 아직 안 쟀다** [미확정]. 잰 것은 시험의 합성
	// 국면뿐이고(3手·11회, §53) 그쪽은 駒가 거의 없어 바닥값이다 — 持駒가 있으면 王手
	// 후보와 合駒가 둘 다 늘고, 그 둘이 곱해진다.
	//
	// **넘으면 문항을 버린다.** 잘린 트리는 완전하지 않고, 완전하지 않은 트리로 채점하면
	// 「정답을 오답이라고 말하는」 경우가 생긴다.
	MateSearchBudget = 1500
