...
type Input struct {
	...
	// 저절로 지워졌지만(로그 함수를 안 거친 뺄셈이었다) 승률은 그 구간에서 포화한다 —
...
//
// 에러를 안 돌려준다. 문항이 없는 것은 고장 없이도 흔히 나오는 결과이고(10수 만에 投了한
// 판이 실제로 그렇다), 하나를 못 만든 것이 나머지를 버릴 이유도 아니다.
//
// 대신 엔진이 한 번이라도 답했는가를 함께 준다. 거짓이면 빈 결과가 「이 판에 문항이
// 없다」 대신 「아무것도 못 봤다」이고, 그 둘은 화면에서 전혀 다른 말이 되어야 한다 —
...
//
// 엔진이 아예 없는 것은 「못 본」 것과 다르다. 그 배포에는 문항이라는 것이 없고, 그건
// 사실이라 그대로 적어도 된다.
