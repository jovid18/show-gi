...
// 「逃す」가 문장에 있으면 안 된다. 詰み은 남아 있고 길어졌을 뿐이다. 실제로 이긴
// 판에서 그 문장이 나갔다(journal §76).
func TestSlowerMateNeverSaysTheMateWasLost(t *testing.T) {
	...
	// 사실이 없을 때 나가는 문구도 같은 규칙을 지킨다. 되짚기가 그것을 쓴다(BaseMessage).
...
// 착수 後의 手数는 문장에 하나도 나오지 않는다. 탐색이 준 미증명 값이라 같은 국면에
// 14·16·「없음」이 나왔다(journal §76). Facts 에 그 칸이 없는 것이 첫 번째 보증이고,
// 이 테스트가 문장이 다른 데서 숫자를 끌어 오는 날을 잡는다.
func TestSlowerMateSaysNoNumberForTheAfterSide(t *testing.T) {
	...
	// 말하지 않기로 한 사실들도 실려 있지만(slowerMateFacts) 문장에는 나오지 않는다.
