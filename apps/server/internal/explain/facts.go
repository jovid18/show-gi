...
type Facts struct {
	...
	// Known 이 false면 판을 읽지 못해 아래 사실들이 없는 것이다. 지어내지 않는다 —
	...
	//
	// 「노리는 매수」와 다르다 — 핀에 묶여 움직일 수 없는 駒는 세지 않는다. 그리고 수 대신
	// 매수로 센다. 같은 駒의 成·不成은 두 수지만 한 장이라, 수로 세면 화면이
	// 「2枚あり」라고 거짓을 말한다.
	Attackers int
	...
	//
	// 증명된 것만 온다 — 탐색의 mate 점수는 증명을 거치지 않아 틀린 手数를 말하고,
	...
	// slower_mate 에서만 쓴다.
	//
	// 여기도 증명된 것만 온다(MatePlies 와 같은 규칙). 착수 후의 手数는 solver 대신
	// 탐색이 준 값이라 증명이 되지 못하고, 실제로 같은 국면에 14·16·「없음」이 나왔다 —
	// 그래서 문장이 그쪽 숫자를 쓰지 않는다(journal §76).
	MateBefore int

	// Threatened 는 반박 수순의 첫 수로 상대가 딸 수 있는 내 駒의 한자다. 없으면 빈 값.
	//
	// 카테고리가 이유를 대지 못하는 3분의 2에 「무엇을 잃는가」를 주는 자리다(journal §25).
	// 「잃습니다」 대신 「取れます」로 말한다 — 실제로 그렇게 될지는 상대가 정하지만,
	...
	//
	// 이것이 붙는 자리는 되물러서 이미 사라진 국면이다 — 「지금 어떻게 두라」 대신
	...
	//
	// 문장은 아직 이 값을 쓰지 않는다 — 기록과 되짚기가 쓰고, 여기 실려 오는 것은 사실 하나에
	// 태그가 붙어 다니게 두는 편이 나중에 문구를 태그로 가를 때 배선이 늘어나지 않기 때문이다.
	Tags []string
}
...
type Branch struct {
	...
	// cp 칸을 보지 않는다. 詰み에는 cp 가 없다(whatifCandidate 와 같은 판단이다).
	MateIn int
}
...
func (f Facts) used() Facts {
	...
	case intervene.CategorySlowerMate:
		...
		// 탐색이 준 미증명 값이라 문장에 쓸 수 없다(MateBefore). 그래서 이 카테고리의 문장은
	...
	default:
		...
		// 곧 「이 문장은 국면을 짚지 않는다」이고, 그러면 카테고리 문구가 그대로 나간다.
	}
	return u
}
