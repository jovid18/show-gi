...
//
// **최선수를 말하지 않는다**(01-core.md §1). 「왜 나쁜가」까지이고, 어느 수를 뒀어야 했는지는
// 짚지 않는다 — 짚어주는 순간 플레이어는 생각을 멈춘다. 그래서 어느 문구도 수를 부르지 않고
// **무엇을 보라**까지만 말한다.
//
// 플레이 기록이 이 넷을 실제로 읽고 평가했다(08-playtest.md §7). `idle_check` 가 가장 좋았고
...
// Render 는 사실을 **결정적으로** 일본어 문장으로 바꾼다.
//
// LLM이 없을 때·느릴 때·못 쓸 문장을 돌려줄 때 나가는 문구이고, 그래서 **이것만으로도
// 제품이 선다.** 여기에 사실이 실려 있어야 LLM은 「같은 사실을 더 좋은 문장으로」만 하게
// 되고, 없으면 LLM이 없을 때 정보가 같이 사라진다.
//
// 레벨을 안 본다. 어휘를 사람에 맞추는 것이 LLM이 하는 일이고, 여기서 레벨별로 문장을
// 갈라 적으면 그 수를 사람이 손으로 관리하게 된다. **[미확정]** 레벨별 결정적 문구가 필요한지.
