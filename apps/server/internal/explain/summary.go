...
//
// 결과(Outcome)만으로는 말할 수 없는 것이 하나 있어서 둔다 — 이기고 있는데 投了한 판이다
// (journal §68). 형세를 보지 않으면 「졌다 = 무너졌다」가 되어, 사실은 이기고 있던 판을
// 그렇게 배운다.
//
...
// 문장 하나가 갈리고, 넘지 못하면 지금까지와 같은 문장이 나간다.
...
// 그것을 보지 않으면 40手 전의 형세로 「有利でした」를 말하게 된다. 한 왕복까지만 받는다.
...
//
// 숫자를 담지 않는다. 手数도 개입 횟수도 화면이 따로 그리고, 같은 수를 문장에도 적으면
// 두 벌이 되어 어긋났을 때 어느 쪽이 맞는지 알 수 없다 — Facts 가 Δ승률을 싣지 않는 것도
// 그래서다.
//
...
type GameFacts struct {
	...
	//
	// 제로값이 「막지 않았다」인 것이 안전한 쪽이다. 채우지 않고 부르면 개입이 돈 판이
...
func RenderSummary(f GameFacts) string {
	...
	if len(f.Top) == 0 {
		// 개입이 없었다. 칭찬으로 끝내지 않는다 — 한 판에서 걸리지 않았다는 것이 실력의
		// 증거가 되지 못하고(skill 패키지의 FallRate 와 같은 판단), 다음 판을 두게 만드는 것이
		// 이 화면의 일이다.
		//
		// 「형세 손해가 없었다」로 말하지 않는다. 그건 재지 않은 것이다(journal §52) —
		// 임계치를 넘지 않았다는 뜻일 뿐 손해가 없었다는 보장이 되지 못한다. 두 문장 다
	...
	// 「崩れた」는 이기고 있던 판에 쓸 수 없다. 그 판에서 배울 것은 「무너졌다」 대신
