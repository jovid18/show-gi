...
//
// 두 값은 서로 다른 숫자다. 강함은 Estimate.Loss(비대칭 EMA)를, 段級은 Estimate.AbsLoss(창
...
//
// 이 값을 道場나 将棋ウォーズ의 段級으로 읽으면 안 된다. 앵커는 사람끼리 둔 판에서 왔지만
// 재는 자가 승률 낙폭이라 그 사이트의 급수와 따로 논다. 화면도 「目安」로 적는다.
//
...
// 그러면 실측한 세 점이 자기 이름에 정확히 떨어진다. 앵커 사이는 실측 없이 이은 값이다 —
