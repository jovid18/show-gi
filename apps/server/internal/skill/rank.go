...
//
// [미확정] 판 21개 · 라벨 일곱이고 값이 전부 단조롭다. 아래 끝은 판이 들어올 때마다
// ±14% 튀고(0.1265 → 0.1439 → 0.1287) 위 끝은 네 라벨이 한 덩어리로 모인 자리다 —
// 5級 0.0770 · 1級 0.0706 · 初段 0.0659 · 三段 0.0651이 서로 1σ 안이다.
//
// 그래서 이름 사이가 실측이 아니다. 이 자가 가르는 것은 세 덩어리다 —
// {15級 0.1287 · 10級 0.1064} · {5級~三段} · {5段 0.0332}. 인접한 이름의 차이는 확인된
// 사실이 아니고, 화면이 「目安」로 적는 이유가 그것이다(journal §94).
...
//
// 이 값은 道場나 将棋ウォーズ의 段級이 아니다. 앵커는 사람끼리 둔 판에서 왔지만 재는 자가
// 승률 낙폭이라 「그 사이트의 급수」가 아니다. 화면도 「目安」로 적는다.
//
// 한 프로파일에 자가 둘 섞인다. 엔진 대국은 상대가 실력에 맞춰 약해져 국면이 쉬워지므로
// 한 계급쯤 세게 나오는데(journal §94), 사람끼리 둔 판은 그 인플레가 없다 — 두 종류가
// 같은 AbsLoss 로 들어오고 비율은 그 사람이 무엇을 뒀느냐가 정한다(journal §95).
