...
// 「개입이 걸렸나」). 그래서 아래 상수를 흔들어 보는 데 엔진도 대국도 필요 없다.
//
// 추정을 대국 결과로 하지 않는다. 한 판이 끝난 뒤 매기면 이번 판에서 흥미를 잃은
// 사람에게는 늦다(journal §21 ①). 신호는 착수 한 수마다 오고 반영은 언제나 한 수
// 뒤다(game.Opponent). 표본 셋을 채운 뒤이므로 첫 조절은 네 번째 판정의 응수부터다.
package skill

// Move 는 판정이 끝난 사람의 수 한 건에서 추정에 쓰는 값이다. 수도 국면도 없이
// 얼마나 손해였는지만 본다.
type Move struct {
	// Blunder 는 개입이 걸렸는가(= 물러졌는가).
	//
	// DeltaWin 과 따로 받는 것은 종반 판정으로 걸린 수 때문이다. 詰み을 놓친 수는 승률이
	// 포화한 구간에서 나와 낙폭이 0에 가깝고(intervene.JudgeMatePlies), 낙폭만 보면 잘 둔
	// 수로 들어온다.
	Blunder bool
	// DeltaWin 은 승률 낙폭(0~1). 통과한 수도 임계치를 넘지 않았을 뿐 값을 갖는다.
	DeltaWin float64
	// Threshold 는 그 판정에 쓰인 임계치다(intervene.Level.Threshold). 낙폭을 이걸로
	// 나눠 0~1로 만든다.
	//
	// 부르는 쪽이 넘긴다. 여기서 레벨을 보고 고르면 이 패키지가 intervene 을 알게 된다.
	Threshold float64
	// Ply 는 이 수의 手数다. 段級이 창 안의 수만 센다(AnchorFromPly).
	Ply int
	// Decided 는 두기 전에 승패가 이미 갈려 있었나다. 부르는 쪽이 정한다. 그 판정에는
	// 승률 환산이 필요한데(game.DecidedWinRate) 이 패키지는 cp를 모른다.
	Decided bool
}
...
type Estimate struct {
	...
	//
	// 실력 대신 손해로 적는다. 「실력 0.7」은 무엇에 대한 0.7인지가 없지만 낙폭은 임계치에
	// 대한 비율이라 뜻이 하나다. 저장되는 것은 언제나 이 값이고 段級은 파생이다(rank.go).
	Loss float64
	// Samples 는 본 수의 개수다.
	Samples int
	// AbsLoss 는 임계치로 나누지 않은 승률 낙폭의 평균(0~1)이다. 창은 AbsWindow.
	//
	// Loss 와 두 자리가 다르다. 분모가 없어서 레벨이 갈려도 같은 값이고, 오르내림에 같은
	// 무게를 준다. 段級이 여기서 나온다(rank.go).
	AbsLoss float64
	// AbsSamples 는 AbsLoss 에 들어간 수의 개수다.
	//
	// Samples 와 따로 둔다. 이 칸이 뒤에 생겨서 그 전에 쌓인 프로파일은 Samples 가 차 있는데
	// AbsLoss 가 비어 있고, 그것을 0으로 메우면 「매 수 최선」이 된다
	// (014_skill_absolute_loss.sql).
	AbsSamples int
}

// PriorLoss 는 아무것도 보기 전의 값이다. 기본 밴드가 여기서 나오고, 모르는 채로
// 상대를 세게도 약하게도 만들지 않는다.
const PriorLoss = 0.5

// Unknown 은 아직 아무 수도 보지 않은 추정치다. 추정기가 꺼져 있을 때도 이 값이다.
// 「없음」과 「모름」이 둘 다 기준선 밴드로 간다.
var Unknown = Estimate{Loss: PriorLoss}

// RiseRate·FallRate 는 낙폭이 오를 때와 내릴 때의 반영 비율이다. 둘은 비대칭이다.
//
// 나빠지는 쪽이 빠른 것은 도움이 헤매는 그 자리에서 와야 하기 때문이다. 좋아지는 쪽이
// 느린 것은 조용한 국면에서 아무 수나 둬도 낙폭이 0이라서다(journal §21 ①). 거기서
// 빠르게 올리면 방금 통과한 사람이 곧바로 더 센 상대를 만난다.
//
...
// AnchorFromPly·AnchorToPly 는 段級을 재는 手数 창이다. Observe 는 밴드에 이 창을 씌우지
// 않는다(조절은 매 수 일어나야 한다). 판이 끝난 뒤 한꺼번에 먹이는 자리만 예외다
// (InAnchorWindow).
//
// 실측한 앵커가 이 창에서 나왔으므로 런타임도 같은 자를 써야 한다. 창이 없으면 定跡
// 구간이 급수 신호를 지우고, 끝까지 세면 자가 手数의 함수가 된다(journal §94).
const (
	AnchorFromPly = 21
	AnchorToPly   = 60
)

// MateAbsLoss 는 詰み을 놓친 수의 절대 낙폭이다(Move.Blunder). 임계치를 그 자리에 놓으면
// 이 축이 레벨을 다시 보게 되고, 앵커가 그 위에 얹힌다. [미확정] 근거는 journal §94.
const MateAbsLoss = 0.25

// AbsWindow 는 AbsLoss 가 기억하는 수의 개수다. 넘으면 평균이 이 창짜리 대칭 EMA가 된다.
//
// 상한이 없으면 판이 쌓인 뒤 段級이 한 판으로 거의 움직이지 않고, 총평의
// 「対局前 → 対局後」가 늘 같은 이름을 그린다. [미확정] 한 판의 판정 수(50~60)의 두 배다.
const AbsWindow = 120

// MinSamples 는 밴드를 옮기기 전에 볼 수의 개수다. 첫 수 하나로 상대가 바뀌면 사람이
// 알아차리기 전에 강함이 흔들린다. [미확정]
const MinSamples = 3
...
// 임의로 밀 수 있다. adaptive.skillShift 도 같은 판단으로 한 번 더 자른다.
func NewTrackFrom(e Estimate) *Track {
	if e.Samples <= 0 {
		return NewTrack()
	}
	t := &Track{loss: clamp01(e.Loss), samples: e.Samples}
	// 개수가 0이면 그 칸이 없던 시절의 프로파일이라 0에서 다시 센다(Estimate.AbsSamples).
	if e.AbsSamples > 0 {
...
// 밴드도 창 안의 手만 봐야 하는데 Observe 는 밴드에 창을 씌우지 않으므로, 규칙을 두 벌 쓰지
...
func (m Move) InAnchorWindow() bool {
	...
	if m.Ply == 0 {
		// 부르는 쪽이 手数를 실어 주지 않았다. 빼면 그 배선에서 段級이 영원히 붙지 않는다.
		return true
	}
...
// 임계치 이상이라 그 바닥에 걸리지 않고, 그래서 이 축에는 레벨이 들어오지 않는다.
