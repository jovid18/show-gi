...
// 「개입이 걸렸나」. intervene 도 같은 성질을 갖고(judge.go), 그래서
...
// Move 는 판정이 끝난 사람의 수 한 건에서 추정에 쓰는 값이다.
//
// 수도 국면도 없다. 보는 것은 얼마나 손해였는지뿐이다.
type Move struct {
	// Blunder 는 개입이 걸렸는가(= 물러졌는가).
	//
	// DeltaWin 과 따로 받는 것은 종반 판정으로 걸린 수가 있기 때문이다. 詰み을
...
type Estimate struct {
	...
	//
	// 실력 대신 손해로 적는다. 「실력 0.7」은 무엇에 대한 0.7인지가 없지만
...
// 한 수 잘 둔 것이 실력의 증거가 되지 못하기 때문이다. 조용한 국면에서는 아무 수나 둬도
...
// 씌우지 않는다 — 조절은 매 수 일어나야 한다. 판이 끝난 뒤 한꺼번에 먹이는 자리만 예외이고
// 거기서는 부르는 쪽이 두 축에 다 씌운다(InAnchorWindow).
//
// 실측한 앵커가 이 창에서 나왔으므로 런타임도 같은 자를 써야 한다(journal §94). 창이
// 필요한 것은 둘 때문이다 — 초반은 定跡이라 급수 신호가 없고(15級의 1-20手 낙폭이 뒤
// 구간의 3분의 1이다), 끝까지 세면 판 길이가 값을 정해서 자가 手数의 함수가 된다
// (같은 표본에서 급수 순서가 뒤집혔다).
const (
	AnchorFromPly = 21
	AnchorToPly   = 60
...
// 갈린다 — 앵커가 그 위에 얹히므로 쓸 수 없다. [미확정] 근거는 journal §94.
const MateAbsLoss = 0.25

// AbsWindow 는 AbsLoss 가 기억하는 수의 개수다. 넘으면 평균이 이 창짜리 대칭 EMA가 된다.
//
// 상한이 없으면 판이 쌓인 뒤 段級이 한 판으로 거의 움직이지 않고, 총평의 「対局前 → 対局後」가
...
//
// Ply 가 0이면 부르는 쪽이 手数를 실어 주지 않은 것이라 창을 씌울 수 없다. 그때는 넣는다 —
// 빼면 그 배선에서 段級이 영원히 붙지 않고, 그건 조용한 고장이다.
...
// 임계치 이상이므로 그 바닥에 걸리지 않고, 그래서 이 축에는 레벨이 들어오지 않는다.
