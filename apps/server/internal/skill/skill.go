...
// 사람에게는 늦다(06-status.md §21 ①). 그래서 신호는 착수 한 수마다 온다 — 다만 **반영은
...
type Move struct {
	...
	//
	// DeltaWin 과 따로 받는 이유는 **종반 판정으로 걸린 수가 있기 때문**이다. 詰み을
...
// RiseRate·FallRate 는 낙폭이 오를 때와 내릴 때의 반영 비율이다. **비대칭이 요점이다.**
//
// 나빠지는 쪽이 빠르다 — 도움은 헤매는 그 자리에서 와야 한다. 좋아지는 쪽이 느린 것은
// 한 수 잘 둔 것이 실력의 증거가 못 되기 때문이다. 조용한 국면에서는 아무 수나 둬도
// 낙폭이 0이고(06-status.md §21 ①의 「후보가 20~40cp에 몰리는 국면」), 거기서 빠르게
// 올리면 방금 통과한 사람이 곧바로 더 센 상대를 만난다.
//
// **[미확정]** 두 값 다 초기값이다. 근거는 06-status.md §47.
const (
	RiseRate = 0.5
	FallRate = 0.1
)
...
//
// 임계치로 나누므로 **낙폭으로 걸린 블런더는 저절로 1**이 된다. Blunder 를 따로 보는 것은
...
// 달라서 최선수보다 좋아 보이는 값이 나오는 국면이 있다(06-status.md §41).
