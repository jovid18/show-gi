...
func TestEvalByDepth(t *testing.T) {
	...
	if len(best) != 2 || best[0].Depth != 1 || best[0].Score != eval.Cp(31) || best[1].Depth != 2 || best[1].Score != eval.Cp(42) {
		t.Fatalf("7g7f 깊이별 = %+v (d1:31, d2:42 기대)", best)
	}

	// 함정 수: 얕게는 +12, 깊게는 -5
	trap := res.EvalByDepth("2g2f")
	if len(trap) != 2 || trap[0].Score != eval.Cp(12) || trap[1].Score != eval.Cp(-5) {
