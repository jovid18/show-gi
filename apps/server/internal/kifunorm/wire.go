...
// 이 파일은 OpenAI Responses API 의 요청·응답 모양이다. 필요한 칸만 담는다 —
// 남의 스키마 전체를 옮겨 적으면 그쪽이 칸을 늘릴 때마다 이 파일이 어긋난다.

// instructions 는 시스템 프롬프트다.
//
// 시키는 일이 하나다: 글자를 옮겨 적는 것. 고치지 말라고 고정하는 문장이 이 프롬프트의
// 값이다 — 「읽을 수 없는 수를 그럴듯하게 메우는」 것이 이 계층의 하나뿐인 위험이고,
// 그것은 뒤의 룰 검증도 잡을 수 없을 때가 있다(메운 수가 우연히 합법일 때).
//
