...
// 이 파일은 OpenAI Responses API 의 요청·응답 모양이다. 남의 스키마 전체를 옮겨 적으면
// 그쪽이 칸을 늘릴 때마다 어긋나므로 필요한 칸만 담는다.

// instructions 는 시스템 프롬프트다.
//
// 시키는 일이 하나다: 글자를 옮겨 적는 것. 「읽을 수 없는 수를 그럴듯하게 메우는」 것이
// 이 계층의 하나뿐인 위험이고, 메운 수가 우연히 합법이면 뒤의 룰 검증도 잡지 못한다.
//
