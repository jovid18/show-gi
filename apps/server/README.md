...
- **`ROLE` 이 이 프로세스가 줄을 집는가를 정한다.** `both`(기본) · `analysis` 는 집고, `interactive` 는 手를 세우기만 한다. 기본이 `both` 라 태스크 하나인 배포는 지금까지와 같다. **라우팅은 안 가른다** — 어느 티어든 같은 핸들러를 세우고, 사람이 오지 않는 것은 대상 그룹이 정한다. **상호작용 티어를 여러 대로 올리는 손잡이가 아니다**: 방이 메모리에 서므로([journal §98](../../docs/journal/82-100.md)) 그쪽은 방을 프로세스 밖으로 내린 뒤다
