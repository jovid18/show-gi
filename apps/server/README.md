...
| `POST /api/position/read`            | 판이 찍힌 그림에서 국면을 읽는다 ([§129](../../docs/journal/121-140.md)). **로그인 안 했으면 401** — 그림 읽기가 돈을 쓰는 일이라 사람마다 세야 하고, 벽이 **6MB · 10회/시간**이다(넘으면 429 `quota`). 형식은 **앞머리로 정한다**(PNG·JPEG·WebP, 아니면 400 `not_image`). 판이 안 보이면 **422 `no_board`**, 키가 없으면 503. **그림을 안 남긴다** — 요청 하나에 실려 나가고 응답을 만든 뒤 버린다. `OPENAI_API_KEY` 가 없으면 이 뿌리만 안 열린다                                                                         |
...
| `POST /api/kifu/import`              | 그 기보를 판으로 남기고 분석 줄에 세운다. **원문을 다시 보낸다** — 서버에 중간 상태가 없다. 벽이 다섯이다: 64KB · 512手 · 하루 10판 · **정규화 20회/시간** · 분석기가 없으면 503                                                                                                                                                                                                                                                                                                                                            |
...
| `BOARDREAD_MODEL` | `boardread.DefaultModel`      | 판이 찍힌 그림을 읽는 계층의 모델(`internal/boardread`). 키는 `OPENAI_API_KEY` 를 같이 쓰고 모델만 갈라 둔다 — 81칸의 작은 글자와 그 방향을 읽는 일이라 mini 가 아니다. **실측으로 골랐다**([§129](../../docs/journal/121-140.md)): 라벨 8장에서 gpt-5.4 가 92.9%·성립 0/8 인데 기본값이 98.1%·8/8 이다 |
...
**값이 비어 있으면 이 경로가 통째로 안 선다** — 그림도 안 남고 라벨 뿌리(`POST /api/position/label`)도 라우팅되지 않는다. 프로덕션은 그 값을 안 준다.
