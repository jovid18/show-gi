...
> **대인전의 거절은 404 하나다.** 없는 방·만료된 방·남이 이미 찬 방·**로그인 안 한 요청**이 전부 같은 답을 받는다 — 응답 본문까지 같다. 마지막 것이 요점이다: 401은 「로그인하면 볼 수 있다」는 뜻이라, 그것만으로 **그 방이 있다는 사실이 로그인 없이 새어 나간다.** 방 id 는 128비트 난수라 훑을 수 없고(22자 base64url), 정원이 2명이라 두 자리가 차면 링크를 알아도 관전조차 안 된다.
>
