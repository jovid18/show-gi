...
| `BOARDREAD_MODEL` | `boardread.DefaultModel`      | 판이 찍힌 그림을 읽는 계층의 모델(`internal/boardread`). 키는 `OPENAI_API_KEY` 를 같이 쓰고 모델만 갈라 둔다 — 81칸의 작은 글자와 그 방향을 읽는 일이라 mini 가 아니다. **[미확정]** 실측으로 고른 값이 아니라서 갈아 끼울 자리를 남겼다 ([§129](../../docs/journal/121-140.md)) |
