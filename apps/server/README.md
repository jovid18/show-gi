...
- **`ROLE` 이 이 프로세스가 줄을 집는가를 정한다.** `both`(기본) · `analysis` 는 집고, `interactive` 는 手를 세우기만 한다. 기본이 `both` 라 태스크 하나인 배포는 지금까지와 같다. **경로를 없애지는 않는다** — `analysis` 도 같은 핸들러를 세우고 짝짓는 표면(`/api/rooms`·`/ws/match`·`/api/queue`)만 503으로 답한다. 없애면 404가 되어 「배포가 낡았다」와 구별되지 않고, 그대로 두면 대상 그룹 설정이 틀렸을 때 **두 사람이 서로 다른 태스크에서 짝지어져 방을 못 연다** — 조용히 깨지는 자리라 여기서 막는다. **상호작용 티어를 여러 대로 올리는 손잡이가 아니다**: 방이 메모리에 서므로([journal §98](../../docs/journal/82-100.md)) 그쪽은 방을 프로세스 밖으로 내린 뒤다
