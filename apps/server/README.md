...
| `GET /ws/game`                     | 연결 하나가 대국 하나. 엔진이 없으면 503. `?color=b`·`w` 와 `opening=<id>` 로 고른다 ([§48](../../docs/journal/41-60.md)). `?handicap=<id>` 면 駒落ち이고 **그것이 앞의 둘을 덮는다** — 사람은 언제나 下手(先手)이고 진형은 안 붙는다 ([§84](../../docs/journal/82-100.md)). `?resume=<id>` 면 중단된 판을 이어 둔다 ([§51](../../docs/journal/41-60.md))                                                               |
