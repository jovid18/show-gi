...
| `POST /api/queue`                    | 대기열에 서고 그 자리에서 짝짓기까지 한다. **DB가 필요하다** — 큐가 표에 있어야 모든 인스턴스가 같은 큐를 본다. 없으면 503이고 방을 링크로 만드는 쪽은 그대로 돈다. 멱등이라 화면이 2초마다 이것만 부른다 ([§98](../../docs/journal/82-100.md))                                                                                                                                                                          |
| `DELETE /api/queue`                  | 큐에서 빠진다. 없는 사람이 불러도 **204** — 탭을 닫는 자리에서 부르는 경로다 ([§98](../../docs/journal/82-100.md))                                                                                                                                                                                                                                                                                                       |
...
>
> **대기열 둘도 401이다.** 방과 달리 큐에는 「있다/없다」를 말할 대상이 없다 — 감출 id 가 없다.
