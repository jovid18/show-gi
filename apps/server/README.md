...
| `POST /api/kifu/import`              | 그 기보를 판으로 남기고 분석 큐에 세운다. 원문을 다시 보낸다 — 서버에 중간 상태가 없다. 상한이 다섯이다: 64KB · 512手 · 하루 10판 · 정규화 20회/시간 · 분석기가 없으면 503                                                                                                                                                                                                                                                                                                                          |
...
> 대인전의 거절은 404 하나다. 없는 방·만료된 방·남이 이미 찬 방·로그인 안 한 요청이 전부 같은 답을 받는다 — 응답 본문까지 같다. 마지막 것 때문이다: 401은 「로그인하면 볼 수 있다」는 뜻이라, 그것만으로 그 방이 있다는 사실이 로그인 없이 새어 나간다. 방 id 는 영숫자 8자 난수라 훑을 수 없고(62^8, 약 47.6비트), 정원이 2명이라 두 자리가 차면 링크를 알아도 관전조차 안 된다.
>
> `POST /api/rooms` 만 401이다. 아직 방이 없으므로 「있다/없다」를 말할 대상부터 없고, 화면이 그 답을 보고 「로그인하고 다시」를 그려야 한다.
>
...
> `whatif` 는 판(SFEN)을 받지 않는다. 받는 것은 「기보의 몇 手目에서」와 「거기서 어떤 수를 뒀나」뿐이고, 뿌리 국면은 서버가 기록에서 다시 둬서 만든다. SFEN을 받으면 그 표면이 곧 아무 국면이나 깊이 12로 재 주는 공개 엔진이 되고, 그 풀은 대국 세 판이 쓰는 것과 같은 풀이다.
>
> 검토(`POST /api/explore`)는 뿌리 둘을 받는다. 手合割 id 를 받으면 표에서 뿌리를 얻어(`internal/handicap`) 수순을 매번 되짚고, 사진에서 읽어 사람이 확인한 판은 SFEN 으로 받는다([§129](../../docs/journal/121-140.md)) — 재생할 수순이 없는 쪽은 `shogi.Faults` 가 국면 자체를 검사한다. 세 표면이 같은 계산부를 쓰고 뿌리만 다르다([§85](../../docs/journal/82-100.md)). 뿌리가 0手目라 합법적으로 도달하는 국면이면 무엇이든 물을 수 있어서, 위 문단이 걱정한 자리에 가장 가까운 것이 이쪽이다. 그래서 막는 것이 엔진 슬롯 1개 하나다 — 로그인은 [§100](../../docs/journal/82-100.md)에서 걷었다.
...
| `SHOWGI_RANK_KIFU` + `SHOWGI_MEASURE` | 好手 빈도 측정 skip                         | `TestMeasureGoodMove` — 段級 앵커와 같은 기보로 임계치별 好手 수와 걸린 수의 목록을 찍는다([journal §141](../../docs/journal/141-160.md)). 엔진과 DB가 필요하다                                                                                                   |
| `SHOWGI_RANK_KIFU` + `SHOWGI_MEASURE` | 지평 측정 skip                              | `TestMeasureJudgeHorizon` — 판정의 착수 후 탐색을 depth 12와 11로 견줘 낙폭이 기우는지 잰다([journal §142](../../docs/journal/141-160.md)). `SHOWGI_HORIZON_LIVE=1` 이면 캐시 없이 엔진 넷으로 잰다. 엔진과 DB가 필요하다                                         |
| `SHOWGI_RANK_KIFU` + `SHOWGI_MEASURE` | 精度 측정 skip                              | `TestMeasureAccuracy` — 段級 앵커 기보로 급수별 精度(Lichess accuracy 방식)를 찍는다([journal §144](../../docs/journal/141-160.md)). 엔진과 DB가 필요하다                                                                                                         |
...
한 연결이 대국 하나. 서버가 먼저 말을 건다(상대의 수·개입) — 요청/응답으로는 안 된다.
...
- **엔진 풀이 둘이고 크기 손잡이도 둘이다.** 탐색부는 `ENGINE_POOL_SIZE`(기본 3), 詰将棋 solver 는 `ENGINE_MATE_POOL_SIZE`(기본 2). 다른 바이너리이고 잡히는 이유도 달라서 따로 뒀다 — solver 쪽은 종반 판정·詰み 게이지에 되짚기 퀴즈 생성이 얹혀 있고, 그것이 한 번에 수십 초를 잡는다(journal §53). 퀴즈는 집는 티어에서만 돈다([journal §138](../../docs/journal/121-140.md))
...
- **문항을 만드는 큐도 표다**(`quiz_jobs` · `023`). 판을 재는 큐에서 뗀 자리다 — 예산이 밀리초와 5분으로 달라, 한 잡에 두면 그래프가 다 찬 뒤에도 되짚기가 「분석 중」에 머문다([journal §138](../../docs/journal/121-140.md)). 세우는 쪽은 엔진 대국이 끝나는 자리와 가져온 판을 다 잰 자리 둘이고, 집는 쪽은 `SERVER_ROLE` 이 정하는 그 워커다
