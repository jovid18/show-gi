...
- **사후 분석의 워커 수 손잡이는 `ANALYSIS_WORKERS` 다.** 기본이 **탐색부 풀보다 하나 적다** — 워커가 엔진을 풀에서 빌리므로 전부 가져가면 사람이 두는 판이 기다린다. 풀이 커지면 이 값도 같이 커지는 것이 요점이다: 워커가 하나였을 때는 vCPU 를 올려도 이 층이 안 빨라졌다([journal §106](../../docs/journal/101-120.md))
