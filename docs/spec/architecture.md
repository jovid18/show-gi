...
flowchart TB
    ...
    subgraph analysis["ROLE = analysis"]
        direction LR
...
**`ROLE=analysis` 만 나머지 셋과 성질이 다르다.** 위 셋은 「그 기능을 못 쓴다」인데 이쪽은 「이 태스크가 그 일을 맡지 않는다」이고, 그래서 하나가 아니라 표면 전부가 꺼진다. 사람이 여기 닿았다면 대상 그룹 설정이 틀린 것이라 **프로세스마다 경고를 한 번** 남긴다(어느 경로였는지는 요청 로그가 매번 적는다) — 방이 메모리에 서므로([§98](../journal/82-100.md)) 그대로 두면 두 사람이 서로 다른 태스크에서 짝지어져 방을 못 연다.
