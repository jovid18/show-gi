...
**되짚기가 엔진에 안 매인 것이 의도다** — 엔진이 죽어 대국을 못 해도 지난 판은 볼 수 있어야 한다. 반대로 **검토에만 로그인이 필요한 이유는 뿌리가 다르기 때문이다**: 가정 수순은 뿌리가 자기 판이라 기록이 자격을 정하는데, 검토는 뿌리가 手合割 표라 아무 국면이나 depth 12 로 재 달라고 물을 수 있다.
...
flowchart TB
    subgraph nostore["Store = nil"]
        ...
        N2["검토 저장 → 503"]
        N5["검토 자체 → **늘 401**<br/>(로그인이 DB를 요구한다)"]
...
**DB 가 없으면 검토는 503 이 아니라 401 이다.** 엔진만 있으면 서는 표면인데 로그인 자체가 DB를 요구하므로(`authHandler.enabled`), 그 배포에서는 이 경로가 서 있기만 하고 언제나 401 이 된다. 그래도 401 인 것이 맞다 — 「없는 기능」과 「로그인이 필요한 기능」은 화면에서 갈려야 한다.
...
**검토는 로그인이 없으면 401 이지 404 가 아니다.** 라우팅으로 가르면 로그인 안 한 사람에게 404 가 되고, 그건 「없는 기능」으로 읽힌다 — 그 판단은 라우터가 아니라 핸들러가 든다.
