...
flowchart TB
    ...
    subgraph nonorm["KifuNorm = nil"]
        direction LR
        K1["취해 오기는 **된다**<br/>(결정적 파서로 읽히는 기보만)"]
...
**`OPENAI_API_KEY` 가 없으면 취해 오기의 폴백 한 겹만 꺼진다.** KIF·KI2·CSA·USI·평문으로 읽히는 기보는 그대로 들어오고, 안 읽히는 텍스트가 거절된다 — 표면째 닫히는 Google 쪽과 다른 자리다([§126](../journal/121-140.md)). **다만 ECS 에서는 그렇지 않다**: 태스크 정의의 `secrets` 에 이름이 있으면 값을 못 찾을 때 컨테이너가 시작하지 않는다(deploy/README.md §2).

**분석기가 없으면 취해 오기가 통째로 닫힌다**(503). 판만 남고 평가치가 영영 안 채워지는 자리를 만들지 않는다 — 그러면 되짚기가 「解析しています」에 굳는다.
