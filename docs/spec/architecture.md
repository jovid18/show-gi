...
`cmd/api/main.go` 가 `archive.Wrap` 하나를 그 여섯에 나눠 준다. 캐시 판정은 깊이만으로 안 되고 「같은 깊이면 후보가 많은 쪽이 이긴다」다 — 같은 깊이에서도 MultiPV 가 갈리기 때문이다.
...
**전법 선택은 보통 50\~200cp** 라 어느 레벨도 안 걸리고, 銀 이상을 공짜로 주면 제일 너그러운 입문에서도 걸린다. 그래서 「초반 N수는 개입 안 함」이라는 구간이 없다 — 관측 구간의 기본값은 0이고 첫 수부터 판정한다. 자세한 것은 [01-core §2](../01-core.md#2-판정식).
...
**④ 판정이 실패해도 대국은 안 멈춘다.** 대신 경고 없이 넘기지도 않는다 — 개입이 없는 화면은 「이 수는 괜찮았다」와 똑같이 생겼는데, 판정 실패는 확인 자체를 못 한 것이다. 그래서 `notice` 가 따로 뜬다.
...
**`SERVER_ROLE=analysis` 만 나머지 셋과 성질이 다르다.** 위 셋은 「그 기능을 못 쓴다」인데 이쪽은 「이 태스크가 그 일을 맡지 않는다」이고, 그래서 표면 전부가 꺼진다. 사람이 여기 닿았다면 대상 그룹 설정이 틀린 것이라 프로세스마다 경고를 한 번 남긴다(어느 경로였는지는 요청 로그가 매번 적는다) — 방이 메모리에 있으므로([§98](../journal/82-100.md)) 그대로 두면 두 사람이 서로 다른 태스크에서 짝지어져 방을 못 연다.

**`/api/me` 는 로그인이 꺼진 배포에도 있다.** 화면이 「로그인이라는 것이 이 배포에 있는가」를 묻는 자리이고, 없으면 그 물음이 404 가 되어 고장과 구별되지 않는다.

**`OPENAI_API_KEY` 가 없으면 가져오기의 폴백 한 겹만 꺼진다.** KIF·KI2·CSA·USI·평문으로 읽히는 기보는 그대로 들어오고, 안 읽히는 텍스트가 거절된다 — 표면째 닫히는 Google 쪽과 다른 자리다([§126](../journal/121-140.md)). 다만 ECS 에서는 그렇지 않다: 태스크 정의의 `secrets` 에 이름이 있으면 값을 못 찾을 때 컨테이너가 시작하지 않는다(deploy/README.md §2).

**분석기가 없으면 가져오기 전체가 닫힌다**(503). 판만 남고 평가치가 영영 안 채워지는 자리를 만들지 않는다 — 그러면 되짚기가 「解析しています」에 굳는다.

**검토에서 로그인 뒤에 남는 것은 저장한 국면뿐이다.** 그 검사는 라우팅에 없고 핸들러에 있다 — 경로를 없애면 로그인 안 한 사람에게 404 가 되고, 그건 「없는 기능」으로 읽힌다.
