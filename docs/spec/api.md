...
| 남의 것 · 없는 것 | **같은 `404 not_found`**              | 가르면 번호를 훑어 남의 판의 존재를 알 수 있다                                        |

**`404` 를 두 뜻으로 쓰는 것이 의도다.** 「없는 판」과 「남의 판」을 가르면 id 를 하나씩 올려 보며 남의 기록이 있는지 셀 수 있다. `GET /api/rooms/{id}` 가 로그인 안 한 사람에게 `401` 이 아니라 `404` 를 주는 것도 같은 이유다.
...
- **`404 no_summary` 는 대인전이다.** 총평이 세는 것이 개입인데 대인전에는 그것이 없다. **취해 온 판에는 총평이 있다** — 판정을 돌리므로 셀 것이 있고, 다만 문구가 「戻す」에서 「悪手」로 갈린다
...
| `imported`              | 밖에서 둔 판을 취해 온 것. 화면이 이 값으로 「介入」을 「悪手」로 옮긴다 — **아무도 그 수를 막지 않았다** |
| `analyzing`             | 그 판의 평가치를 지금 채우는 중. 「분석 중」과 「남지 않았다」를 가른다. 대인전과 취해 온 판 둘 다        |
...
| `head`·`tail` | **앞뒤를 같이 준다.** 앞만 주면 「뒤가 잘렸는가」를 사람이 알 수 없고, 그것이 취해 오기의 가장 흔한 오류다 |
