...
**`404` 를 두 뜻으로 쓰는 것이 의도다.** 「없는 판」과 「남의 판」을 구별해 주면 id 를 하나씩 올려 보며 남의 기록이 있는지 셀 수 있다. `GET /api/rooms/{id}` 가 로그인 안 한 사람에게 `401` 이 아니라 `404` 를 주는 것도 같은 이유다.
...
- **`warnings` 는 거절이 아니다.** 「駒が N 枚足りません」과 「すでに詰んでいます」이고 둘 다 그대로 분석할 수 있다 — 실물 한 판은 언제나 40장이라 앞엣것이 놓친 駒의 신호이지만, 駒台가 잘려 나간 사진도 정상이다
- **`sfen` 의 手番은 읽기 시점에 언제나 `b` 다.** 사진이 手番을 말해 주지 않아서 그 한 글자는 아직 사실이 아니고, 사람이 「あなたの手番ですか」에 답한 값이 그것을 덮는다. 아래쪽 편이 `b` 다(사진은 찍은 사람의 시점이다)
- **상한**: 그림 6MB · 사람당 10회/시간(`429`). 형식은 앞머리로 정한다(PNG·JPEG·WebP) — 클라이언트가 말한 이름을 안 믿는다
- **`422 no_board` 를 따로 뒀다.** 고장이 아니라 사실이라, 화면이 「다시 눌러 보라」가 아니라 「판이 보이는 그림을 올려라」를 말해야 한다
...
**`sfen` 뿌리는 룰 엔진의 검사를 먼저 지난다** — 성립하지 않는 판이면 `400 bad_position` 이고 엔진에 닿지 않는다. 手合割 뿌리는 수순을 한 수씩 `ValidateMove` 로 지나가므로 재생이 곧 보증인인데, 판 뿌리에는 지나갈 수순이 없어서 `shogi.Faults` 가 그 자리를 맡는다(말 수·玉 수·二歩·行き所のない駒·王手放置). 엔진 쪽 문이 넓어진 것은 아니다 — 막는 것은 SFEN 의 부재가 아니라 아래 슬롯 하나였고, 뿌리가 0手目인 이상 합법적으로 도달하는 국면은 이미 무엇이든 물을 수 있었다([§85](../journal/82-100.md)).
...
`?` 는 없을 수 있는 칸이다. `omitempty` 가 아니라 포인터인 칸은 「0 과 없음이 다르다」는 뜻이다 — cp 의 0 은 호각이라는 실제 값이다.
