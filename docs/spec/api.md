...
- **`imageId` 는 서버가 지은 이름이다**(`board-01`). 모양을 검사한 뒤에야 경로가 되므로 화면이 준 글자가 폴더 밖을 가리킬 수 없고, 성립하지 않는 판은 라벨이 안 된다 — 틀린 라벨은 없는 라벨보다 나쁘다(측정이 조용히 나빠 보인다)
...
`GET` 은 자리를 잡지 않는다. 앉는 것은 `/ws/match` 가 붙을 때다 — 링크를 잘못 누른 사람이 자기도 모르게 자리를 차지하면 정원 2명인 그 방은 아무도 못 들어간다.
...
| `imported`              | 밖에서 둔 판을 가져온 것. 화면이 이 값으로 「介入」을 「悪手」로 옮긴다 — 아무도 그 수를 막지 않았다 |
...
힌트의 계단은 1 = 「어느 駒인가」 · 2 = 「어떻게 움직이나」이고 3은 없다. 한 국면의 답을 통째로 보려면 2회를 쓰므로, 판당 6회는 최대 세 수다.
