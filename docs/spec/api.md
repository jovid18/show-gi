...
- **벽 다섯**: 원문 64KB · 512手 · 사람당 하루 10판(`429`) · **사람당 정규화 20회/시간** · 분석기가 없으면 `503 import_unavailable`
- **하루 몫이 정규화를 안 막는다.** 그쪽은 판이 만들어지는 것을 세는데 옮겨 적는 일은 그 전에 일어난다 — 읽기만 반복하는 사람이 그 벽에 영영 안 닿으면서 토큰을 쓴다. 그래서 벽이 둘이다
...
- **두 뿌리가 같은 모양을 준다** — `{sfen, faults[], warnings[]}`. 확인 화면이 「방금 읽은 판」과 「내가 고친 판」을 같은 코드로 그리는 자리라 갈라 두지 않았다
...
- **벽**: 그림 6MB · **사람당 10회/시간**(`429`). 형식은 **앞머리로 정한다**(PNG·JPEG·WebP) — 클라이언트가 말한 이름을 안 믿는다
- **`422 no_board` 를 갈라 뒀다.** 고장이 아니라 사실이라, 화면이 「다시 눌러 보라」가 아니라 「판이 보이는 그림을 올려라」를 말해야 한다
