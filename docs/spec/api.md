...
| 본문 상한  | 8 KiB                        | 16 KiB                                |
| 로그인     | 불필요 (뿌리가 자기 판이다)  | **필수**                              |
| 후보 k     | 3                            | 3                                     |
| 시한       | 20s                          | 슬롯 대기 3s                          |
| 동시 슬롯  | —                            | **1** (풀 3개 중 2개를 대국에 남긴다) |

**검토에만 로그인이 필요한 이유는 뿌리가 다르기 때문이다.** 저쪽은 뿌리가 자기 판이라 기록이 자격을 정하는데, 검토는 뿌리가 手合割 표라 **아무 국면이나 depth 12 로 재 달라고 물을 수 있다** — 막는 것이 기록이 아니라 엔진이고, 그 벽이 로그인이다.
