...
| 분석 전용 태스크  | `503 not_served_here`                 | 이 태스크는 사람을 안 받는다(`SERVER_ROLE=analysis`). `/healthz`·`/metrics` 만 답한다 |
| 로그인 안 했다    | `401 unauthorized` / `login_required` | 로그인하면 되는 자리                                                                  |
| 남의 것 · 없는 것 | **같은 `404 not_found`**              | 구별해 주면 번호를 훑어 남의 판의 존재를 알 수 있다                                   |

**`404` 를 두 뜻으로 쓰는 것이 의도다.** 「없는 판」과 「남의 판」을 구별해 주면 id 를 하나씩 올려 보며 남의 기록이 있는지 셀 수 있다. `GET /api/rooms/{id}` 가 로그인 안 한 사람에게 `401` 대신 `404` 를 주는 것도 같은 이유다.
...
- **두 단계다.** `parse` 는 판도 안 만들고 엔진도 안 쓴다 — 잘못 읽은 기보에 엔진 몇 분을 쓰지 않기 위해서고, 그 사이에 사람이 手数와 앞뒤 수를 눈으로 확인한다
- **원문을 두 번 보낸다.** 서버에 중간 상태가 없다 — 파싱이 결정적이라 같은 원문이 같은 결과를 준다
- **읽는 순서가 규약이다**: USI → CSA → KIF → KI2 → 평문. 하나라도 성공하면 OpenAI 를 안 부른다. 전부 실패했을 때만 그 계층이 서고, 거기서 낸 표기도 룰 엔진의 전수 검증을 지나야 수가 된다([journal §126](../journal/121-140.md))
- **`400 move` 는 `ply` 를 같이 준다.** 「読み取れませんでした」만으로는 사람이 어디를 고쳐야 하는지 모른다
- **`400 result` 는 기보가 결과를 안 말한 판이다.** 화면이 그때 사람에게 묻는다 — `games.result` 는 주인 관점이라 `win`·`loss`·`draw` 중 하나여야 목록에 뜬다
- **상한 다섯**: 원문 64KB · 512手 · 사람당 하루 10판(`429`) · 사람당 정규화 20회/시간 · 분석기가 없으면 `503 import_unavailable`
- **하루 몫이 정규화를 안 막는다.** 그쪽은 판이 만들어지는 것을 세는데 옮겨 적는 일은 그 전에 일어난다 — 읽기만 반복하는 사람이 그 상한에 영영 안 닿으면서 토큰을 쓴다. 그래서 상한이 둘이다
...
- **읽기만 로그인이 필요하다.** 그림 읽기가 돈을 쓰는 일이라 사람마다 세야 하고, 익명끼리는 구별할 수단이 없다. 검사는 엔진도 DB도 로그인도 안 쓴다 — 순수 룰 계산이라 한 칸을 고칠 때마다 불러도 대국이 쓰는 풀에 안 닿는다
- **`faults` 가 비어야 분석으로 넘어간다.** 하나에서 안 멈추고 전부 준다 — 잘못 읽은 사진은 여러 자리가 함께 틀리고, 하나씩 말하면 사람이 고치고 다시 누르기를 사유 수만큼 반복한다
- **`faults[].square` 는 화면 배열 인덱스(0~80)다.** 칸으로 짚을 수 없는 사유(말 수·玉 수)면 안 온다. 서버와 화면의 좌표 규약이 같아 변환이 없다
...
- **상한**: 그림 6MB · 사람당 10회/시간(`429`). 형식은 앞머리로 정한다(PNG·JPEG·WebP) — 클라이언트가 말한 이름을 안 믿는다
...
- **`label` 은 판독을 재는 그림을 모을 때만 라우팅된다.** 서버가 그 폴더를 안 들고 있으면 경로가 아예 없다 — 파일을 쓰는 자리라 안 쓸 배포에서는 없는 편이 낫고, 프로덕션은 그 값을 안 준다(apps/server/README.md)
- **`imageId` 는 서버가 지은 이름이다**(`board-01`). 모양을 검사한 뒤에야 경로가 되므로 화면이 준 글자가 폴더 밖을 가리킬 수 없고, 성립하지 않는 판은 라벨이 안 된다 — 틀린 라벨은 없는 라벨보다 나쁘다(측정이 이유 없이 나빠 보인다)
...
**`429 busy`** 는 엔진 슬롯을 3초 기다려도 안 났을 때다 — 다시 눌러 볼 수 있는 실패라 `503` 과 따로 뒀다. 이 API 에서 `429` 가 나가는 자리는 여기 하나다.
...
**검토의 뿌리가 둘이고, 같이 못 온다.** `handicap` 은 그 手合割의 0手目이고 `sfen` 은 사진에서 읽어 와 사람이 확인한 국면이다([§129](../journal/121-140.md) — 그 국면은 手合割+수순으로 표현할 수가 없다). 둘을 같이 주면 `400 bad_root`: 어느 쪽이 뿌리인지를 서버가 골라야 하고 그 선택은 화면과 어긋날 수 있다.
...
`sfen` 뿌리에는 `handicapJa` 도 `baselineCp` 도 안 실린다. 임의의 국면에 手合割이 없어 「형세 0」이 정의되지 않고, 그래서 화면이 「互角ライン」을 말하지 않는다.
...
`?color=b`(先手) · `w`(後手) · `r`(振り駒). 振り駒는 서버가 뽑는다 — 클라이언트가 뽑아 `b`/`w` 로 보내면 마음에 안 드는 결과를 다시 뽑을 수 있고, 그러면 振り駒가 그냥 고르는 일이 된다.

`GET` 은 자리를 잡지 않는다. 앉는 것은 `/ws/match` 가 붙을 때다 — 링크를 잘못 누른 사람이 자기도 모르게 자리를 차지하면 정원 2명인 그 방은 누구도 못 들어간다.
...
| 칸            | 주의                                                                                                      |
| ------------- | --------------------------------------------------------------------------------------------------------- |
| `result`      | **기보가 말한 결과**다(先手/後手 기준). 안 말하면 안 오고, 그때 화면이 사람에게 묻는다                    |
| `transcribed` | 결정적 파서가 못 읽어 OpenAI 를 지났나. 참이면 화면이 「AI が書式を読み取りました」를 한 줄 붙인다        |
| `head`·`tail` | **앞뒤를 같이 준다.** 앞만 주면 「뒤가 잘렸는가」를 사람이 알 수 없고, 그것이 가져오기의 가장 흔한 오류다 |
| `handicapJa`  | 平手면 안 온다 — `GameSummary` 와 같은 규약이다                                                           |

**표기는 서버가 만든다**(`shogi.LineJa`). 원문의 표기를 그대로 돌려주면 정규화를 지난 판과 안 지난 판이 화면에서 다른 모양이 되고, 사람이 「이게 내가 올린 그 판인가」를 표기로 확인할 수 없다.
...
- `attempt` 는 화면이 센다. 서버에 안 남는다 — 몇 번 틀렸는지는 지금 이 화면에서만 하고 있는 일이다
...
| `hint`   | **성공해도 즉시 안 뜬다** — 탐색이 끝나면 온다 | 판당 6회. 한 국면은 2단계까지 (`hint_seen`)      |
...
| `finished`        | 이미 끝난 판                               |
| `bad_move`        | USI 형식에 안 맞는 수 · 모르는 메시지 타입 |
...
**룰 위반**은 `internal/shogi` 가 코드를 낸다. `reason` 이 공백을 포함한 영어 구절이라 `snake_case` 로 맞추면 안 걸린다:
...
> 정상 경로에서는 여기 안 온다 — 클라이언트가 서버에서 받은 합법수만 고르므로, 도달하면 국면이 어긋났다는 뜻(서버 버그)이다. 그래도 문구는 반칙 이름과 「무엇이 문제인가」를 함께 적는다 — 「二歩ってなに」에서 막히기 때문이다.
...
로그인 필수. 정원 2명. 방 id 는 영숫자 8자이고 연번을 안 쓴다 — id 가 곧 열쇠다.
...
`waiting` 은 초대 링크를 그리는 데 필요한 것들을 싣는다. `record` 는 판이 끝난 뒤 한 번 오고 「振り返り」 링크가 그 값으로 만들어진다. 총평은 안 온다.
...
`not_your_turn` · `finished` · `bad_move` · `room_closed` (누구도 안 들어온 채 30분이 지났거나, 방을 만든 사람이 그 뒤로 방을 더 만들어 이 방이 밀려났다 — 호스트당 열린 방 1개).
