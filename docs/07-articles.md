...
- **제품이 아는 쇼기에 이름이 없다.** 囲い·전법·手筋 태그가 하나도 없어서 설명이 「그 駒가 잡힙니다」 층에 머문다. 「将棋を教えてくれる」로 쓰면 거짓말이 된다
- 국면 캐시를 쓰는 곳이 없다 / 詰み 게이지 화면 없음(판정은 돈다) / Google 로그인 없음(익명 대국만) / three.js 없음(2D 보드만) / 리뷰 화면 없음 / 정석 북 없음
