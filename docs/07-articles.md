...
- ~~LLM 계층은 붙었지만 켜져 있지 않다~~ — **켜졌다.** 실키가 SSM에 들어갔고 실제 모델로 재봤다([06-status.md §28](06-status.md)). 이제 쓸 수 있는 것은 **지연(1.7~3.2초)·모델 이름·문장 품질**이고, 여전히 못 쓰는 것은 **비용**이다 ↓
...
- 상수 K(600)·레벨별 임계치·상대 밴드·갇힘 힌트의 3과 5가 **전부 초기값 그대로다.** ~~실측 전이다~~ **재채점은 했고 하나도 안 옮겼다**([06-status.md §39](06-status.md)) — 그런데 그 결론이 「맞는 값이었다」가 아니라 **「표본이 전부 에이전트라 사람 기준으로 정할 것이 없었다」**다. 「실측으로 잡았다」로 쓰면 거짓말이 된다
...
- ~~**제품이 아는 쇼기에 이름이 없다.**~~ — **이름은 들어왔다**([06-status.md §30](06-status.md)). 다만 **쓰는 쪽이 아직 화면 하나뿐**이다: 태그가 판에 뜨는 데까지이고, RAG 검색 키·제안형 힌트 게이트·「정석 이탈」 카테고리에는 아직 안 이어져 있다. 그래서 설명은 여전히 「그 駒가 잡힙니다」 층이고, 「将棋を教えてくれる」로 쓰면 아직 거짓말이 된다
- ~~국면 캐시를 쓰는 곳이 없다~~ **캐시는 쌓고 다시 꺼내 쓴다**([06-status.md §37](06-status.md)) — 다만 **히트율을 안 쟀고 프로덕션에서는 아직 한 행도 안 쌓였다.** 「같은 국면 1.54s → 27ms」는 로컬 숫자다 / Google 로그인 없음(익명 대국만) / 정석 북 없음 / 선행 계산 없음
- **리뷰 화면은 들어왔다**([06-status.md §33](06-status.md) · [§37](06-status.md)) — 한 판을 되짚고, **어느 국면에서든 그 자리에서 둬 본다.** 「네 방향 분기」와 이동 애니메이션도 들어왔다. 남은 것은 **평가치 스파크라인** 하나이고, **대국 중의 개입 화면에서는 아직 둬 볼 수 없다** — 「대국 중에 가정 수순을 둬 볼 수 있다」로 쓰면 거짓말이 된다
- ~~three.js 없음(2D 보드만)~~ — **판 표면은 three.js가 그린다**([06-status.md §35](06-status.md)). 다만 **원근이 아니다**: 정사영 카메라에 판과 픽셀이 맞는 평면이고, 그 위의 駒·광선·표식은 그대로 DOM이다. 「3D 보드」라고 쓰면 거짓말이 된다 — 3D 예산은 **利きの그늘** 한 겹에 다 들어갔다
...
**D5·D6이 들어오면 A·B에 각각 서너 편이 더 붙는다** — ~~LLM 계층, 리뷰 화면, three.js~~ **리뷰 화면.** LLM 계층은 [§27·§28](06-status.md)로, three.js 판 표면은 [§35](06-status.md)로 들어왔고, 적응형 상대(밴드 제어)는 [§16](06-status.md)으로 이미 들어와 2026-08-10 재싱크에 반영됐다. 나머지가 들어올 때 이 목록을 갱신한다.
