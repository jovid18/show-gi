...
| ⬜   | 이미지 동기화 확인 — 첫 도판을 이 테스트 기사에 얹어서 본다  |
| ⬜   | 확인 끝나면 테스트 기사 삭제, 동기화 브랜치를 `main`으로     |
...
공식 문서는 **「등록한 브랜치에 변경이 있으면 자동으로 동기화된다」**고 한다. 작업 브랜치는 `docs/zenn-article`인데 [CLAUDE.md](../CLAUDE.md)상 `main` 머지는 사람이 GitHub PR 화면에서 하므로, **Zenn 설정의 동기화 브랜치를 작업 브랜치로 옮겨서** 머지 없이 확인했다. 대시보드에 下書き로 떴다.

> **끝나면 `main`으로 되돌릴 것.** 그대로 두면 이 브랜치가 사라진 뒤 동기화가 조용히 멈춘다.
