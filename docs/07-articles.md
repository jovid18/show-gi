...
- **LLM 계층은 붙었지만 켜져 있지 않다**([06-status.md §27](06-status.md)). `internal/explain` 이 Tier 0·1·2를 다 갖고 테스트도 도는데, `ORCA_API_KEY`가 SSM에 `unset` 자리표시자라 **실제로 나가는 문구는 결정적 템플릿이다.** 그러니 「AIが説明してくれる」는 아직 사실이 아니고, 「そう作った · 鍵を入れれば動く」까지가 사실이다. **한 번도 실제 모델을 부른 적이 없다** — 지연·비용·히트율·문장 품질이 전부 미측정이라 숫자를 쓰면 거짓말이 된다
