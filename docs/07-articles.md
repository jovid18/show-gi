...
- 상수 K(600)·레벨별 임계치·상대 밴드·갇힘 힌트의 3과 5가 **전부 초기값 그대로다.** **재채점은 했고 하나도 안 옮겼다**([06-status.md §39](06-status.md)) — 그런데 그 결론이 「맞는 값이었다」가 아니라 **「표본이 전부 에이전트라 사람 기준으로 정할 것이 없었다」**다. 「실측으로 잡았다」로 쓰면 거짓말이 된다
...
- **캐시는 쌓고 다시 꺼내 쓴다**([06-status.md §37](06-status.md)) — 다만 **히트율을 안 쟀고 프로덕션에서는 아직 한 행도 안 쌓였다.** 「같은 국면 1.54s → 27ms」는 로컬 숫자다 / Google 로그인 없음(익명 대국만) / 정석 북 없음 / 선행 계산 없음
