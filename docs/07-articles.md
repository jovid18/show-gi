...
| 실제 소스          | **말미에 GitHub 임베드 하나.** SHA 퍼머링크                        |
| 독자가 돌려볼 코드 | 그 블록만 복사해서 돌아가는 최소 예제. 우리 코드에서 자르지 않는다 |

**번역했다는 사실을 말미에 밝힌다.** 안 밝히면 임베드의 글자와 본문의 글자가 달라서 독자가 어느 쪽이 진짜인지 모른다.
...
`pnpm zenn:preview`를 띄우고 **브라우저로 연다.** C1에서 확인된 것: mermaid · `:::message` · `:::message alert` · ` ```diff hcl ` · 코드 파일명 라벨 · 표 · GitHub 임베드 · 목차. 전부 실제로 그려진다.

**mermaid와 GitHub 임베드도 로컬에서 그대로 그려진다.** 이 둘은 `embed.zenn.studio` iframe으로 나가는데, 프리뷰 페이지가 `listen-embed-event.js`를 같은 곳에서 불러오기 때문에 **온라인이면 로컬에서도 Zenn과 같은 것이 보인다.** 다이어그램 문법이 맞는지도 여기서 확인된다.
...
### GitHub 임베드가 한글 주석을 그대로 내보낸다 — 눈으로 확인함

위의 「주석은 일본어로 옮긴다」는 실제 렌더를 보고 정한 것이다. `infra/ecs.tf`를 임베드하면 **한글 주석 아홉 줄이 그대로 화면에 뜬다.** 일본 독자에게는 읽히지 않는 덩어리다.

그래서 임베드는 **말미에 하나**, 「원본은 여기 있다」는 표시로만 쓴다. 본문에서 설명에 쓰는 코드는 번역한 발췌로 간다.
