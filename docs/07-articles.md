...
`zenn preview`를 띄우고 `/api/articles/<slug>`의 `bodyHtml`을 본다(§11과 같은 방법). C1에서 확인된 것: mermaid · `:::message` · `:::message alert` · ` ```diff hcl ` · 코드 파일명 라벨 · 표 · GitHub 임베드.

**다만 mermaid와 GitHub 임베드는 `embed.zenn.studio` iframe으로 나간다.** 로컬에서 확인되는 것은 「블록이 인식돼 iframe이 됐다」까지고, **다이어그램 문법이 맞는지는 Zenn에 올라간 뒤에 봐야 한다.** 첫 동기화에서 이것부터 본다.
