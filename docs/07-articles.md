...
- 상수 K(600)·레벨별 임계치·상대 밴드·갇힘 힌트의 3과 5가 전부 초기값이고 실측 전이다
...
- 국면 캐시를 쓰는 곳이 없다 / Google 로그인 없음(익명 대국만) / 정석 북 없음
- **리뷰 화면은 들어왔다**([06-status.md §33](06-status.md)) — 다만 **한 줄을 앞뒤로 넘기는 데까지**다. 설계에 적힌 「네 방향 분기」와 이동 애니메이션, 평가치 스파크라인은 아직 없으므로 「가정 수순을 둬 볼 수 있다」로 쓰면 거짓말이 된다
