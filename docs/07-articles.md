...
- **「적응형 상대」는 이제 사람에게도 적응한다**([06-status.md §47](06-status.md)) — 판정의 낙폭을 쌓아 밴드를 매 수 옮기고, 12 국면 중 9에서 실제로 다른 수가 나왔다. **E4의 결론이 바뀐 것이 아니라 뒷부분이 생겼다**: 「고정 밴드에 국면을 맞추고 있었다」가 진단이고 그 뒤에 「그래서 사람 쪽을 붙였다」가 온다. 다만 **상수 다섯이 전부 초기값이고 `skill_profile` 에 안 쌓인다**(판마다 초기화된다) — 「레이팅」이라고 쓰면 판을 가로질러 남는 값처럼 읽혀 거짓말이 된다
...
- **캐시는 쌓고 다시 꺼내 쓴다**([06-status.md §37](06-status.md)) — 다만 **히트율을 안 쟀고 프로덕션에서는 아직 한 행도 안 쌓였다.** 「같은 국면 1.54s → 27ms」는 로컬 숫자다 / **Google 로그인은 프로덕션에서 돈다**([06-status.md §46](06-status.md)) — 다만 안 누르면 익명이다 / 정석 북 없음 / 선행 계산 없음
