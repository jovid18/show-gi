...
4번이 필요한 이유는, PR을 나중에 다시 볼 때 "이 변경이 DB를 건드렸는가"를 diff로 찾게 하지 않기 위해서다. 라벨이 있다는 것과 무엇을 돌려야 했는지는 다른 정보다.
...
지표는 api 컨테이너가 stdout 으로 내는 EMF 한 줄에서 나온다([§90](../docs/journal/82-100.md)). CloudWatch 가 로그에서 뽑아 `show-gi` 이름 공간에 넣으므로, 만들 자원이 따로 없고 태스크 정의의 `ENVIRONMENT` 가 그 스위치다.
